...
- page stays intentionally narrow: top-level sections `Обновление данных витрины` / `Расчёт поставок` / `Отчёты`, separate buttons `Загрузить данные` / `Отправить данные`, two compact read-only collapsible block `Ежедневные отчёты` / `Отчёт по остаткам` inside `Отчёты`, compact status, one compact `Сервер и расписание` block and one fixed-height scrollable `Лог` block with `Скачать лог`
...
- stock-report block reads current business day stocks only from persisted ready snapshot `DATA_VITRINA[today_current]`, keeps threshold `<50`, uses all six canonical district keys and surfaces short truthful labels (`Центральный ФО`, `Северо-Западный ФО`, `Приволжский ФО`, `Уральский ФО`, `Юг и СКФО`, `ДВ и Сибирь`)
