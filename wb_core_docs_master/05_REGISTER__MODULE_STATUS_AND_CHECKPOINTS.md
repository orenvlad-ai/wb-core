...
- page uses `POST /v1/sheet-vitrina-v1/refresh`, `POST /v1/sheet-vitrina-v1/load`, `GET /v1/sheet-vitrina-v1/daily-report`, `GET /v1/sheet-vitrina-v1/stock-report`, `GET /v1/sheet-vitrina-v1/plan-report`, `GET /v1/sheet-vitrina-v1/status` and `GET /v1/sheet-vitrina-v1/job`
...
- plan-report block now exposes a third compact reports subsection `Выполнение плана`; `GET /v1/sheet-vitrina-v1/plan-report` stays cheap read-only and server-driven, reads only accepted closed-day temporal snapshots for `fin_report_daily` + `ads_compact`, uses active `config_v2` as current SKU truth, accepts `period + quarterly buyout plans + plan_drr_pct` and always returns the selected period plus MTD/QTD/YTD blocks with `Выкуп / DRR / Рекламные расходы` fact-vs-plan metrics
- plan-report buyout plan math is calendar-day based across quarter boundaries, so windows like `last_7_days`, `last_30_days`, `current_year`, `QTD` and `YTD` sum the per-day quarter plan for each included date instead of reusing one quarter total blindly; ads-spend plan = `buyout_plan * plan_drr_pct / 100`, while DRR remains a fixed percent block compared directly against `ads_sum / fin_buyout_rub * 100`
