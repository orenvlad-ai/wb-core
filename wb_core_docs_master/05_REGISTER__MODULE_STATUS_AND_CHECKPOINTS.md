...
- stock-report block reads current business day stocks only from persisted ready snapshot `DATA_VITRINA[today_current]`, keeps threshold `<50`, uses five district keys (`stock_ru_central`, `stock_ru_northwest`, `stock_ru_volga`, `stock_ru_ural`, `stock_ru_south_caucasus`) and deliberately excludes merged bucket `stock_ru_far_siberia` / `ДВ и Сибирь`
