...
- page stays intentionally narrow: top-level sections `Обновление данных витрины` / `Расчёт поставок` / `Отчёты`, separate buttons `Загрузить данные` / `Отправить данные`, one compact read-only block `Ежедневные отчёты` inside `Отчёты`, compact status, one compact `Сервер и расписание` block and one fixed-height scrollable `Лог` block with `Скачать лог`
...
- daily-report JSON also carries `metric_ranking_diagnostics`, so the observed `3`-item decline list is now explicitly explained as `raw=11`, `present=10`, `negative=3`, `positive=7`, with `avg_ads_bid_search` excluded because both closed-day values are missing
