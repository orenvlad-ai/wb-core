...
- status/refresh responses drive the block through `server_context`, so timezone/scheduler wording is not hardcoded in UI; `Автоцепочка` now truthfully describes the full daily chain `Ежедневно в 11:00, 20:00 Asia/Yekaterinburg: загрузка данных + отправка данных в таблицу`
