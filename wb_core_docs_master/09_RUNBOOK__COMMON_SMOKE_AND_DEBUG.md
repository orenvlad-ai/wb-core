...
| в `Топ-5 самых снизившихся метрик` показывается только `3` позиции | truthful data shape for the current closed-day pair: diagnostics should show `raw_candidate_count=11`, `present_after_none_filter_count=10`, `negative_count=3`, `positive_count=7`, with `avg_ads_bid_search` excluded because both closed-day values are missing |
