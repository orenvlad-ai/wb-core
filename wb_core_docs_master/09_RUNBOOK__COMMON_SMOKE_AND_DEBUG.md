...
- `GET /v1/sheet-vitrina-v1/plan-report` остаётся cheap read-only JSON path: route принимает only `period + q1/q2/q3/q4_buyout_plan_rub + plan_drr_pct + optional as_of_date`, читает only persisted accepted closed-day temporal snapshots `fin_report_daily` + `ads_compact`, не trigger-ит refresh/upstream fetch и truthfully возвращает either `status=available` with selected period + `MTD/QTD/YTD` fact-vs-plan blocks, or `status=unavailable` with `coverage.missing_dates_by_source` when coverage incomplete;
- subsection `Выполнение плана` keeps compact server-driven UX only: browser sends operator-entered quarter plans and plan DRR, while buyout/DRR/ads calculations, cross-quarter day splitting and status labels stay repo-side; no server-side user preferences or Apps Script logic are added;
...
| `Выполнение плана пока недоступно` при ожидаемо готовом историческом покрытии | missing/stale deploy, broken `GET /v1/sheet-vitrina-v1/plan-report` route, invalid operator query params либо неполное accepted closed-day coverage по `fin_report_daily` / `ads_compact` в одном из дат периода |
