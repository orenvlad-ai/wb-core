...
- operator page показывает narrow server-driven surface: top-level sections `Обновление данных витрины` / `Расчёт поставок` / `Отчёты`, separate actions `Загрузить данные` / `Отправить данные`, two compact collapsible block `Ежедневные отчёты` / `Отчёт по остаткам` inside `Отчёты`, compact Russian chrome для status/`Лог` и row-count labels плюс один compact server-driven block `Сервер и расписание`; log viewport при этом fixed-height scrollable, completed run можно скачать через `Скачать лог`, а raw log/error text и technical values остаются canonical;
...
- `GET /v1/sheet-vitrina-v1/stock-report` остаётся cheap read-only JSON path: route читает current business day only from persisted ready snapshot `DATA_VITRINA[today_current]`, не trigger-ит refresh/upstream fetch и включает только SKU с district stock `< 50`;
...
| `Отчёт по остаткам пока недоступен` при ожидаемо готовом current snapshot | missing/stale deploy, broken `GET /v1/sheet-vitrina-v1/stock-report` route, либо current ready snapshot `default_business_as_of_date(now)` не materialized или его `today_current` slot не совпадает с current business date |
