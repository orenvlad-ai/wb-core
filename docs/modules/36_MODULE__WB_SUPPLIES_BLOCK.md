...
For an ordinary supply, `FF → WB = max(packed - accepted, 0)` until final acceptance. Accepted quantity is never manually added to WB: official contour snapshot owns WB quantity. At final acceptance the transit layer closes and positive `packed - final accepted` enters the separate pooled `Расхождения приёмки WB` warehouse by SKU. `Допринято` never repeats the FF debit; it consumes only a positive discrepancy of the same SKU, while surplus becomes transitional unmatched audit. Planned quantity/date, upload date, ambiguous identity and fabricated zero are forbidden substitutes.
