...
- Additional read-only methods reused by `Поставки -> Расчёты -> Подобрать склады WB`:
  ...
  - acceptance/options normalization treats official `result[]` rows as barcode-level evidence, flattens nested `result[].warehouses[]`, groups business options by destination warehouse, carries factual all/partial barcode coverage and nests coefficient/date evidence under `dates[]`; per-barcode upstream errors become controlled warnings/blockers, so mixed success/error can still return partial options;
  - `GET /api/tariffs/v1/acceptance/coefficients` is read from the Common/Tariffs API base (`WB_TARIFFS_API_BASE_URL` override) as coefficient/date/allowUnload evidence for ranking; `coefficient=-1` is treated as unavailable/problematic and ranked below `0/1`;
  ...
  - the regional planning response includes major-warehouse diagnostics for key Central district warehouses, so expected warehouses such as Коледино and Электросталь are either visible in grouped options or have explicit evidence explaining absence/hidden reason.
...
Since `2026-07-01`, the cost layer used by that movement is the immutable canonical snapshot of the exact `ff_stock_ledger` debit, not the latest FF cost line by nmID. Accepted quantity enters WB rolling only with accepted status, final accepted quantity and factual accepted date. Transit/accepted Fulfillment add-ons attach to the same supply/SKU component graph. `Недопринято WB` is derived from sent minus cumulative accepted/doprinato and stays inside `ФФ → WB`; direct original identity wins, otherwise strict `warehouse + destination + nmID` FIFO, with future layers and surplus rejected.

At the `2026-07-01` boundary every authoritative pre-cutover WB operation is legacy audit-only and does not enter opening replay or underaccepted. A post-cutover composition mismatch remains blocked unless the exact operation and all source/line/evidence fingerprints are pinned by `CUTOVER_POSTCUTOVER_SOURCE_NORMALIZATION_V1`; the supply-level normalized pool preserves aggregate quantity/capital and never crosses supply identity.
...
WB supplies with authoritative business dates before cutover remain immutable source audit only and are not replayed into opening WB/FF→WB quantity, capital or underaccepted. Post-cutover composition normalization is possible only for operations pinned by every identity and line-set fingerprint in `CUTOVER_POSTCUTOVER_SOURCE_NORMALIZATION_V1`; allocation never crosses a supply, warehouse or date, and raw persisted evidence is unchanged.
