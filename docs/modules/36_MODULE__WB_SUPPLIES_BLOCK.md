...
At the one-time `2026-07-01` opening boundary only, `CUTOVER_IMMATERIAL_ANOMALY_POLICY_V1` may preserve a fully identified checkpoint-matched historical `accepted > sent` or unmatched doprinato discrepancy as internal audit evidence. Every limit and the global 20-unit budget is fingerprinted; applied acceptance never exceeds sent, the surplus creates no inbound/capital and cannot offset another SKU. This is not an ordinary WB supply normalization rule: future/post-cutover evidence and missing identity/date/cost remain blocked.
