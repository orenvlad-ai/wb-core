---
title: "Модуль: dark baseline/diff engine реестра изменений"
doc_id: "WB-CORE-MODULE-56-CHANGE-REGISTRY-BASELINE-ENGINE"
doc_type: "module"
status: "foundation_dark"
purpose: "Транзакционно фиксировать явные joint Prices+Ads checkpoints, нормализованные observations, identity incidents и только доказанные checkpoint_diff facts; давать deterministic interval projection без автоматической активации."
...
update_note: "Dark explicit-invocation baseline/checkpoint/diff/projection engine; no scheduler, HTTP/UI, writer instrumentation, manual pending, WB mutation or production invocation."
---

# 1. Activation and transaction boundary

`ChangeRegistryBaselineEngine` is an internal callable only. It has no timer,
scheduler, refresh hook, HTTP/UI route, writer instrumentation or automatic
startup invocation. It accepts only the canonical sanitized
...
baseline and never create facts. A later complete invocation compares only with
the immediately previous complete checkpoint and then advances the complete
chain. Complete checkpoints are strictly chronological; same-time conflicting
or out-of-order complete evidence is rejected.
...
produce a fact. Unchanged values create none. A disappeared target never becomes
zero, null, delete or transition fact.
...
Every `checkpoint_diff` fact has exact before/after values, interval
`previous.completed_at .. current.completed_at`, deterministic evidence identity
...
interval mismatch, before/after mismatch, unproven campaign absence, missing
transition proof and ambiguous evidence gaps. It does not calculate outcomes,
causality, performance, recommendations or ML features.

# 6. Excluded scope

Excluded: scheduler/timer/manual HTTP scan, production engine invocation,
...
cursor projection and rejection of unbound/non-checkpoint proof.
