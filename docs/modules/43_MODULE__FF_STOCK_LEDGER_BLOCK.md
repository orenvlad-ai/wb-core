...
Ordinary WB status/facts can create one guarded FF writeoff. Status `4` transfers only the cumulative `acceptedQuantity` delta into WB capital and leaves `sent - accepted` in `ФФ → WB`; transitions `3 → 4 → 5` reuse the original debit/cost snapshot and reject accepted regression. `Допринято` explicitly creates no second debit and only reconciles persisted `Недопринято WB` outstanding state. Unknown nomenclature, negative stock/capital or ambiguous identity fail closed. A narrowly bounded historical surplus can be classified only by `CUTOVER_IMMATERIAL_ANOMALY_POLICY_V1`: exact checkpoint/date/SKU/cost, strictly pre-cutover, integer, ≤3 per line, ≤5 per operation and ≤20 globally. It is visible as raw/applied/surplus audit evidence, creates no quantity/capital and never closes another SKU; every post-cutover or out-of-budget surplus remains blocked.
...
Module 45 may classify the single exact historical orphan `Допринято 40654176 / nmID 391663632 / qty=1` as zero-capital audit evidence only under its full fingerprinted guard. That classification never calls `record_wb_supply_debit`, never creates an FF receipt/writeoff/reversal, never changes supply `40561872` and does not generalize to another supply/SKU/date/quantity.
