...
The additive `build_inventory_balance_evidence` read model exposes explicit `stock_wb`/`stock_ff`, recalculates availability-adjusted demand for the Balance-owned 7/14/30/60-day setting, and emits only supplier-registry `production`/`in_transit` milestones with empirical completed-shipment ETA evidence. The incumbent `Общее` row remains strict: aggregate-only WB evidence leaves its `stock_wb`, incident projection, regions and forecast unknown. A private Balance-only seam may promote a complete exact per-SKU official aggregate total into the separate evidence response only after exact requested-SKU coverage, pagination, snapshot-date, numeric-quantity and raw-digest validation; it records aggregate provenance and never creates warehouse/region allocation. It does not change the existing `Общее` forecast or write behavior; module 53 owns the all-fronts opening, coefficient and pacing semantics.
