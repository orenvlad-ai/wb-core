...
- The runtime-extended catalog also makes the existing SKU `buyoutPercent` metric effective-visible without mutating the persisted legacy registry bundle. Both the DATA snapshot builder and Web Vitrina read contract use the same extension; the read contract may complete an older ready snapshot only from the existing exact-date `sales_funnel_history` temporal snapshot, while future refreshes materialize the same official-api value normally. The read contract additionally derives `TOTAL|buyoutPercent` per exact date with positive-`orderCount` weighting and blank-without-denominator semantics. Because TOTAL and SKU share the one public key `buyoutPercent`, metric-presentation v4 treats them as one proven common pair for order/status/presets/user config; disabled legacy `avg_buyoutPercent` is not re-enabled or rendered.
