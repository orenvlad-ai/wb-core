...
- Канонический operator-facing supply surface в том же repo-owned page:
  ...
  - planning-zone XLSX files use stable stems (`wb_regional_central_north_fo.xlsx`, `wb_regional_central_east_fo.xlsx`, `wb_regional_central_south_fo.xlsx`, plus unchanged non-Central stems) and include Russian headers `nmId / SKU / Количество к поставке / Дефицит`; the deficit is backend-calculated, not browser recomputation.
