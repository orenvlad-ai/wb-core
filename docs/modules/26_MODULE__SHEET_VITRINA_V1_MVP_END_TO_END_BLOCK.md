...
- Канонический user-facing UI surface:
  ...
  - plan-report block `Выполнение плана` остаётся read-only и server-owned:
    ...
    - operator UI defaults are WB/VB-specific when persisted plan inputs are absent/invalid: `H1=155379879`, `H2=294620121`, `DRR=6`, `period=first_half`, contract start enabled with `2026-02-01`, annual-even disabled; `H2=294620121` is the annual remainder to `450000000`, while Q3+Q4 source figures total `294620120`, so the 1 rub discrepancy is explicit
    ...
    - annual-even mode uses the same daily annual load for H1 and H2 dates and does not rewrite persisted H1/H2 inputs
