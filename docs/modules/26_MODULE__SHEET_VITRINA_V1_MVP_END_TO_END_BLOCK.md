...
- Канонический user-facing UI surface:
  ...
  - top-level tabs = `Витрина`, `Расчет поставок`, `Отчеты`, `Отзывы`, `Исследования`; no separate top-level `Обновление данных` tab is active
  ...
  - web-vitrina read surface:
    ...
    - table controls are compact by default: the former separate `Фильтры и настройки` card is not rendered, while `Диапазон`, `Поиск`, `Секции`, `Группа`, `Scope`, `Метрики`, `Столбцы` and `Сортировка` live in one toolbar above the table and continue to use only browser-local filter/search/sort/column-visibility state over the already loaded server payload
