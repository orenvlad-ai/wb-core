...
- Канонический user-facing UI surface:
  ...
  - plan-report block `Выполнение плана` остаётся read-only и server-owned:
    ...
    - ordinary operator UI exposes only period, H1, H2, DRR and `Рассчитать`; the former contract-start and annual-even toggles are not visible operator controls
    - operator UI defaults are WB/VB-specific when persisted plan inputs are absent/invalid: `H1=155379879`, `H2=294620121`, `DRR=6`, canonical contract start `2026-02-01`, annual-even disabled; default period follows the current WB/VB target half-year (`first_half` through 2026-06-30, then `second_half`); `H2=294620121` is the annual remainder to `450000000`, while Q3+Q4 source figures total `294620120`, so the 1 rub discrepancy is explicit
    ...
    - backend compatibility keeps annual-even query mode, but ordinary UI always sends canonical `annual_plan_evenly_distributed=false` because WB/VB evidence matches H1/H2 denominator mode
