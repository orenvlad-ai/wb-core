...
related_runners:
  ...
  - "apps/sheet_vitrina_v1_proxy_v4_transit_repair_smoke.py"
  - "apps/sheet_vitrina_v1_proxy_v4_refresh_preservation_smoke.py"
...
update_note: "Proxy V4 excludes both accrued and capitalized transit from percentage expenses through an explicit versioned formula and a guarded 2026-08-14..last-closed repair; Finance/canonical cost and V3 remain unchanged."
---
...
- Канонический operator-facing supply surface в том же repo-owned page:
  ...
  - only supplier orders in derived `production`/`in_transit` status with an explicit target facility contribute their positive unmatched-to-receipt remainder. Unassigned `NULL` targets are excluded until an internal operator assigns an active facility; there is no Moscow/default planning fallback. Explicit Оренбург never enters Moscow coverage. `accepted_ff`, cancelled/inactive and non-product/unmatched lines are excluded
...
- Active bounded publication с `2026-07-01` выполняет только `packages/application/warehouse_functional_economics_backfill.py` через repo-owned команды `warehouse-functional-economics-dry-run/apply`:
  - она читает frozen/canonical daily WB compatibility WAC и effective versioned calculation parameters, затем публикует только свой reviewed historical cost/coverage/Proxy 3 scope; forward-only WB+FF blend с `2026-08-22` ordinary refresh не превращает эту legacy utility в неявный historical writer;
...
| `wb_stock_incident_qty*`, `total_wb_stock_incident_qty*` | exact selected physical warehouse quantity | legacy incident audit/persisted history | archived from public catalog/read/filter/UI; evaluator/evidence retained |
| `wb_stock_effective_qty*`, `total_wb_stock_effective_qty*` | `fact − incident` | legacy availability audit/persisted history | archived from public catalog/read/filter/UI; evaluator/evidence retained |
...
| `price_seller_discounted` | exact-slot seller discounted price per enabled SKU | Web Vitrina price row | unchanged active SKU identity and history |
| `avg_price_seller_discounted` | arithmetic mean of available enabled-SKU `price_seller_discounted` values | legacy TOTAL `Цена продавца средняя` | stable legacy key/formula/data retained for raw and historical audit; excluded from new public plans/catalog/read/filter/settings/picker/presets |
| `weighted_price_seller_discounted` | exact-slot `SUM(price_seller_discounted × orderCount) / SUM(orderCount)` over positive `orderCount` | active TOTAL `Цена продавца взвеш.` | runtime-extended active/public TOTAL paired explicitly with `price_seller_discounted`; no denominator or any positive-weight missing/invalid price is blank |
...
| `our_wb_unit_cost_rub`, `proxy_profit_3_rub`, `proxy_margin_3_pct` and TOTAL keys | forward-only exact as-of WB+FF physical capital/quantity blend plus versioned calculation parameters | Web Vitrina and indicative Proxy 3; SKU Management read side | active/public; pre-2026-08-22 ready values remain frozen WB compatibility |
| `proxy_profit_4_rub`, `proxy_margin_4_pct` and TOTAL keys | immutable latest-common-week Buyout/Finance V4 parameters plus the same per-SKU WB+FF informational blend | Web Vitrina | active/public only from fixed `2026-08-01`; cost switches forward-only on 2026-08-22 |
...
- Current informational inventory-cost keys are runtime-extended from repo code:
  - `our_wb_unit_cost_rub` / `total_our_wb_unit_cost_rub` = `Себестоимость наша, ₽/шт`; from `2026-08-22`, SKU is `SUM(exact WB + FF location capital) / SUM(exact physical quantity)`, and TOTAL repeats the ratio after summing all included SKU/location capital and quantity. FF facility/pool rows are reconciled disclosure, not extra stages. Missing positive-quantity cost/location/version/coverage is blank with evidence. Earlier ready dates retain WB-only compatibility values and are not rematerialized by this change;
  ...
  - `proxy_profit_3_rub` / `total_proxy_profit_3_rub` is indicative. Before `2026-08-22` its entire prior ready compatibility projection stays frozen, including the covered-sales operands/WB cost. On/after that boundary it uses full date-specific `orderSum/orderCount/ads_sum` and the same per-SKU exact-date WB+FF informational cost row. New-boundary formula is `orderSum × buyout_rate × retained_share − orderCount × buyout_rate × informational_inventory_WAC − ads_sum`; TOTAL is the sum of complete SKU calculations, not a global-WAC shortcut and not a realized-sale coverage subset.
  ...
  - `proxy_profit_4_rub` / `total_proxy_profit_4_rub` and `proxy_margin_4_pct` / `proxy_margin_4_pct_total` use the separate effective immutable V4 version only on/after `2026-08-01`, and the same forward-only per-SKU informational inventory cost as Proxy 3. Formula is `orderSum × V4 buyout_rate × (1 − V4 included_expense_rate) − orderCount × V4 buyout_rate × informational_inventory_WAC − ads_sum`; `other_expense_rate` is exactly `subscriptions + paid_services + review_points + other_deductions + acceptance − capitalized_acceptance`, so neither accrued nor capitalized transit can change included expenses, retained share, V4 profit or V4 margin. Missing positive-order operand makes SKU and TOTAL blank. TOTAL sums eligible SKU profits and divides by summed eligible expected buyout revenue, never averages SKU margins or applies the global blended WAC shortcut.
...
- Seller price keeps three deliberately distinct identities. SKU `price_seller_discounted` is unchanged. Legacy TOTAL `avg_price_seller_discounted` keeps its arithmetic evaluator and persisted historical rows but is archived from ordinary active/public surfaces. New TOTAL `weighted_price_seller_discounted` (`Цена продавца взвеш.`) is an explicit evaluator branch, not an `avg_` naming inference: for each business-date slot it uses only same-slot `price_seller_discounted` and `orderCount`, ignores non-positive weights, returns blank when total positive weight is absent, and fails closed to blank when any positive-weight SKU lacks a finite positive price. A prices group refresh therefore acquires the exact-date `sales_funnel_history` dependency. The registry extension is idempotent and does not mutate the two existing metric records.
- This seller-price schema change has no historical backfill/refetch path. Ready snapshots created before `weighted_price_seller_discounted` existed remain byte-semantically unchanged; period assembly projects their missing new row as blank and only newly materialized applicable snapshots carry a value.
- The runtime-extended catalog makes the existing SKU `buyoutPercent` metric effective-visible without mutating the persisted legacy registry bundle, but its public value is fail-closed behind the fixed `Asia/Yekaterinburg` D-6 maturity boundary. Read projection discards every persisted D0..D-5 zero/non-zero value and renders blank/`—` for both all enabled SKU rows and the one `TOTAL|buyoutPercent`. Mature values come only from exact-date `sales_funnel_history` snapshots with capture provenance on/after their D-6 boundary and full enabled-SKU coverage; a polluted or same-day-captured ready row is never a fallback. The read contract derives mature TOTAL with positive-`orderCount` weighting and blank-without-denominator semantics. Because TOTAL and SKU share the one public key `buyoutPercent`, metric-presentation v5 treats them as one common pair for order/status/presets/user config; disabled legacy `avg_buyoutPercent` is not re-enabled or rendered. Ordinary refresh checks only D-7..D-6 for mature capture/catch-up and skips already proven dates. Historical official reconciliation uses separate guarded dry-run/explicit-apply captures backed by Seller Analytics CSV `DETAIL_HISTORY_REPORT`: current mature proof starts `2026-07-13`, and V4 requires one final exact `2026-07-06..2026-07-12` official repair with all `33 × 7 = 231` pairs. It records official report/CSV provenance and applies only through reviewed manifest, exact deployment, human gate, backup and reconciliation. The resulting mature exact-date snapshots feed Vitrina, the Settings reference and immutable V4 source windows; no `DATA_VITRINA`, timestamp-only rewrite or arithmetic fallback is allowed.
- The ordinary Vitrina refresh uses its existing seam to inspect the latest three closed slots and compute the exact READY COMPLETE Buyout∩Finance intersection, then selects exactly its freshest one common Monday-Sunday week. Buyout and every Finance rate are direct SUM/SUM only inside that selected week; the separate Settings combined one-to-three-week reference is analytics only. A missing newest slot falls back to the freshest earlier common READY week; zero common weeks keep the last immutable V4 version (or fail closed before initialization). It creates at most one new revision for a changed selected-source fingerprint, including arrival of a fresher previously missing week, carries the latest manual V4 tax, and reports ready/stale/pending/historical-repair-required without loading expensive history. `proxy_profit_4_v2_no_transit` stores its version explicitly; persisted v1 revisions remain v1 and are never reinterpreted by a later deployment. Initial as-of V4 projection remains stricter and separate: after exact official proof it creates the reviewed 3-of-3 versions effective `2026-08-01` (`2026-07-06..2026-07-26`) and `2026-08-08` (`2026-07-13..2026-08-02`), then updates only V4 rows of ready snapshots. Dates `2026-08-01..2026-08-07` resolve the first version and `2026-08-08..2026-08-09` the second; the not-yet-ready `2026-08-03..2026-08-09` slot cannot blank those historical metrics. Every later ordinary full refresh copies all V4 SKU/TOTAL cells for business dates before the current date from the previous ready plan, including blank cells, while current-day V4 and every non-V4 source row remain eligible for normal refresh. The guarded repo-owned `apps/sheet_vitrina_v1_proxy_v4_transit_repair.py` appends the corrected immutable revision effective `2026-08-16`, fills 14–15 August and recalculates every closed snapshot from 16 August through the exact last closed date of its fresh manifest. It never targets the current/future date column. Dry-run is default and pins exact snapshot keys/cells, operands, source-week amounts, legacy/corrected rates, prior-version/V3/Finance/WAC/COGS/non-V4/non-target digests and the split Finance raw-store identity. Apply requires the exact manifest, deployed SHA and separate approval reference, creates a capacity-checked coherent verified backup, uses one parameter insert plus plan-json CAS transaction, and performs exact readback; repeat is an idempotent no-op. The older generic `apps/sheet_vitrina_v1_proxy_v4_reconcile.py` remains the reviewed initialization-manifest drift repair. Pre-boundary values remain blank; V3 semantics/history are unchanged.
...
- Legacy Proxy 1 calculation remains server-side audit logic only:
  ...
  - `proxy_profit_3_rub` always uses effective versioned `buyout_rate` and included expense rates. Before `2026-08-22` the prior ready projection, including covered-sale operand selection and WB cost, stays unchanged; on/after that boundary it uses full date-specific proxy operands plus shared exact as-of WB+FF physical `our_wb_unit_cost_rub` and never reads realized Finance coverage/COGS as a new proxy input. It never reads Proxy 2;
  ...
  - `proxy_profit_4_rub` and `proxy_margin_4_pct` resolve only the V4 revision effective on the row's business date and are blank before `2026-08-01`; their cost operand follows the same forward-only WB+FF informational rule as Proxy 3, and their TOTAL rows sum per-SKU results then use ratio-of-aggregates semantics;
...
- Пустой или неполный `COST_PRICE` dataset не валит active refresh:
  ...
  - current public truth не подменяет the WB+FF informational cost/Proxy 3 or exact Finance/Partner COGS with legacy group values.
...
- Explicit runtime-derived aggregations are exceptions only by stable key and declared evaluator contract: `weighted_price_seller_discounted` is the positive-`orderCount`, exact-date, fail-closed weighted TOTAL and never changes the arithmetic `avg_price_seller_discounted` rule.
...
- Production deploy выполняет только one-shot Release Runner после exact-head PR Gate и merge; прямой checkout/deploy из незамёрженной branch запрещён. Перечисленные команды остаются canonical implementation seam внутри release path, а не разрешением на branch-side production deploy.
...
- Smoke проверяет:
  ...
  - что canonical `stock_*`, WB WAC / Proxy 3 and product-capital rows remain active, while every incident/effective family is hidden-not-deleted through the archived public-metric boundary;
  ...
  - что V3 formula/history remain byte-for-byte semantically unchanged while V4 keeps its own fixed boundary, immutable latest-common-week ordinary versions, unchanged frozen historical initialization, automatic tax effective date, missing-operand blanks and eligible TOTAL ratio;
