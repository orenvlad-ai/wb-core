...
- Канонический user-facing UI surface:
  ...
  - web-vitrina read surface:
    ...
    - activity/reporting inside that sibling page stays server-owned: `Загрузка данных` now renders as a full-width compact table over existing source outcomes with today/yesterday server-business dates, OK/not-OK status cells, reason columns, Russian metric labels and secondary technical endpoints; `Лог` stays below it with the existing download contour; the former `Обновление данных` activity block is no longer active page surface, while raw technical note/traceback payload stays in the existing log path and `Свежесть данных` keeps server-owned semantics but uses the same readable timestamp style as `Последнее обновление страницы`
