...
- Канонический operator-facing supply surface в том же repo-owned page:
  ...
  - the shared `Остатки ФФ` block also exposes calculation-only selector `Учесть WB-поставки` backed by `GET /v1/sheet-vitrina-v1/supply/wb-supplies/overlay-options`; selector options include only statuses `2/3/4/6`, statuses `1/5` are not rendered, selected ids are sent explicitly as `selected_wb_supply_ids` in both factory-order and WB regional calculate requests, while backend always revalidates status/date/composition/active SKU/mapping from server runtime cache; long unmapped-warehouse warnings are collapsed behind details in the operator UI
