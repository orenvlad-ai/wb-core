...
- Канонический user-facing UI surface:
  ...
  - session controls are exposed in the unified `Витрина` loading table for `Seller Portal / бот` after source-status details are explicitly loaded; the archived manual operator panel remains available only as embedded compatibility context:
    - top compact `Сессия` before the table is a read-only status indicator derived from existing last-run runtime/source-status/result-state, not from an automatic session probe on page/table load
    - `Проверить сессию` manually performs a cheap probe against saved `storage_state.json`, truthfully distinguishes `session_valid_canonical / session_valid_wrong_org / session_invalid / session_missing / session_probe_error`, displays active/expired/check-error to the operator and may update the indicator from that manual check
    - `Установить сессию` is the only recovery action in the unified loading table: it starts the repo-owned recovery lifecycle and downloads the reusable launcher as soon as readiness is explicit, without rendering extra launcher/retry/stepper buttons or nested controls
