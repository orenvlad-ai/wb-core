...
- The ordinary Vitrina refresh uses its existing seam to inspect the latest aligned three-week Buyout/Finance window. It creates at most one new V4 revision for a new source-window fingerprint, carries the latest manual V4 tax, and otherwise reports ready/stale/pending/historical-repair-required without rewriting frozen versions or loading expensive history. Initial as-of V4 projection is a separate guarded runner: it creates exact effective versions `2026-08-01` and `2026-08-08`, then updates only V4 rows of existing ready snapshots with pre-boundary blanks, V3/non-target digests and idempotent readback.
