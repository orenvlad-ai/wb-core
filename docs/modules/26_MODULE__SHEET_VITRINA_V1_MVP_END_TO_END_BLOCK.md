...
- Канонический operator-facing refresh surface:
  ...
  - в том же `Обновление данных` current operator page additionally показывает compact block `Проверка и восстановление Seller-сессии`:
    ...
    - `Восстановить сессию` стартует repo-owned recovery lifecycle
    - `Скачать launcher для Mac` отдаёт reusable `.command`, который сам поднимает SSH tunnel к localhost-only noVNC, ждёт local HTTP-ready и открывает browser по explicit websocket path/reconnect URL
