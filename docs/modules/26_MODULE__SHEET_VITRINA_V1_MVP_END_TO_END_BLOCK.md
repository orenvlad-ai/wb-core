...
- Канонический user-facing UI surface:
  ...
  - plan-report block `Выполнение плана` остаётся read-only и server-owned:
    ...
    - DRR fact = `ads_sum / fin_buyout_rub * 100` and is diagnostic; ads plan follows WB/VB semantics `max(buyout_plan, buyout_fact) * plan_drr_pct / 100`, with payload diagnostics showing whether the base was plan turnover or fact turnover due to overperformance
...
- Канонический operator-facing supply surface в том же repo-owned page:
  ...
  - regional block renders six federal-district checkboxes for regional share methodology with short primary labels `ЦФО`, `СЗФО`, `ПФО`, `УФО`, `ЮФО/СКФО`, `ДВФО/СФО` and a separate `Доставка, дней` input per district. The delivery value means lag until goods are available on WB (delivery + acceptance/sorting/appearance on sale), defaults to `15` per district, and is preserved independently from district inclusion. Default district inclusion is all districts; excluding a district removes it from share validation/normalization and from result/download surfaces while keeping it visible in selector/options and in the diagnostics text line `Исключённые округа: ...`; direct XLSX download for an excluded district returns a controlled JSON error instead of a 500 or empty workbook.
  ...
  - regional result surface gives server-driven summary and a compact district table immediately under the result totals: `Федеральный округ / Рекомендовано / к поставке / Дефицит / Скачать XLSX / Подбор WB`. It shows only included districts from the latest methodology settings; excluded districts stay out of summary/download rows. Each included row links to the district XLSX route, so a duplicated lower `Excel/XLSX по округам` list is not rendered. Rows with positive planned quantity expose `Подобрать склады WB`, which opens a read-only planning panel for the selected district.
  - WB regional planning panel is bounded to the latest regional result and server-owned nomenclature barcodes. It blocks visibly on missing `nmId -> barcode` rows, otherwise calls official WB FBW `acceptance/options` with `barcode + quantity`, enriches returned options with warehouse district mapping, coefficients, box tariff and transit tariff evidence, and ranks same-district/available/lower-coefficient/direct/lower-known-tariff/earlier-date options above weaker evidence. It is a manual handoff assistant only: copy JSON includes district, warehouse/date/route, products `barcode + quantity`, warnings and evidence; no FBW/FBS supply is created or drafted.
...
- WB supply cost layer rolling uses a normalized local business-day key from `supply_date`: both `2026-07-03` and `2026-07-03T00:00:00+03:00` roll into `2026-07-03`; invalid dates are skipped, planned quantity cannot create a confirmed bucket, and zero-stock inbound carry is applied only inside recalculation while persisted bucket values stay capped to stock.
