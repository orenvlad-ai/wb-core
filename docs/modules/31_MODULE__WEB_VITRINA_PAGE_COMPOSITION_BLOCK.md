...
- `apps/seller_portal_feedbacks_complaint_submit_smoke.py`
  - confirms submit candidate selection (`yes` before `review`, no `complaint_fit=no`), preliminary non-exact matches defer to the filter-aware resolver rather than authorizing submit, hard denylist skip for `GPe9vrq0kctlSfobrgq2`, duplicate skip, ready complaint reason validation, explicit real-submit flag requirement, hard max-submit cap 1, click-alone-is-not-success classification, validation/network/success classification, network sanitizer and journal record shape.
- `apps/seller_portal_feedbacks_complaints_status_sync_smoke.py`
