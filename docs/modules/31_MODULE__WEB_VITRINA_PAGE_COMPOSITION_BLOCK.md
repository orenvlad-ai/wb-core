...
- `GET /sheet-vitrina-v1/vitrina` теперь является реальной usable web-vitrina page:
  ...
  - status next to the load action is visually attached to data loading and means the last load window only: `today_current` + `yesterday_closed` in `Asia/Yekaterinburg`. Old snapshot errors elsewhere in the selected historical table range must not force visible `Ошибка`; errors/warnings inside that latest two-day window still surface truthfully.
...
- period read status can still aggregate source semantics for the selected ready-snapshot window in the backend payload, but the operator-facing header status next to `Загрузить` is the latest two-day load window (`today_current` + `yesterday_closed`) and must not be driven red by old errors outside that window.
