...
- `GET /sheet-vitrina-v1/vitrina` теперь является реальной usable web-vitrina page:
  ...
  - table controls are one compact toolbar above the table, not a separate `Фильтры и настройки` section: `Диапазон`, `Поиск`, `Секции`, `Группа`, `Метрики`, `Столбцы`, `Сброс` share the same desktop line and reuse the existing local filter/search/internal stable order/column-visibility state; `Тип строк`, visible `Сортировка` selector and row-count `Итог` block are not rendered. `Метрики` in the toolbar remains the primary source of included metric rows: it is a compact checkbox multi-select with a top `Все метрики` indeterminate/all toggle, count suffixes from the filter surface, namespaced browser-local restore, default all-visible behavior, and safe fallback to all metrics for corrupted or obsolete stored keys.
  - a separate collapsible `Метрики` presentation block sits after the toolbar and before the table. It shows only metrics currently selected in the toolbar, groups them by current scope/section metadata, and stores browser-local presentation preferences only: order inside the group, one anchor metric, and visible vs hidden-under-anchor placement. The block uses the same dark operator theme as the page, renders compact rows instead of card-in-card metric tiles, uses short zone labels `Показано` / `Скрыто`, short action labels `Скрыть` / `Показать`, and on desktop lays out the two primary scope buckets as four compact columns (`scope shown`, `scope hidden`, `next scope shown`, `next scope hidden`) with responsive collapse on narrow screens. Hidden metrics are not shown in the table until the anchor toggle (`Показать ещё N` / `Скрыть`) is opened; reset returns order/anchor/hidden state to safe server/view-model defaults. This block is not a metric registry, formula, ready snapshot or accepted-truth owner.
  ...
  - custom browser floating controls in the unified shell close on outside click and `Escape`: historical period popover, main metric multiselect, column multiselect, feedbacks date-range picker, research SKU/metric multiselects, research date-range pickers and embedded operator stock-report SKU selector. Inside clicks for checkbox multiselects/date-range selection stay inside the control, and the rule is browser-only UX state, not backend/data truth.
...
- Browser-side state now exists only as page-local UI state for filter/search/internal ordering plus namespaced browser-owned preferences for main-table metric selection and optional column visibility.
...
- Table control UX is intentionally thin:
  ...
  - search, section/group/metric filters, column visibility and reset live in the compact toolbar next to `Диапазон`; the former `Тип строк` toolbar control is not rendered and obsolete stored state is ignored safely; internal stable row ordering remains browser-local without a visible sort selector
  - the collapsible `Метрики` presentation block is browser-local state over selected toolbar metrics only: it can reorder selected metrics inside current scope/section groups, choose one anchor per group and hide selected metrics under that anchor, while the table applies that order/collapsed state without changing metric registry, formulas, ready snapshots or accepted truth; its four-column desktop layout is presentation-only and derives scope labels from existing metadata rather than introducing a new truth model
