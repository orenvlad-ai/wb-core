...
update_note: "Phase 4 live page composition остаётся server-driven; вкладка `Отзывы` теперь содержит подразделы `Отзывы`, `AI-промпт разбора` и `Жалобы`, snapshot-independent bounded feedback date picker, strict server-side feedback period/star/answered load diagnostics, official WB review tags/chips (`review_tags` from fields such as `bables`), Excel export of the current table, resizable feedback columns, full-width prompt editor, discovered AI model selector and transient OpenAI-backed analysis via saved server-side prompt/model. `Жалобы` читает operational runtime-журнал и запускает только async read-only status sync job из WB `Мои жалобы`: public POST быстро возвращает `run_id`, UI poll-ит job route и перечитывает журнал после финала. Real complaint submit remains CLI-only with explicit guarded runner and is not exposed as a public UI submit route; the runner proves the WB `Опишите ситуацию` field value after blur before final click, records sanitized submit payload description evidence when available, and blocks future submit when AI reason says the review has no text while API/UI tags exist. Post-submit uncertainty is handled by read-only confirmation and detail/network probe runners that may update only the runtime journal after direct-id or strict strong-composite proof."
---
...
- `GET /sheet-vitrina-v1/vitrina` теперь является реальной usable web-vitrina page:
  ...
  - `apps/seller_portal_feedbacks_complaint_submit.py` is the bounded instrumented controlled submit runner. Real submit is CLI-only and requires `--dry-run 0` plus `--i-understand-this-submits-complaints`; `--max-submit` is hard-capped at 1. The runner always hard-denylists the historical uncertain `feedback_id=GPe9vrq0kctlSfobrgq2` and the already-successful empty-description investigation id `feedback_id=fdQpHhNXTosEkArTHAZF`, loads canonical feedback rows, runs saved-prompt transient AI analysis, selects only fresh `yes/review` candidates, skips existing runtime complaint records by `feedback_id`, requires exact Seller Portal cursor/UI match, reuses the same filter-aware actionable-row resolver before opening a complaint modal, blocks `reason_contradicts_review_tags` when AI says the review has no text while API/UI tags exist, opens the scoped complaint modal, selects the AI WB category label or `Другое`, waits for the `Опишите ситуацию` textbox after category selection, fills AI `reason` without legacy prefixes, blurs the field and blocks final submit unless the re-read field value matches the intended text. Around the single possible final click it records pre-click modal state, selected category/text, API/UI tag diagnostics, submit button label/enabled state, validation hints, sanitized network response/request-payload description summaries, toast/snackbar/modal messages, post-click modal state and row/menu state. The journal status is evidence-based: `confirmed_success` becomes `Ждёт ответа`; `confirmed_validation_error`, `confirmed_network_error` and `unconfirmed_after_click` become `Ошибка`. A click alone is not success, and the public UI still exposes no submit route/control.
...
- `apps/seller_portal_feedbacks_target_row_probe_smoke.py`
  - confirms the read-only target probe count comparison, exact DOM matching by direct `feedback_id` or fallback text/date/rating/article fields, row-menu parser detection of `Пожаловаться на отзыв`, complaint modal category parsing, JSON/Markdown report shape and no-submit/no-journal-write guards.
- `apps/seller_portal_feedbacks_complaint_dry_run_plan_smoke.py`
  - confirms dry-run candidate selection prefers `complaint_fit=yes` before `review`, skips `no`, respects the historical hard denylist before modal planning, exact-only guard blocks `high/ambiguous/not_found` from modal drafting, unique DOM row targeting can use an exact Seller Portal cursor `feedback_id` plus exact datetime/article/text support when DOM rating is absent, answered feedbacks prefer `Есть ответ`, unanswered feedbacks prefer `Ждут ответа`, fallback tries both subtabs, forced `Другое` category selection stays explicit, `force_category_other=0` can choose an available AI WB category label and falls back to `Другое` otherwise, draft text uses AI `reason` as the ready WB description without legacy prefix duplication, the `Опишите ситуацию` field is detected/focused/filled/blurred and `value_match=true` is required, aggregate/report shape records `submit_clicked_count=0`, and no-submit guards keep final complaint submission disabled.
...
- `apps/seller_portal_feedbacks_complaint_submit_smoke.py`
  - confirms submit candidate selection (`yes` before `review`, no `complaint_fit=no`), exact-only guard, hard denylist skip for `GPe9vrq0kctlSfobrgq2`, duplicate skip, ready complaint reason validation, explicit real-submit flag requirement, hard max-submit cap 1, click-alone-is-not-success classification, validation/network/success classification, network sanitizer and journal record shape.
- `apps/seller_portal_feedbacks_complaints_status_sync_smoke.py`
