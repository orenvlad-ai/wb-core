...
- `GET /sheet-vitrina-v1/vitrina` теперь является реальной usable web-vitrina page:
  ...
  - `apps/seller_portal_feedbacks_complaints_status_sync.py` is a read-only Seller Portal sync runner over `Мои жалобы`. It reads `Ждут ответа` and `Есть ответ`, maps statuses to `Ждёт ответа` / `Удовлетворена` / `Отклонена` / `Ошибка`, rejects weak matches, and updates only the operational complaint journal, not accepted truth/ЕБД/Google Sheets/GAS.
