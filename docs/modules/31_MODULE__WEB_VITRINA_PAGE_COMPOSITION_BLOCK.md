...
- `GET /sheet-vitrina-v1/vitrina` теперь является реальной usable web-vitrina page:
  ...
  - `apps/seller_portal_feedbacks_complaint_submit.py` is the bounded instrumented controlled submit runner. Real submit is CLI-only and requires `--dry-run 0` plus `--i-understand-this-submits-complaints`; `--max-submit` is hard-capped at 1. The runner always hard-denylists the historical uncertain `feedback_id=GPe9vrq0kctlSfobrgq2`, loads canonical feedback rows, runs saved-prompt transient AI analysis, selects only fresh `yes/review` candidates, skips existing runtime complaint records by `feedback_id`, requires exact Seller Portal cursor/UI match, opens the scoped complaint modal, selects the AI WB category label or `Другое`, and fills AI `reason` without legacy prefixes. Around the single possible final click it records pre-click modal state, selected category/text, submit button label/enabled state, validation hints, sanitized network response summaries, toast/snackbar/modal messages, post-click modal state and row/menu state. The journal status is evidence-based: `confirmed_success` becomes `Ждёт ответа`; `confirmed_validation_error`, `confirmed_network_error` and `unconfirmed_after_click` become `Ошибка`. A click alone is not success, and the public UI still exposes no submit route/control.
