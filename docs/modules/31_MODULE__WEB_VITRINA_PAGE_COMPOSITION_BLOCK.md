...
- `GET /sheet-vitrina-v1/vitrina` теперь является реальной usable web-vitrina page:
  ...
  - `Автообновления` is a compact web-vitrina schedule panel above the table. It reads the repo-owned hosted runtime schedule from the deployed systemd timer contract and shows the current `11:00` / `20:00` `Asia/Yekaterinburg` rows for the canonical full-refresh action. The browser can stage add/change/delete/disable rows and submit them to the schedule endpoint, but changing the live production cadence is deliberately blocked by the backend unless an operator-approved hosted deploy updates the timer; unchanged saves are accepted as no-ops. Automatic refreshes call the same canonical `POST /v1/sheet-vitrina-v1/refresh` contour as the manual button and are marked with `auto_refresh=true` / automatic trigger metadata in status/log payloads.
...
- The web-vitrina schedule panel is read-through over the current repo-owned hosted runtime timer model. Current production cadence is `11:00` and `20:00` in `Asia/Yekaterinburg`, implemented by the UTC systemd timer entries documented in the hosted runtime contract. Schedule rows are not hardcoded as browser truth: they are returned by the server context/API, and attempted cadence changes return a controlled blocker unless the same change is made through the deploy-owned timer path. Manual and automatic refresh paths stay unified; only the trigger metadata differs.
...
- Historical period UX is intentionally thin:
  ...
  - first open / hard refresh without explicit query uses the latest three server-readable business dates inclusive, ending on backend-owned `today_current_date` when that date is present in the current visible/readable context
...
- Table control UX is intentionally thin:
  ...
  - `Сбросить` removes `as_of_date/date_from/date_to` and returns to the same latest-three-dates default
