...
scope: "Real page composition для `GET /sheet-vitrina-v1/vitrina`: separate sibling page shell, compact page/status strip, runtime-managed `Автообновления`, default week ending backend-owned today, server-driven full-width table `Загрузка данных`, secondary block `Лог`, semantic compact inline latest-load-window status for `today_current`/`yesterday_closed`, bounded `Обновить` vs `Загрузить и обновить` action semantics, compact table toolbar for period/search/section/group/columns/reset with internal stable ordering, browser-local dark compact two-table metrics presentation accordion for scope-level `Итого`/`SKU` metric ordering, display statuses `Показано`/`Свернуто`/`Скрыто`, drag-and-drop row ordering, icon-only disclosure under the nearest previous shown metric, вкладка `Отзывы` с manual read-only WB API feedbacks load/filter table, transient AI-assisted разбора отзывов через server-side prompt/OpenAI route, runtime-журналом `Жалобы` и runtime schedule UI `Авто-жалобы`, вкладка `Исследования` с read-only SKU group comparison MVP, promo candidate chips, compact research date-range controls, scrollable table/grid result, table container, truthful loading/empty/error states и minimal inline client island поверх stable server seams `web_vitrina_contract -> web_vitrina_view_model -> web_vitrina_gravity_table_adapter` без SPA/platform redesign."
source_basis:
...
- `GET /sheet-vitrina-v1/vitrina` теперь является реальной usable web-vitrina page:
  - canonical entrypoint for the unified `sheet_vitrina_v1` UI; first visible tab is `Витрина`, alongside `Поставки`, `Отчёты`, `Отзывы` and `Исследования`
  ...
  - compact table header inside `Витрина`: visible source badge `sheet_vitrina_v1`, explanatory ready-snapshot copy, grid-library/row/column diagnostics and `Снимок/Вчера/Сегодня/TZ` technical line are not rendered. The header shows `Таблица`, semantic freshness badge (`актуально` when fresh), compact `Обновлено` and `Свежесть данных` timestamps, with `Загрузить` / `Загрузить и обновить` in the top-right corner.
  - status next to the load action is a compact inline pill immediately left of `Загрузить` / `Загрузить и обновить`, visually attached to data loading and worded only as `Последняя загрузка: успешно`, `Последняя загрузка: ошибка`, `Последняя загрузка: предупреждение` or `Последняя загрузка: нет данных`. Its semantics remain the last load window only: `today_current` + `yesterday_closed` in `Asia/Yekaterinburg`; the UI must not expose those technical slot names in the header. Old snapshot errors elsewhere in the selected historical table range must not force visible `Ошибка`; errors/warnings inside that latest two-day window still surface truthfully.
  ...
  - compact summary strip with browser-owned `Обновлено` and server freshness `Свежесть данных`; semantic load status is rendered inline left of the load button, not under the button and not as a left-side period/table aggregate. The former bulky `Период`, `Строки`, grid-library and source-diagnostic cards are not separate operator blocks because period is controlled by the toolbar and row counts remain table/internal diagnostics
  ...
  - `Автообновления` is a compact runtime-managed web-vitrina schedule panel above the table. It reads/writes `GET/POST /v1/sheet-vitrina-v1/web-vitrina/auto-schedules`, where business schedule rows live in server runtime JSON, not browser localStorage and not hardcoded systemd times. Rows expose stable id, enabled flag, editable `HH:mm`, timezone `Asia/Yekaterinburg`, next run, last run, last success, last status/error and run-now action. Save returns canonical persisted rows; the browser replaces local draft state with that response. `POST /v1/sheet-vitrina-v1/web-vitrina/auto-schedules/run-now` starts the same canonical full refresh path as `Загрузить и обновить`, but marks trigger source as auto-schedule run-now. The systemd timer is only a repo-owned due-check every 10 minutes; it no longer hardcodes 11:00/20:00 business times and no longer calls the auth-protected refresh route without a session cookie.
  - two server-driven action-adjacent information blocks:
    ...
    - `Seller Portal / бот` group additionally renders bounded session status and recovery run state on the left side of the group header and session controls (`Проверить сессию`, `Восстановить сессию`, `Скачать лаунчер`) over the existing seller-session/recovery seams; `Восстановить сессию` starts/replaces a run, treats `run_status=starting` as normal "окно готовится", polls status until `awaiting_login`/final state, and enables launcher download only when `launcher_ready`/`can_download_launcher=true`. A controlled launcher `409` while the run is still not ready is rendered as a warning/retry state, not as fatal "Не удалось скачать launcher". The hosted deploy contract owns the required EU runtime dependencies for that contour, including `/opt/wb-web-bot/venv/bin/python`, pinned Playwright/psycopg2 in that venv, localhost-only noVNC/Xvfb tooling and Chromium launchability. The same group-refresh path reads materialized bot-backed source truth from the localhost-only owner API on `127.0.0.1:8000` (`wb-ai-api.service`) after `/opt/wb-web-bot/bot` capture and `/opt/wb-ai/run_web_source_handoff.py` handoff, not from a public nginx route.
...
- Existing `GET /v1/sheet-vitrina-v1/web-vitrina` keeps the default public contract unchanged:
  ...
  - first/no-query page load defaults to `D-6..D` inclusive, where `D = today_current_date` from the backend in `Asia/Yekaterinburg`. `today_current_date` is selectable/visible even before a ready snapshot exists; source/status cells must truthfully show blank/partial/not-ready for missing materialization.
...
- Browser-side state now exists only as page-local UI state for filter/search/internal ordering plus namespaced browser-owned preferences for metric presentation (scope-local `Итого`/`SKU` order, per-metric display status `Показано`/`Свернуто`/`Скрыто`, expanded disclosure state; legacy toolbar metric selection, grouped shown/hidden, block-order and anchor fields are ignored) and optional column visibility.
- Historical access state stays query-string-owned on the same route:
  - no query = default week ending backend-owned `today_current_date`
...
- Server persistence mode stays `none`; no server-side user profile/preference path is introduced. The only persisted browser-owned keys in this surface are namespaced under `wb-core:sheet-vitrina-v1:web-vitrina:page-state:v1` and may be dropped without changing backend truth.
...
- `apps/sheet_vitrina_v1_seller_portal_recovery_ui_smoke.py`
  - confirms the unified web-vitrina `Seller Portal / бот` controls keep `run_status=starting` as a normal "окно готовится" UI state, do not auto-download launcher after `Восстановить сессию`, enable launcher only after `awaiting_login`/readiness, handle controlled launcher `409` as non-fatal warning and keep repeated restore clicks on `replace=true`
- `apps/sheet_vitrina_v1_seller_portal_recovery_live_smoke.py`
...
- Freshness for automatic refresh is surfaced by `Автообновления` (`last run`, `last success`, `next run`, status/error). The table header is intentionally compact: browser-owned `Обновлено`, server freshness `Свежесть данных`, and load-adjacent latest-window status under the top-right load action; raw snapshot ids/as_of/debug remain in JSON/log surfaces.
- Embedded `Поставки` / `Отчёты` remain UI-only composition inside `/sheet-vitrina-v1/vitrina`; iframe sizing/scroll relay changes do not introduce browser-side business truth or server-side profile persistence.
...
- source-aware temporal policy is now part of that read-side truth:
  ...
  - `prices_snapshot`, `ads_bids` and `spp` preserve accepted-current rollover/latest-confirmed filled values, but closed-day preservation is surfaced as warning/latest-confirmed instead of false fresh-green closed-day refetch; missing required current without accepted fallback remains not OK;
...
- The web-vitrina schedule panel is runtime-managed, not a frozen view of systemd. Current default rows are `11:00` and `20:00` in `Asia/Yekaterinburg`, but add/edit/delete/disable are persisted in server runtime JSON through the auto-schedules API. The repo-owned systemd timer runs an authenticated due-check every 10 minutes; manual and automatic refresh paths stay unified and differ only by trigger metadata.
...
- Historical period UX is intentionally thin:
  ...
  - first open / hard refresh without explicit query uses the backend-owned current business week `D-6..D`, ending on `today_current_date` in `Asia/Yekaterinburg`
...
- Table control UX is intentionally thin:
  ...
  - search, section/group filters, column visibility and reset live in the compact toolbar next to `Диапазон`; the former `Тип строк` toolbar control and toolbar `Метрики` selector are not rendered, and obsolete stored state is ignored safely; internal stable row ordering remains browser-local without a visible sort selector
  - the collapsible `Метрики` presentation block is browser-local state over the already received metric rows: it renders two compact scope tables (`Итого`, `SKU`) with row drag-and-drop across the whole scope list, informational `Группа` metadata, and a display selector `Показано` / `Свернуто` / `Скрыто`. The table applies that scope order plus display status without changing metric registry, formulas, ready snapshots or accepted truth; collapsed metrics attach to the nearest previous shown metric and reveal through an icon-only disclosure arrow in the `Метрика` cell, while hidden metrics stay available in settings but are absent from the main table.
  ...
  - `Сбросить` removes `as_of_date/date_from/date_to` and returns to the same current-week default
