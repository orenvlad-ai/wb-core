...
- `GET /sheet-vitrina-v1/vitrina` теперь является реальной usable web-vitrina page:
  ...
  - compact summary strip with browser-owned `Обновлено`, semantic `Статус` and selected `Период`; the former bulky `Свежесть данных` and `Строки` cards are not separate operator blocks because auto-run freshness now belongs to `Автообновления` and row counts remain table/internal diagnostics
  ...
  - table controls are one compact toolbar above the table, not a separate `Фильтры и настройки` section: `Диапазон`, `Поиск`, `Секции`, `Группа`, `Тип строк`, `Метрики`, `Столбцы`, `Сброс` share the same desktop line and reuse the existing local filter/search/internal stable order/column-visibility state; the visible `Сортировка` selector and row-count `Итог` block are not rendered. `Метрики` is a compact checkbox multi-select with a top `Все метрики` indeterminate/all toggle, count suffixes from the filter surface, namespaced browser-local restore, default all-visible behavior, and safe fallback to all metrics for corrupted or obsolete stored keys.
...
- Freshness for automatic refresh is now surfaced by `Автообновления` (`last run`, `last success`, `next run`, status/error). The top summary strip is intentionally compact: browser-owned `Обновлено`, semantic `Статус` and selected `Период`; raw snapshot ids/as_of/debug remain in JSON/log surfaces.
...
- Table control UX is intentionally thin:
  ...
  - search, section/group/scope/metric filters, column visibility and reset live in the compact toolbar next to `Диапазон`; internal stable row ordering remains browser-local without a visible sort selector
