...
локального claim не завершает всю группу: worker читает тот же operation ID и
повторяет свежий preview с ограниченной паузой только если exact run всё ещё
`queued`, отсутствуют Stage E binding и write operation. Пауза и число попыток
сохраняются в журнале, поэтому перезапуск worker не сбрасывает ход группы.
После исчерпания попыток группа ожидает явной повторной сверки той же операции;
старые завершённые группы автоматически не открываются. Если право отправки
уже могло быть получено, разрешён лишь readback, без нового WB submit.
