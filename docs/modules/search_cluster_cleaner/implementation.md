...
локального claim не завершает всю группу. Для ещё не привязанного `queued` run
...
Если полный scan обнаружил, что ранее исключённая фраза исчезла из свежего
списка WB, для этой пары сохраняются `external_state_drift` и target hold.
Только доказанный scan без write operation и с точным событием drift позволяет
перейти к следующей паре; фраза автоматически не исключается повторно и hold
не снимается. Другие partial/failed результаты по-прежнему останавливают batch.
