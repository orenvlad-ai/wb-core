...
Collector can produce a metadata-only `period_id=pending` record first and later a complete workbook-backed record for the same `promo_id`, title, source tab/filter and exact `promo_start_at..promo_end_at`. Replay treats that pending metadata-only duplicate as superseded only when the sibling complete artifact exists; the complete workbook materializes values, while diagnostics keep `superseded_metadata_only_artifacts_ignored` evidence. Standalone active metadata-only artifacts remain fatal and cannot be turned into zeros.
...
- `apps/promo_campaign_archive_gc.py apply --confirm --fingerprint <exact-dry-run> --deployed-sha <40-hex>` deletes only guarded candidates from the structured plan. Full apply binds every candidate SHA/stat/inode/mtime, writes a durable crash-resume audit, fsyncs each unlink and rechecks the candidate-run non-target digest. Hosted production uses `promo-archive-gc-dry-run|apply`; it never removes archive records, metadata, normalized rows/manifests, exact-date runtime snapshots or unknown/incomplete parse artifacts.
