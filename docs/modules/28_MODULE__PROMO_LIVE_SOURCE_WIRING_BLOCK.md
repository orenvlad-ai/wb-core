...
scope: "Repo-owned wiring `promo_xlsx_collector_block` обратно в current `sheet_vitrina_v1` refresh/runtime/read-side contour: source contract, accepted snapshot semantics, STATUS/read-side exposure и bounded live integration smoke."
source_basis:
...
update_note: "Создан как канонический модульный документ для bounded live-wired promo source checkpoint: `promo_xlsx_collector_block` больше не local-only precursor, а materialize-ит truthful `promo_by_price` read-side rows внутри existing refresh/runtime contour без invent-едённых exact dates и без stale-value fallback."
---
...
- `promo_by_price[today_current]`:
  ...
  - считает numeric truth только по promo, которые truthfully classified as `current`;
  - future promo не попадают в current numeric fill;
  - workbook rows и promo-level sidecar metadata сводятся без подмены source-of-truth semantics.
- `promo_by_price[yesterday_closed]`:
  - не пересобирается live из текущего seller portal state;
  - читает только accepted/runtime-cached promo truth из уже сохранённого server-owned snapshot seam;
...
- `promo_by_price` теперь относится к `dual_day_capable` группе read-side, но с asymmetric capture semantics:
  - `today_current` = live collector attempt for the requested business day
  - `yesterday_closed` = accepted/runtime-cached prior truth only
- exact promo dates materialize-ятся только при reliable parse.
...
- Numeric mapping живёт server-side и не переносится в Apps Script:
  ...
  - `promo_count_by_price` = count of active current promo rows whose effective discounted price is at or below plan price within bounded epsilon
