...
- Operator-only `POST /v1/sheet-vitrina-v1/supply/supplier-shipments/{shipment_id}/price-check` manually rechecks all lines against current nomenclature, updates persisted price fields, writes `check_mode=manual_recheck` and stores the current web actor/context when available. Supplier/factory role can read saved statuses but cannot call the route and the supplier UI does not render the manual recheck button.
