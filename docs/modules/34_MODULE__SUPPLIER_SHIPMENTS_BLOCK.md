...
- `bank_transfer_application` normalized parse stores transfer application number/date, execution status/time when present, debit account as available, currency, transfer amount, ordering customer, beneficiary/customer, beneficiary account, beneficiary bank, payment details including contract reference, and charges mode such as `OUR`. Missing fields add parser warnings and set review status instead of failing storage.
