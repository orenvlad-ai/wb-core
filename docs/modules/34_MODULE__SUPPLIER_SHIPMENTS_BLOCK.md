...
current_update_note: "`Настройки` is now a common-shell tab with `Справочники` and admin-only `Пользователи`; supplier-only access remains standalone and cannot reach settings/users, while `supply_operator` is limited to the `Поставки` shell section."
---
...
- Operator/admin settings surface: `Настройки` is now a top-level tab in the common WebCore shell. Direct `GET /sheet-vitrina-v1/settings` remains backward-compatible by rendering the same shell with active `Настройки`; the embedded body uses two groups, `Справочники` and `Пользователи`. `Справочники` contains `Номенклатура`, `Договоры` and `Инвойсы` without reload; `Номенклатура` is the default dictionary subsection and any browser-local tab state is UI preference only, not runtime truth. `Пользователи` is admin-only and is not part of supplier shipment truth.
...
- `bank_transfer_application` normalized parse stores transfer application number/date, normalized execution status/time when present, debit account as available, currency, transfer amount, amount in words, ordering customer, payer INN/country/address when present, beneficiary/customer, beneficiary account/address/country, beneficiary bank, SWIFT/BIC, bank address/country/clearing code when present, payment details, parsed contract reference/number/date, charges mode such as `OUR`, sender-to-receiver info and regulatory reporting when present. Missing critical fields add parser warnings and set review status instead of failing storage.
...
- Admin role can access the full WebCore shell, `Настройки`, `Настройки -> Пользователи`, nomenclature CRUD/export/import API, trade documents, the read-only supplier shipment registry matrix and supplier shipment APIs including delete/rematch, manual price check and order-status update.
- Operator role can access the full WebCore shell, `Настройки -> Справочники`, nomenclature CRUD/export/import API, trade documents, the read-only supplier shipment registry matrix and supplier shipment APIs including delete/rematch, manual price check and order-status update, but cannot use the users admin API/UI.
- Supply-operator role can access only the common shell with active `Поставки` and the supply routes needed by the supply section. It cannot open `Витрина`, `Отчёты`, `Отзывы`, `Исследования`, `Настройки`, settings dictionaries, users API or unrelated operator APIs.
