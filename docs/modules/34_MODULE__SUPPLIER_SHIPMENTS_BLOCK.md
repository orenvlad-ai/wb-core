...
- Row groups are: passport, cargo physics, cargo value, logistics quote, fact expenses, normalized fact metrics, lead times and documents. The lead-times group exposes quote delivery plan and `Срок до ДТ` (`declaration_date - shipment_date`) only; it does not treat customs declaration date as fulfillment acceptance and does not expose a delivery-plan deviation row. The UI renders these groups with horizontal scroll and sticky left row labels.
