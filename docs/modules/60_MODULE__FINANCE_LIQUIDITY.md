...
Справочники v3 создают три RUB кассы и 23 статьи без остатков и операций;
контрагенты начинаются пустыми. Названия редактируются, архив и восстановление
сохраняют ссылки, а удаление возможно лишь без ссылок, включая черновики.
«Перезаказ» — замена клиенту стекла с компенсацией менеджеру; это отдельная
статья от маркетинговых «Раздач».
Пополнение собственными деньгами имеет отдельный `owner_funding` эффект и не
является выручкой; перевод остаётся одним атомарным фактом без статьи.
Классификация статьи и названия при проведении сохраняются снимком в документе:
переименование не меняет старые деньги или аналитику. Платёж по кредиту
помечается `debt_service_unallocated` для будущего разнесения тела и процентов;
P&L здесь не рассчитывается. Существующий v2 TEST store требует отдельной
явной миграции по runbook; чтение и запуск не меняют схему.
При переходе v2 известное на дату миграции имя статьи сохраняется отдельно от
неизменяемого проведённого документа; это не восстановление прежнего имени.
Старые расходные статьи получают нейтральный `legacy_expense_unclassified`,
а старые приходные — `external_inflow_unclassified`, без догадки о P&L.
Форма общего старта принимает одну явно выбранную дату и три явно указанные
суммы, атомарно создаёт три черновика и не проводит деньги сама. Неизвестный
остаток не подставляется нулём. Каждый черновик затем проводится существующим
механизмом после проверки владельцем.
