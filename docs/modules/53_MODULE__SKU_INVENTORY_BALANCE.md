...
doc_type: "module"
status: "active"
purpose: "Зафиксировать server-owned подраздел `Управление SKU → Баланс запасов`: immutable расчёты темпа и рекламных целей, раздельные ручные overrides, расширенный XLSX и durable подтверждаемое применение ставок и состояний кампаний."
scope: "Одна строка на active nmID поверх canonical SKU Management evidence; conservative stock pacing, old CPM/new CPC campaign groups, exact target recommendations and reload-safe live WB bid/campaign-state application protocol."
source_basis:
...
update_note: "Compact inline cells объединяют bid и typed campaign-state action; одна durable Balance apply queue подтверждает оба типа exact readback, а terminal calculation render защищён от stale secondary responses."
---
...
Inline CPC/CPM cells показывают для каждого exact campaign target не более примерно двух видимых строк: compact `Кампания` disclosure, текущую ставку, typed numeric target input с единицей и отдельный typed state-action control. CPC подписан `₽/клик`, CPM — `₽/1000 показов`. Полное campaign name, exact `advert_id`, current status/state и warnings доступны по click/hover в disclosure/tooltip; guessed portal deep-link отсутствует. Доступная auto recommendation prefill-ит bid target; при `insufficient_stats`, `insufficient_inventory_evidence`, unknown или отсутствующем calculated target input остаётся пустым и позволяет явный ручной ввод только при сохранённой actionable identity/evidence. Несколько targets одного типа не смешиваются: каждый имеет отдельные bid/state controls в той же SKU row, без subordinate row.
...
Editable bid control находится только inline в CPC/CPM cell и подписан единицами. Значение durable сохраняется по debounced input и немедленно по change/Enter; корректность не зависит от неочевидного blur и отдельной кнопки Save. Если первый click выбора пришёл, пока row disabled только из-за pending override save, intent удерживается: после successful response row сразу становится enabled+checked и selected count обновляется без второго click. UI различает `сохраняется`, `готово к выбору`, controlled save error и точную business-причину блокировки; stale response не меняет более новый draft/calculation. При доступной auto target control показывает effective final target; возврат к calculated value очищает manual override. Blank, invalid, equal-current и unchanged target не входят в atomic apply selection.

# 5. Selection, confirmation and durable apply

Для каждой exact campaign typed choice имеет `не менять` и ровно одно разрешённое из текущего WB status действие: status `9/active` — `остановить` через reversible pause; status `4/ready` или `11/paused` — `запустить/возобновить` через start. Выбор меняет только browser pending state и никогда не вызывает WB до explicit owner confirmation. Campaign state и numeric bid остаются разными typed payload fields; action text не попадает в numeric input.

Оператор выбирает строки либо `Выбрать все доступные`; row считается actionable, если содержит хотя бы один valid bid change или valid state action. Selected count и точная причина disabled Apply остаются рядом с действием. Кнопка и modal называются `Применить изменения`. До job browser показывает selected SKU, atomic bid current→target с единицами, exact campaign state transitions, число повышений/понижений, исключённые targets и предупреждения. Modal прямо сообщает, что только после отдельного подтверждения exact изменения действительно будут отправлены в WB.
...
`apps/sku_inventory_balance_browser_smoke.py` проверяет latest auto-load/reopen freshness, initial latest `500 → terminal operation.result → visible rows`, default columns для empty arrays и stale latest/override isolation; compact two-line cells/disclosure, debounced/change/Enter input, first click во время save, save failure, multi-target identity, typed state preview/payload и `Применить изменения`; modal preview→running→full/partial/zero/stalled result, reload reconnect, human safety warning и last result; browser-side WB writer call отсутствует.
