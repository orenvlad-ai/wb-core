...
В интерфейсе «Настройки расчёта» содержит только коэффициент WB, страховой запас и
период продаж. Редактор ширин, видимости и порядка колонок удалён; ранее сохранённые
table preferences продолжают применяться. Пресет находится рядом с поиском как
фильтр таблицы. Сортировка по заголовкам сохраняется.

CPC и CPM имеют по две отдельные колонки: `ставка` (default `200px`, диапазон `190..200px`) и `статус` (default `120px`). Старые presentation preferences получают status сразу после соответствующей rate column; явно сохранённые новые порядок/видимость сохраняются. Это меняет только layout preferences, не business data.

Клик по заголовку содержательной колонки сортирует целые SKU rows и показывает стрелку направления; `Выбор` не сортируется. Rate columns сравнивают текущие ставки, первый клик — убывание, следующий — возрастание. Status columns: active перед paused/ready, затем обратный порядок. Numeric/date/text columns используют соответствующие значения, отсутствующие данные остаются последними в обе стороны. Для нескольких кампаний значения сравниваются последовательно в их отображаемом порядке (для state — уникальные campaign controls), без max/any/суммы; полное равенство сохраняет исходный порядок SKU. Сортировка не переносит отдельные cells между SKU и не меняет drafts/selection/apply identity. Sort preference сохраняется вместе с настройками таблицы.
...
Inline CPC/CPM cells показывают для каждого exact campaign target одну плотную логичную группу: полноценный numeric target input с видимым caret и многозначным значением, фактическую current bid с единицей под input и последнюю маленькую accessible info-кнопку. Повторной единицы справа от input нет; status/action control и pending badge находятся в отдельной соседней колонке. Постоянного текста/кнопки `Кампания` нет. CPC подписан `₽/клик`, CPM — `₽/1000 показов`. Info click открывает compact popover с полным campaign name, exact `advert_id`, placement, фактическим current state, warnings и identity detail; guessed portal deep-link отсутствует. Поле новой ставки изначально пустое. Ручной ввод доступен при точной campaign identity и известной текущей ставке независимо от полноты прогноза запасов. Старые расчётные рекомендации не подставляются даже при открытии исторического расчёта. Несколько targets одного типа не смешиваются: каждый имеет отдельные bid/state controls в той же SKU row, без subordinate row.
...
Новая ставка находится в первой строке, текущая — под ней обычным читаемым шрифтом и с единицей. Ввод не заменяет DOM input и сохраняет focus/caret, включая паузы между символами и редактирование середины. Commit происходит после blur/перехода к другому полю либо Enter. Save предыдущего поля не заменяет активное следующее поле или sibling target из устаревшего ответа. Только успешный durable save автоматически выбирает SKU; незавершённый/ошибочный ввод блокирует preview, поэтому Apply не использует прежнее значение. Blank, invalid и equal-current не входят в apply selection.

Current ads read projection накладывает последние подтверждённые exact readback значения из durable apply items поверх ответа расчёта, не изменяя immutable payload. Она хранит observation timestamp/source/job и отдельные исходные calculation current fields; новые ручные решения берут before из этой projection. Наблюдение stale-state preflight показывается как наблюдение без отправки, не как применённый факт. Старый job не заменяет более новые observation/расчёт. Применённый manual override скрывается в editable projection только при совпадении значения и timestamp не позже создания его job; override audit остаётся, более поздние drafts сохраняются. Это last confirmed/observed значение с временем, не обещание live polling WB.


# 5. Selection, confirmation and durable apply

Для каждой exact campaign в отдельной status колонке показывается фактический status: зелёный play для `9/active`, красный pause для `4/ready` или `11/paused`; unknown/unsupported state остаётся neutral non-actionable с точным raw-status/identity reason. Click по action-capable status открывает compact menu только с одним уместным действием: `остановить` через reversible pause для active либо `запустить/возобновить` через start для ready/paused. Отсутствие pending action и есть `не менять`, поэтому постоянного select нет. Выбор меняет только browser pending state, добавляет короткий badge `Ост.`/`Возоб.` с cancel и никогда не вызывает WB до explicit owner confirmation. Фактическая иконка не меняется на optimistic state до confirmed apply. Campaign state и numeric bid остаются разными typed payload fields; action text не попадает в numeric input.
...
Preview modal после подтверждения не исчезает: та же modal переходит `preview → running → terminal result`. UI показывает real animated progress, живые числа `применено / проверяется / ожидает / не применено / требует проверки`, per-target current→target/status и очевидный полный/частичный/нулевой итог. Human-readable ошибка находится в строке, technical identity/detail — только под optional disclosure. Reopen страницы восстанавливает active/latest durable job и polling без автоматического открытия modal; остаётся persistent page indicator/reopen action. Результат explicit текущего apply продолжает показываться в той же modal. Последний apply result также виден compact badge у соответствующей SKU row.
