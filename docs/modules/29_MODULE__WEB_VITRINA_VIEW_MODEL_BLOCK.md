...
Итоговая `weighted_price_seller_discounted` («Цена продавца взвеш.») имеет
датированную формулу. Для дат показателя до `2026-09-14` сохраняется
`SUM(price_seller_discounted * orderCount) / SUM(orderCount)`. Начиная с
`2026-09-14` применяется `SUM(orderSum) / SUM(orderCount)` — средняя сумма
оформленного заказа на единицу, по одному принятому дневному источнику и одному
составу SKU. Обновление новой формулы относится к `sales_funnel_history`.

Полностью отсутствующие в дневном источнике товары не обнуляются: результат
показывает наблюдаемые заказы с явной неполнотой состава. Неполная или
некорректная пара суммы/количества у присутствующего товара блокирует результат;
при отсутствии заказов ячейка пуста. Датированный состав сохраняется после
закрытия дня, если состав нового расчёта совпадает с подтверждённым.

Обычные полное и групповое обновления сохраняют опубликованные исторические
ячейки до этой границы, включая пропуски и представление полноты. При смене дня
или bundle прежняя ячейка находится по дате колонки в сохранённых снимках.
API и интерфейс читают опубликованное значение; отдельный экспорт этой таблицы
сейчас не включён (`exportable=False`). Формулы/дата перехода фиксируются в
metadata; цены SKU, маржа, SPP и остальные показатели не используют новый итог.
