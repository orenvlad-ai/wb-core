...
Для ordinary WB supply `in_transit = sent - accepted`. Status `4` переносит только cumulative accepted quantity; каждое увеличение создаёт только delta-event, а повтор того же значения даёт zero delta. Переходы `3 → 4 → 5` используют один FF debit и один cost snapshot, поэтому принятая часть не остаётся в транзите и не учитывается дважды. Регресс accepted quantity fail closed. Финальная положительная разница создаёт outstanding row с original supply, SKU/nmID, warehouse/destination, original cost layer и event dates. `Допринято` не списывает ФФ повторно: direct upstream identity имеет приоритет, иначе применяется строгий `warehouse + destination + SKU` FIFO. Surplus, ambiguous identity и нарушение инварианта блокируются без fabricated allocation.
