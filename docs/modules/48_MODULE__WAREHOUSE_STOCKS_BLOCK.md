...
Weekly Finance читает WB unit cost только через общий resolver канонических строк `sheet_vitrina_v1_warehouse_wb_daily_cost`. Для operation date до `2026-07-01` resolver проецирует назад exact same-`nmId` row на 01.07; на/после границы требует exact operation-date row. Отсутствие required row является unknown и не может наследовать current/later/last-good, другой SKU, average, legacy cost или zero.
