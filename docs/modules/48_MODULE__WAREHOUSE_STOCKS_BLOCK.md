...
The policy creates an availability projection only. It excludes exact physical `quantity` from operational total/regions once upstream and requires complete pagination plus snapshot digest; it never removes unattributed `inWayToClient`/`inWayFromClient`. It does not change this section's factual WB contour, WAC, stage quantity/capital, `Всего единиц`, total capital, weighted cost or reconciliation. A product remains capital until a separate audited writeoff/compensation operation. The incident control therefore cannot create a loss, compensation or hidden capital recalculation.
