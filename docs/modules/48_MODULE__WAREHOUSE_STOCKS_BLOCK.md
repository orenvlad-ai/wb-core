...
Запросы текущего склада WB, официального FBS и периодического источника `stocks`
используют общий `stock_catalog_scope`: все основные активные SKU номенклатуры
и все скрытые SKU, включая неактивные скрытые. Список заново читается при
обновлении; `config_v2.enabled`, фотографии, продажи и положительный остаток
не ограничивают сбор. Состав отображаемых строк «Управления SKU» остаётся
отдельным от состава собираемых складских данных.

Скрытые позиции сохраняются в запросах консервативно: отсутствие строк остатков
или заказов не доказывает завершение учёта. Автоматическое исключение скрытых
SKU, активация номенклатуры и правила допустимости складских записей этим
источником не управляются. История и документы не удаляются. Отсутствующий или
неоднозначный внешний идентификатор делает охват неполным, а не нулевым.
Изменение каталога во время запроса текущего склада WB/FBS отклоняет нестабильный снимок.
...
Для текущей информационной оценки WB+FBS отсутствие materialized WB-строки
допускает нулевой WB-компонент только при наличии полного плотного снимка той
же опубликованной good-версии и даты. Проверяются охват SKU, исходные строки,
контрольные суммы, пагинация и все три количества: склад, к покупателю, возврат.
При подтверждённом WB=0 положительный FBS-компонент оценивается по своей
имеющейся базе стоимости; нулевому компоненту цена не выдумывается. Неизвестный
WB-компонент сохраняет общую оценку неполной. Это чтение не меняет WAC,
финансовые отчёты или физические стадии склада.
