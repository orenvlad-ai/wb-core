...
- The one-off September 2026 recovery is separately bounded to `2026-09-03..2026-09-17`. It writes only `temporal_source_snapshots[source_key=sales_funnel_history_buyout_confirmation_v1]`, containing the official `buyoutPercent` and `orderCount` pairs for the current confirmation roster. Buyout readers select this overlay only after full D-6 proof for that same roster; an absent or partial overlay falls back to the original and cannot certify an observed subset. The original `sales_funnel_history` rows, their `captured_at`, and all other metrics remain unchanged. Preview is read-only and writes a reviewed manifest outside Git. Apply requires its checksum, deployment/approval binding, unchanged schema, roster, overlay pre-state, original-source digest and frozen Proxy V4 parameter digest; exact pre-overlay rows are retained in that manifest for governed rollback. V4 preserves the legacy byte-compatible source digest wherever the original remains selected and uses a distinct digest only for an actually selected overlay date.
...
- guarded September buyout confirmation overlay: `apps/sheet_vitrina_v1_buyout_confirmation_recovery.py`
...
- September overlay selection, legacy digest compatibility, full-week recovery, source retention, dry-run/apply/idempotency and schema/roster/tamper CAS are checked by `apps/sheet_vitrina_v1_buyout_percent_smoke.py` and `apps/sheet_vitrina_v1_buyout_confirmation_recovery_smoke.py`.
