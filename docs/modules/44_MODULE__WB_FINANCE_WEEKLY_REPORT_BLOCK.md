...
Proxy V4 consumes the existing immutable `wb_finance_weekly_aggregates` and `wb_finance_weekly_sync` read model; it does not fetch or reclassify Finance rows and does not add a second scheduler. For each of the latest three fully closed Monday-Sunday slots, Finance READY requires complete seller coverage, `completed` sync provenance, every required metric and current `wb_finance_weekly_classifier_v3_signed_review_points`. An ordinary candidate finds the exact intersection of READY COMPLETE Finance weeks and mature READY Buyout weeks inside those same slots, then selects exactly the freshest one common range. If the newest slot is unavailable in either source, the freshest earlier common READY range is selected; unrelated periods are never mixed. Historical as-of initialization remains unchanged and stricter: both initial versions retain their reviewed exact 3-of-3 common proof, and each Finance week's `first_loaded_at` business date must be no later than the candidate effective date.

Every ordinary automatic rate is the direct single-selected-week `SUM(signed amount) / SUM(net_revenue)`, never an arithmetic mean or multi-week combined rate. The Settings reference may still publish a separate direct combined value across one to three READY slots, but that analytical value is not a V4 formula input. Missing/partial weeks contribute neither amount nor denominator; proven signed zero remains zero. Composition is exact:
...
- other expense combines only `subscriptions + paid_services + review_points + other_deductions + acceptance − capitalized_acceptance`;
- both `transit_logistics` and `capitalized_transit_logistics` are excluded from V4 percentage expenses, retained share, profit and margin. They remain unchanged in Finance diagnostics and in canonical WB cost/WAC/COGS; marketing remains excluded because V4 subtracts canonical `ads_sum` separately, while capitalized acceptance, positive adjustments and `wb_remuneration_adjustment` remain excluded to prevent double count.

The exact selected range, selection-contract version, current classifier, source amounts, Buyout `orderCount` weight, Finance `net_revenue` denominator, seller coverage and source digests participate in the immutable V4 source-window fingerprint. Zero common READY weeks or a fallback to an older week leaves the last confirmed V4 version effective when that selected fingerprint is already materialized; with no prior version the formula remains fail-closed. A fresher READY common week changes the fingerprint and creates at most one version from its actual materialization business date; repetition is idempotent. The one-time legacy-combined transition may create a higher revision on the current business date, while the ordinary refresh preserves every earlier V4 date. A payload correction for an already frozen selected range requires the separate guarded historical reconciliation contract; the normal refresh never mutates prior V4 history.

Formula contract `proxy_profit_4_v2_no_transit` and latest-week selection contract `sheet_vitrina_v1_proxy_v4_latest_confirmed_week_v2_no_transit` make that exclusion explicit in every new immutable revision and source fingerprint. Existing `proxy_profit_4_v1` rows remain readable historical facts; they are never rewritten or silently relabelled. The production correction appends one higher revision effective `2026-08-16`, preserving every non-transit automatic rate and proving `legacy other expense = corrected other expense + (transit_logistics − capitalized_transit_logistics) / net_revenue` for the exact `2026-08-03..2026-08-09` source week.
...
Authenticated production acceptance uses `finance-ui-flow` in a fresh isolated Chromium context. It is calculation/read-only: it may POST preview/XLSX generation but never saves settings, finalizes a partner report or changes Finance/business data. The storage API/card must prove exactly one canonical lifecycle phase: either the implicit monolith with both logical stores on `monolith`, rollback ready and no selected cutover, or a selected split manifest whose top-level/raw/operational generation identities match, whose schema revisions are exact, whose consumer cursor is caught up, whose pending outbox, mismatches and dead letters are zero, and whose retained rollback generation is `monolith`. A shadow, mixed-generation, lagged, unhealthy or ambiguous state fails closed. The report persists that phase classification with the full storage-health readback. Acceptance also requires `preview.attempted=true`, `preview.ready=true`, empty blockers, visible table, enabled download and an actually downloaded/opened semantic XLSX that reconciles nmId, selected weeks, source/formula digest and displayed Decimal amounts. A non-empty but wrong workbook, hidden sheet, external link, missing download or incomplete preview fails the flow.
