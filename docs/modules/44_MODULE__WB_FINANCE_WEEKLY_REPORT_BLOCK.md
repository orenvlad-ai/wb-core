...
The full-scope preflight keeps only one raw Finance week in memory at a time. It never retains a cross-week movement list: an iterator re-reads cost-bearing rows one at a time to build the immutable retro map, and each week is then re-read separately for projection. Sale/return-tagged rows with zero quantity remain represented by deterministic count/digest diagnostics and the account-level no-`nmId` manifest, but they are not COGS movements and cannot create retro-cost identity blockers or `problem_skus`.
...
Production UI acceptance uses the repo-owned `finance-ui-flow` hosted command. It enters the unified Reports surface in a fresh authenticated Chromium context, verifies Finance API/table week alignment, COGS/profit/margin/no-marketing rows, Partner options and desktop/narrow layout, and rejects `5xx`, `pageerror`, console errors or any write-capable request. When complete server-owned parameters already exist, the only allowed POST is the calculation-only preview; otherwise the Flow shows why preview was not attempted and does not invent or save parameters. It never saves Partner settings, builds a package or creates a finalized report.

Targeted verification includes `apps/wb_finance_weekly_smoke.py`, `apps/wb_finance_weekly_cost_cutover_smoke.py`, `apps/wb_finance_weekly_business_approved_backfill_smoke.py`, `apps/wb_finance_weekly_browser_smoke.py`, `apps/partner_report_browser_smoke.py` and `apps/registry_upload_http_entrypoint_hosted_runtime_smoke.py`. The tracked `22.06.2026–28.06.2026` artifact is pre-change control evidence only; post-backfill production values must come from reviewed dry-run/readback rather than being invented in repository fixtures.

Partner Report evidence text, including complete `sha256:` source digests and bounded blocker diagnostics, wraps inside the report card. On narrow viewports it must not expand the document; horizontal scrolling remains local to the report table.
