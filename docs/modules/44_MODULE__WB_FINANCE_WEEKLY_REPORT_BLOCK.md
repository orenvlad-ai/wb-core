...
The full-scope preflight keeps only one raw Finance week in memory at a time and re-reads each week for projection after building the immutable retro map. Sale/return-tagged rows with zero quantity remain represented by deterministic count/digest diagnostics and the account-level no-`nmId` manifest, but they are not COGS movements and cannot create retro-cost identity blockers or `problem_skus`.
