...
| `23_MODULE__REGISTRY_UPLOAD_HTTP_ENTRYPOINT_BLOCK.md` | `registry_upload_http_entrypoint_block` | `registry` | live HTTP entrypoint, simple WebCore app-level auth, hosted public route allowlist/deploy publication, strict feedbacks load/export/AI prompt-model discovery routes, protected submit-selected job routes, feedback auto-complaints schedule/run/tick routes plus repo-owned systemd due-check timer, shared Seller Portal automation lock/busy responses, canonical EU storage-state/no-local-fallback policy, thin runtime wiring, source-aware web-source closed-day acceptance/retry, explicit seller-session probe и permanent operator-facing seller-session block (`session-check/start/status/stop/launcher`, safe stop, per-run `run_id`/final outcome, hardened noVNC launcher) подтверждены, смёржены в `main` |
