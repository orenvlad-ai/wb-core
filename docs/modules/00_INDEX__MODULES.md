...
- `web_vitrina_page_composition_block` как bounded phase-4 live page composition на `/sheet-vitrina-v1/vitrina` с existing read route, вкладкой `Отзывы` поверх read-only WB feedbacks route, transient AI prompt/analyze flow с real WB complaint category schema и `reason` как текстом для WB `Опишите ситуацию`, read-only Seller Portal complaint scout + no-submit matching replay/dry-run runners и minimal inline client island.
