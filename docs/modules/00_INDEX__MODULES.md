...
| `23_MODULE__REGISTRY_UPLOAD_HTTP_ENTRYPOINT_BLOCK.md` | `registry_upload_http_entrypoint_block` | `registry` | live HTTP entrypoint, hosted public route allowlist/deploy publication, feedbacks AI prompt/analyze routes, thin runtime wiring, source-aware web-source closed-day acceptance/retry, explicit seller-session probe и permanent operator-facing seller-session block (`session-check/start/status/stop/launcher`, safe stop, per-run `run_id`/final outcome, hardened noVNC launcher) подтверждены, смёржены в `main` |
