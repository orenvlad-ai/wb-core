...
scope: "Папка `docs/modules/`, её naming rules, статус source of truth и полный список модульных документов `01–28`."
source_basis:
...
update_note: "Обновлён под current `main`: индекс отражает комплект модулей `01–28`, где `promo_xlsx_collector_block` остаётся browser-capture precursor, а `promo_live_source_wiring_block` materialize-ит truthful `promo_by_price` live rows внутри existing refresh/runtime contour."
---
...
На текущем `main` main-confirmed модульные блоки доходят до `01–28`.
