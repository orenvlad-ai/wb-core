...
scope: "Папка `docs/modules/`, её naming rules, статус source of truth и полный список модульных документов `01–27`."
source_basis:
...
update_note: "Обновлён под current `main`: индекс отражает комплект модулей `01–27`, включая repo-owned bounded `promo_xlsx_collector_block` как local-runner bridge между thin browser adapter boundary и truthful promo sidecar contract."
---
...
На текущем `main` main-confirmed модульные блоки доходят до `01–27`.
...
- `promo_xlsx_collector_block` как bounded repo-owned local collector contour для promo XLSX + metadata sidecar, сохраняющий thin browser adapter boundary и не wire-ящийся пока в live refresh/load contour.
...
- bounded promo collector contour теперь уже repo-owned, но его output ещё не wire-ится в current refresh/load/operator line;
- незакрытым остаются full legacy parity, final live numeric coverage integration для promo-backed metrics и других bounded long-tail gaps beyond current `COST_PRICE` overlay, plus actual hosted deploy rights/publish wiring, final auth-hardening и production storage binding вокруг уже repo-owned deploy contract.
...
| `19_MODULE__SHEET_VITRINA_V1_PRESENTATION_BLOCK.md` | `sheet_vitrina_v1_presentation_block` | `sheet-side` | live formatting, включая canonical percent rows `ads_cr/avg_ads_cr`, подтверждён, смёржен в `main` |
...
| `23_MODULE__REGISTRY_UPLOAD_HTTP_ENTRYPOINT_BLOCK.md` | `registry_upload_http_entrypoint_block` | `registry` | live HTTP entrypoint, thin runtime wiring, source-aware web-source closed-day acceptance/retry и current-day sync подтверждены, смёржены в `main` |
...
| `26_MODULE__SHEET_VITRINA_V1_MVP_END_TO_END_BLOCK.md` | `sheet_vitrina_v1_mvp_end_to_end_block` | `sheet-side` | первый bounded end-to-end MVP, включая strict `yesterday_closed + today_current` truth, closure retry semantics и truthful blank-overwrite в live `DATA_VITRINA`, подтверждён и смёржен в `main` |
