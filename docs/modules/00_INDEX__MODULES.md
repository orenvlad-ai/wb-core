...
| `40_MODULE__OUR_WB_COST_MODEL_BLOCK.md` | `our_wb_cost_model` | `web/operator/supply/runtime-cost` | Management proxy WB cost contour: SKU-level FF cost layers on `actual_ff_acceptance_date`, direct-zero/transit classifier, final-status-5 accepted-date physical rolling without planned/open or unbucketed inbound, opening baseline `2026-07-01`, weighted-average state and vitrina metrics `Себестоимость WB наша`, `Доля подтверждённой себестоимости`, `proxy прибыль 3`, `Прокси маржинальность 3`; no strict FIFO/accounting truth and no proxy2 replacement |
