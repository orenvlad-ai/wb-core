...
| `26_MODULE__SHEET_VITRINA_V1_MVP_END_TO_END_BLOCK.md` | `sheet_vitrina_v1_mvp_end_to_end_block` | `web/operator` | current website/operator/web-vitrina contour active, including compact supply district XLSX actions, WB regional district selector/diagnostics UI, plan-report annual-even UI mode and read-only `Отзывы` tab; former Google Sheets load/write path archived |
