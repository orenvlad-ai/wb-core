...
scope: "Папка `docs/modules/`, её naming rules, статус source of truth и полный список модульных документов `01–30`."
source_basis:
...
На текущем `main` main-confirmed модульные блоки доходят до `01–30`.
...
- `web_vitrina_gravity_table_adapter_block` как bounded phase-3 concrete adapter для `@gravity-ui/table` над stable `view_model`, без live page integration.
