...
- Для `sheet_vitrina_v1` тот же entrypoint обслуживает narrow operator surface в двух блоках:
  ...
  - `apps/seller_portal_feedbacks_complaint_batch.py` is a CLI-only production batch coordinator over that same guarded submit runner. It walks date ranges day-by-day, includes only AI `yes/review` candidates, preserves existing-journal skips, keeps `max_submit<=5` per run and reports submitted/not-submitted grouped reasons; it does not add a public WB submit route.
