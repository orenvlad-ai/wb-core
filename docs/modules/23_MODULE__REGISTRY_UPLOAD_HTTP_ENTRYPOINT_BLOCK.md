...
- Operator page keeps one compact server-driven auto block `Автообновления`:
  ...
  - Policy mode `interval` accepts only `interval_hours` 3/4/6, defaults to 4, fixes `window_start_hhmm=10:00`, `window_end_hhmm=22:00`, `timezone=Asia/Yekaterinburg`, returns slot metadata and materializes read-only effective schedule rows from policy. Unsupported intervals, intervals below 3h, duplicate enabled manual times and more than 6 runs/day are rejected. The operator UI maps this contract through one selector (`Ручные настройки`, `Каждые 3 часа`, `Каждые 4 часа`, `Каждые 6 часов`) and treats the save response as canonical state.
