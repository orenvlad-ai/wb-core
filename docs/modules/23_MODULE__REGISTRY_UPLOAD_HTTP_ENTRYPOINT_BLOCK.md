...
- Contract document upload accepts the same supported file set as the document registry. It attempts to parse `number` from the first non-empty line and `document_date` from the bounded header fragment; if the first line is only a generic contract heading, the nearest top lines are searched for `Contract No`, `Contract Number`, `№`, `合同编号` or `合同号`. Manual form values remain authoritative, and responses expose `parsed_number`, `parsed_document_date`, `parser_warnings` and `parser_errors`. XLSX and text-layer PDF are parsed without new external services; image-only PDF/JPG/PNG returns a non-fatal warning unless runtime OCR tools are already available.
