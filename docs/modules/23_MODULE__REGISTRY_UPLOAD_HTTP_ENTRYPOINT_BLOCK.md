...
- Hosted/public publication boundary теперь repo-owned:
  ...
  - active EU target is explicitly marked `target_role=primary_live`, `target_lifecycle=current_live`, `mutation_policy=routine_writes_allowed` and sets `nginx_public_routes.server_names=["89.191.226.88"]`; `api.selleros.pro` remains rollback-only/deprecated legacy target evidence until a separate DNS/TLS contract update;
