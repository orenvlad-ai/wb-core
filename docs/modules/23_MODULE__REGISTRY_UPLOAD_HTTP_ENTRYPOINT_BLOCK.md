...
- `Выполнение плана`:
  ...
  - operator UI uses Russian period labels, a grid for selected/MTD/QTD/YTD cards plus the separate projection card, metric columns `Показатель / Факт / План / Выполнение / Отклонение / Отклонение %`, and keeps status/coverage as text rather than a table column;
