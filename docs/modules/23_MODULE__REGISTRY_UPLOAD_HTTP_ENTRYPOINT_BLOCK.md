...
- Для regional supply block result contract теперь также server-driven:
  ...
  - each WB regional calculate still overwrites the single current `sheet_vitrina_v1_wb_regional_supply_result_state` slot, and also writes a bounded metadata-only `sheet_vitrina_v1_wb_regional_supply_calculation_audit` row for diagnostics. The audit ring-buffer keeps summary fields such as `calculation_id`, `report_date`, selected supply count, included district count, district totals and overlay aggregate counts; it must not store WB supply ids, barcodes or row-level recommendation payloads and is not a source for calculation/planning truth.
