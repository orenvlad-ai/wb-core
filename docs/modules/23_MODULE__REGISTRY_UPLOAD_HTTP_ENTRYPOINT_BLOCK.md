...
- Settings UI is split into three inner subsections, `Номенклатура`, `Договоры` and `Инвойсы`, under the single `Настройки` page. `Договоры` renders only contract documents and supports operator-only inline edit for `number`, `document_date` and `supplier_name`. `Инвойсы` renders only invoice documents, shows the `Сумма invoice` column from document `amount_total`/`currency`, and supports operator-only link/change/unlink for invoice->contract. Neither document table needs a type column. Tab state may be stored in browser-local UI preference only; the server-owned runtime remains the source of truth for nomenclature and documents.
