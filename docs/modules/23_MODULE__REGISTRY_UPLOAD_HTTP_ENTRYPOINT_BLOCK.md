...
- `Выполнение плана`:
  ...
  - ordinary operator UI exposes only `period`, `H1`, `H2`, `DRR` and `Рассчитать`; it does not expose the legacy contract-start or annual-even toggles;
  ...
  - backend compatibility keeps `annual_plan_evenly_distributed=true` as a read-only query mode: it uses `h1_buyout_plan_rub + h2_buyout_plan_rub` as the annual plan, distributed evenly by calendar day across the year or contract-start calculation window; ordinary UI does not expose this mode because live WB/VB evidence matches H1/H2 denominator mode instead;
  ...
  - operator UI uses Russian period labels, a uniform 2x2 grid for selected/MTD/QTD/YTD cards, metric columns `Показатель / Факт / План / Выполнение / Отклонение / Отклонение %`, and keeps status/coverage as text rather than a table column;
  - only H1/H2/DRR/period browser inputs may persist in namespaced `localStorage` as UI convenience; old persisted annual-even/contract-start fields are ignored by ordinary UI, and browser state is not server-side profile state or a fact/source-of-truth layer.
