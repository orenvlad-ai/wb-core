...
related_modules:
  ...
  - "packages/application/wb_regional_supply_export.py"
  - "packages/application/resources/wb_supply_upload_template.xlsx"
...
related_endpoints:
  ...
  - "GET /v1/sheet-vitrina-v1/warehouses/sync/status"
  - "GET /v1/sheet-vitrina-v1/warehouses/ff/documents"
  - "GET /v1/sheet-vitrina-v1/warehouses/ff/documents/{document_id}"
...
related_runners:
  ...
  - "apps/wb_regional_supply_export_smoke.py"
  ...
  - "apps/cny_ledger_browser_smoke.py"
...
- `Настройки` is a right-side common-shell action next to `Выйти`, not part of the main tab strip and not a separate-looking site. It activates the same internal `settings` panel inside the shared shell; `GET /sheet-vitrina-v1/settings` remains backward-compatible and renders the same shell with active `settings` state, while the embedded settings body is also available through `?embedded=1`. The settings content has five top groups: `Справочники`, `Расчётные параметры`, `Автообновления`, `Источники и сессии`, and `Пользователи`. `Источники и сессии` is the only login/recovery UI: separate Seller Portal and WB Buyer cards share one visual recovery state machine without sharing adapters/storage, while `WB Card / SPP Proxy` is explicitly anonymous and exposes availability/freshness only. The first read uses cached server status; a stale status is refreshed after the 180-second TTL, identical checks are browser-single-flight, and short polling exists only during an active recovery. Seller green requires canonical cabinet plus exact transit route/collector/coverage truth; Buyer green requires the exact authenticated-price capability; generic auth alone is insufficient. `Расчётные параметры` keeps its versioned effective-date/revision/preview/diff/save contract and does not repeat the same internal heading. `Автообновления` is a server-owned projection over owner policy plus feature lifecycles. GET returns `process_key`, owner/location/capability, desired source/value, actual/lifecycle state, run/success/next/error, runtime schedule, drift, master suspension and component states. Web-vitrina refresh, closure retry, warehouse/cost and Finance are Settings-managed. Autoanswers, auto-complaints and SPP test are monitoring-only: their cards have no individual mutation control, and backend `set_process` rejects them. The Web-vitrina card owns the existing runtime-JSON schedule editor and run-now route; no schedule data is copied or reset. The global switch is a cross-system pause that preserves every feature-owned intent and resumes it through its canonical lifecycle. A POST is confirmed only after persisted revision/readback proof and an immediate second GET; lifecycle startup may be explicitly pending, while no-op, backend failure, concurrent hold or mismatch stays an error after factual reload. Browser automation and acceptance address controls inside the embedded frame rather than treating them as top-level shell DOM. `Справочники` contains `Номенклатура`, `Группы SKU`, `Договоры` and `Инвойсы`; `Номенклатура` includes server-owned WB barcode/vendorCode/title/subject/updatedAt fields, separate hidden visibility outside operational `is_active`, visible/hidden/all selector, column selector, per-row barcode sync and bounded batch read-only WB Content SKU sync with matching priority `nm_id -> barcode -> vendorCode`. Hidden rows stay eligible for sync matching and are not automatically restored by WB evidence. `Группы SKU` stores the single server-owned key/label/aliases registry used dynamically by the nomenclature selector/export/import and by supplier-invoice group projection/UI; there is no second JavaScript enum or supplier-local group list. Defaults seed Clean/Anti-spy/Matte/No Frame groups plus `extra`/`other`; a legacy key absent from the registry remains visible as its stable key with controlled diagnostics instead of an invented label. `Договоры` renders only contract documents and supports inline edit for `number`, `document_date` and `supplier_name` when the current user has `settings`. `Инвойсы` renders only invoice documents, shows the `Сумма invoice` column from document `amount_total`/`currency`, and supports link/change/unlink for invoice->contract. Neither document table needs a type column. Tab/group/column state may be stored in browser-local UI preference only; the server-owned runtime remains the source of truth for nomenclature, SKU groups, documents, owner policy and users.
...
- Для `sheet_vitrina_v1` тот же entrypoint обслуживает narrow operator surface в двух блоках:
  ...
  - `GET /v1/sheet-vitrina-v1/status` = cheap metadata read для последнего persisted refresh result, where root `status` is active-vitrina semantic snapshot outcome rather than mere ready-snapshot existence. `technical_status` сохраняет transport/read completion; archive-inclusive semantic fields and source rows remain separately available as `technical_semantic_*`, `technical_source_outcome_counts` and `technical_source_outcomes`, while public `source_outcome_counts` and `source_outcomes` are filtered in lockstep.
...
- `GET /sheet-vitrina-v1/operator` = compatibility entry to the same unified shell as `/sheet-vitrina-v1/vitrina`; active top-level tabs include `Витрина` / `Расчёт поставок` / `Отчёты` / `Отзывы` / `Исследования`, while former standalone `Обновление данных` is not a separate top-level tab
  ...
  - Seller Portal recovery remains a bounded backend seam, but its login/recovery controls are rendered only in `Настройки → Источники и сессии`; operator data/action pages expose monitoring and exact collector/source checks only:
    ...
    - the centralized source card shows `Текущий запуск / Статус запуска / Финал запуска / Статус сессии / Старт / Завершение`, so one recovery-run final is never confused with a generic session outcome
  ...
  - current bounded live UX on `/sheet-vitrina-v1/vitrina` now includes:
    ...
    - operator-style layout: compact top panel and summary/status strip, main vitrina table first, then filters/settings, historical controls and bottom `Действия и состояния`; Web-vitrina schedule controls are absent from this page and live only in Settings
    ...
    - `Seller Portal / бот` in `Действия и состояния` is monitoring-only: it exposes the group-local `Проверить` action and source/job/data truth, but no generic session check, relogin, launcher or recovery control. The ambiguous top Seller-session badge is absent. Login/recovery is available only through `Настройки → Источники и сессии`. `Лог` renders below the loading table and keeps the existing job/log download contour; the former `Обновление данных` activity block is not an active page-composition surface
    ...
    - the former bulky `Свежесть данных` and `Строки` cards are merged away from the top summary; automatic freshness is surfaced by the Vitrina card in `Настройки → Автообновления`, while the top strip keeps browser reread timestamp, semantic status and selected period. Refresh/manual/auto job semantics are reduced over active metric sources before persistence, so an archived-only 1C failure remains in `technical_semantic_*` / `technical_source_*` audit fields but cannot mark the active job failed or suppress `last_successful_auto_update_at`
    - `export_layer`
  ...
  - `GET /v1/sheet-vitrina-v1/supply/ff-stocks` = protected server-owned `Остатки ФФ` ledger status with current balance registry and operation journal
  - `GET /v1/sheet-vitrina-v1/supply/ff-stocks/export.xlsx` = current `Остатки ФФ` balance export in the same stable manual document shape
  - `POST /v1/sheet-vitrina-v1/supply/ff-stocks/preview` = manual receipt/writeoff Excel parse preview without applying balance movements
  ...
  - `POST /v1/sheet-vitrina-v1/supply/wb-regional/planning-options` = protected read-only storage picker for one planning zone. It resolves all allocated `nmId -> barcode`, calls official WB `POST /api/v1/acceptance/options` with exact quantities, then requires the same exact `warehouseID` with `canBox=true` for every barcode, active catalog evidence and internal storage eligibility. Central warehouse identity is exact-ID registry-owned; СЦ/СГТ/specialised/inactive/partial/blocked/unclassified options are exclusion diagnostics only. Box coefficient rows use `boxTypeID=1/2`; available days require `allowUnload=true` and coefficient `0/1`, are unique and chronological. Response fields and reason codes are backend-ready; no WB supply/slot is created or drafted.
  ...
  - `GET /v1/sheet-vitrina-v1/supply/calculations` = read-only unified registry list for factory-order and WB regional calculations; supports exact `calculation_type`, inclusive `date_from/date_to`, stable `calculated_at desc, record_id desc` ordering, offset pagination, default `limit=25` and hard maximum `100`
  - `GET /v1/sheet-vitrina-v1/supply/calculations/{record_id}` = immutable calculation detail with exact saved settings, selected WB supply ids, incident/source evidence, warnings, summary and full result payload; legacy regional metadata-only rows truthfully return `is_reproducible=false` and no invented payload
  - `GET /v1/sheet-vitrina-v1/supply/calculations/{record_id}/download` = historical XLSX/ZIP bytes stored with that complete registry record; it never rebuilds from the mutable latest slot or current sources
  ...
  - `POST /v1/sheet-vitrina-v1/supply/cny-account/documents` = idempotent PDF upload for canonical CNY currency documents from the CNY account context; upload stores one runtime document, parses VTB-like conversion purchase PDFs, confirms deterministic ledger readback and then enqueues one coalesced derived replay
  ...
  - operator `Конвертации RUB → CNY` renders the direct delete action only for documents accepted by that account route; it requires an explicit document-named replay warning, blocks repeat clicks while DELETE is in flight, preserves the current row on backend error, and reloads the complete CNY account read model after `deleted=true`. Linked source-owned rows expose a disabled `Через карточку заказа` state instead of bypassing ownership guards
...
- Для strict closed-day acceptance тех же bot/web-source families existing refresh contour теперь различает три уровня truth:
  - dated Seller Portal replay теперь принадлежит `packages/adapters/seller_portal_web_source_collector.py` и `apps/seller_portal_web_source_collect.py` в `wb-core`; Release Runner устанавливает их вместе с приложением. `/opt/wb-web-bot` предоставляет существующую сессию, зависимости и конфигурацию исходной БД; `/opt/wb-ai` сохраняет существующий handoff/API. Production-копии старых `bot.fetch_*` больше не являются кодом сборщиков, запускаемых витриной.
  - replay использует реально захваченное имя `seller-lk` либо `wb-seller-lk`, подтверждает единственный канонический кабинет и точный период; отсутствие/конфликт идентичности останавливает сбор. Воронка читает все страницы с проверкой групповых сумм, поиск сверяет число товаров с `commonInfo.totalProducts`.
  - ошибка сбора не принимает старый API payload. Для дат с 11.09.2026 accepted/cache closed truth требует `source_fetched_at` после конца делового дня; новое время приёмки не заменяет исходное. История до этой границы остаётся прежней. Последний полезный текущий срез остаётся `incomplete/latest_confirmed`, задолженность сохраняется в существующей ограниченной очереди повторов. Текущий срез старше часа требует нового сбора; нулевой текущий payload остаётся непригодным для принятия.
  - Freshness привязана к serving generation: exact raw→handoff состав, значения, report JSON и clocks должны совпасть; затем API payload сверяется с хешами строк этой же версии. Свежий raw после неуспешного handoff не делает старый API успешным.
  - адресное восстановление этих двух источников выполняется `web_source_publication_v1` через общий Production Apply launcher: отдельные source/publication операции, frozen dated responses, точные before/candidate fingerprints, один submit и readback. Publication меняет только показы, открытия и CTR, их source status/provenance и accepted slots; остальные ready cells/metadata защищены structural digest. До записи сохраняются exact before images исходных и handoff-таблиц и конкретных ready/slot/closure строк. Source phase использует одну проверенную connection identity для before/write/readback/rollback; raw и handoff фиксируются двумя короткими транзакциями с журналом точного after-state, сохранённым до каждого commit. Readback сравнивает все поля исходных/handoff строк; rollback допускает только exact before/own-after mixture под locks и не затирает сторонний drift. Publication CAS включает полные slot/closure записи, в том числе captured_at и состояния попыток; сначала отменяется publication, затем source.
...
- Browser-side operator navigation/report state remains page-owned and non-persistent on the server:
...
- Во второй tab `Расчёт поставок` current bounded scope materialize-ит два sibling block внутри одного narrow operator page:
  - shared calculation input `Остатки ФФ` остаётся один для обоих расчётов: manual Excel хранится в одном server-owned dataset state, optional `1С / Фулфилмент` reads current materialized 1C `FF_STOCK` rows without writing them into the manual upload state, and source `Остатки ФФ` / `ff_stock_ledger` reads computed balances from the server-owned ФФ quantity ledger;
  - same shared block now exposes `Учесть WB-поставки`: only statuses `3/4/6` are returned as selector candidates, statuses `1/2/5` and `Допринято` are excluded from UI options. On the first successful option load of a fresh page the UI visibly checks every backend option where `eligible_for_overlay=true` and `disabled=false`; it does not reproduce eligibility rules in JavaScript. After that initialization, manual checks/unchecks survive option refresh, background rerender and switching between the two calculation forms, except that options which disappear or become invalid are dropped. A new page load creates a new selection session and applies the then-current eligible default. Exact selected ids are sent as `selected_wb_supply_ids` to both calculate routes; backend revalidates them from server WB supplies cache and returns `wb_supply_overlay` diagnostics in the calculate result;
  - рядом расположен общий selector `Не учитывать при расчёте`, backed by a fresh complete official WB stock snapshot. Unlike page navigation, its selected IDs are canonical server-owned user config `wb_warehouse_exclusions`; existing local-only IDs migrate once when that record is absent, after which localStorage is only cache. The same record is read by factory order, WB regional planning, warehouse picker and `Управление SKU`. Stable identity is numeric `warehouseID`, including `0 / Остальные`; duplicate names stay separate. Options sort by `total_contour desc`, then Russian name and ID; selected temporarily missing rows remain at the bottom with warning. Backend exclusions run before picker probes/ranking/download and before SKU total/regional forecast openings. IDs are never dropped merely because a warehouse became zero or temporarily disappeared. `warehouseID=0` remains available as the explained `Остальные — служебная группа WB` reconciliation bucket but is always forbidden as a destination. The server revalidates IDs/snapshot on every calculation and persists snapshot fingerprint plus actual/excluded/effective per-`nmID` reconciliation. Historical `exclude_elektrostal_stock=true` is read-only compatibility for ID `120762`; new payloads never emit the boolean;
  ...
  - common ФФ overlay: for manual Excel and `1С / Фулфилмент`, when selected WB supplies exist, `effective_stock_ff = max(base_stock_ff - selected_wb_supply_qty, 0)` and `over_reserved_qty = max(selected_wb_supply_qty - base_stock_ff, 0)`; for `ff_stock_ledger`, selected WB supplies do not reduce `stock_ff` again because the ledger balance is already current after movements, including WB auto writeoffs;
  ...
  - block `Поставка на Wildberries` uses the shared `stock_ff` source and a separate `v2_planning_zones` settings/result contract: `central_north`, `central_east`, `central_south`, `northwest`, `volga`, `ural`, `south_caucasus`, `far_siberia`. Legacy scalar lead time still expands across all current zones; a v1 saved result is preserved as migration evidence and requires recalculation instead of an invented Central split;
  - selected WB supplies reduce the bounded FF pool and add in-transit quantity only to their planned/target destination planning zone. Central requires exact registry warehouseID mapping; actual/transit warehouses never decide the zone, and unknown Central destinations fail closed with diagnostics;
  ...
  - operator chooses planning zones participating in share validation/normalization. Default is all eight repo-owned zones; `ЦФО Запад` does not exist. Excluded zones remain in selector/options but are hidden from result/downloads, receive zero share/demand and return controlled `422` on direct XLSX download;
  ...
  - field `Доставка, дней` in the WB regional UI means regional lag until goods are available on WB: physical delivery plus acceptance, sorting/breakdown and appearance on sale. Calculation uses `settings.lead_time_to_region_days_by_district[district_key]` in `projected_stock_on_eta = max(current_stock + selected_wb_supply_qty - district_daily_demand * district_lead_time_days, 0)`;
  - result and row diagnostics expose `regional_share_method_counts`, `share_source_counts`, `fallback_sku_count`, `primary_sku_count`, low-confidence and partial/group/global/seed SKU-district counts, per-SKU `district_share_sources`, observation counts, `group_prior_key`, peer counts, final shares, confidence, seed reasons, result-level `lead_time_to_region_days_by_district`, and row-level `lead_time_to_region_days`, `share_source`, `share_confidence`, `demand_recommendation_qty`, `seed_qty`, `allocation_reason`;
  - each positive planning-zone row opens the latest calculation in one separate full-width picker below the parameters/result grid and scrolls to it. The request sends exact `calculation_id` and selected zone; stale IDs retry once after status refresh. Missing barcodes stop before WB. The manager table shows only eligible storage warehouses for that zone with role, exact ID, complete coverage, box support, first available/free and several normalized dates, tariffs, direct-destination status and explicit blockers; excluded raw evidence stays in bounded diagnostics. Table overflow is local on desktop/mobile. The action is read-only and does not persist, reallocate, create or book a WB supply.
  ...
  - factory-order treats selected WB supplies as automatic overlay over existing `Товары в пути ФФ -> Wildberries`: selected quantities add to `inbound_ff_to_wb` only when their selected operational date is inside the existing inbound window; for manual Excel and `1С / Фулфилмент` they also reduce free `stock_ff`, while for `ff_stock_ledger` they do not reduce ledger `stock_ff` a second time; out-of-window events are diagnosed and do not add current-window coverage;
  - settings fields остаются server-owned и валидируются на backend;
  - operator-facing vocabulary uses `Период усреднения продаж`, lead times, safety, `Кратность штук в коробке`, `Цикл`; supply selector labels are `ЦФО Север`, `ЦФО Восток`, `ЦФО Юг`, `СЗФО`, `ПФО`, `УФО`, `ЮФО/СКФО`, `ДВФО/СФО`;
  ...
  - `Остатки ФФ` now has mutually exclusive source selection: `manual_excel` (default/backward-compatible uploaded XLSX), `onec_ff_stock` (`1С / Фулфилмент`) or `ff_stock_ledger` (`Остатки ФФ`). The 1C source is read-only over existing `onec_stocks` / `onec_product_capital` materialized ready snapshot rows and uses only metric `onec_FF_STOCK_qty` from stage bucket `FF_STOCK`; `onec_total_qty` is intentionally not a valid `stock_ff` input because it includes other stages;
  - when `onec_ff_stock` or `ff_stock_ledger` is selected, factory-order and WB regional calculations do not require an uploaded manual `stock_ff` file. The 1C resolver requires active SKU coverage from materialized `onec_FF_STOCK_qty`; explicit zero qty is valid, missing/non-numeric/negative rows produce controlled check/partial/error state and block calculation instead of fabricating stock. The ledger resolver emits current active-SKU balances from ledger lines; explicit zero and negative balances are valid, with negative rows surfaced as warnings;
  ...
  - supplier registry source converts saved supplier orders into factory inbound rows only from `production` / `in_transit` shipments whose product lines are resolved to `nmId` by exact barcode (`matched_by_barcode`; legacy `matched` / `matched_by_compatibility` remain readable), have positive quantity and belong to a saved shipment with valid planned `shipment_date`; legacy shipments without `order_status` fall back to `production`, while `accepted_ff` shipments are excluded with diagnostics/warning to avoid double-counting accepted ФФ stock;
  ...
  - factory-order status/calculate responses expose selected `factory_inbound_source`, selected `stock_ff_source`, manual stock ФФ dataset state, 1C FF_STOCK summary, manual inbound dataset state, supplier registry shipment summary/diagnostics/warnings, `target_window_days`, `inbound_window_end` and effective `inbound_factory_to_ff` rows that actually entered the calculation;
  - operator UI can refresh the supplier registry inbound summary with `Обновить` in the `Товары в пути из реестра поставщика` block; it re-reads the factory-order status endpoint and updates supplier rows/totals/warnings/excluded `accepted_ff` counters without a full page reload. Embedded supplier-registry status PATCH also signals the parent operator page to refresh this summary;
...
- Для regional supply block result contract теперь также server-driven:
  ...
  - server хранит и публикует отдельный operator XLSX на каждый included округ and one atomic ZIP archive for all included recommendations. ZIP filename is `Рекомендации_поставок_<date>_<time>_<calculation_id>.zip`; recommendation order follows the result/UI order, and each recommendation has one safe unique prefix derived from ordinal, `calculation_id` and destination label;
  - inside ZIP every recommendation is a separate folder with exactly `<prefix>__01_РЕКОМЕНДАЦИЯ.xlsx` and `<prefix>__02_ЗАГРУЗКА_WB.xlsx`. The operator workbook keeps district identification and `nmId / SKU / Количество к поставке` but no longer exports the `Дефицит` column; internal deficit calculation and the UI summary remain unchanged;
  - the WB upload workbook is populated only from the checked-in byte-identical canonical resource `packages/application/resources/wb_supply_upload_template.xlsx` (source SHA-256 `db02c1d7f01759b24fca1b513d32600a61da2473db871b64d427d1cdbf21df79`). Its factual current structure is one `Sheet1` with text headers `Баркод / Количество` and no OOXML structured-table part; generator preserves that topology and expands a structured table/filter range if a later canonical template contains one;
  - WB rows resolve exact ASCII-digit barcode text only through active server-owned nomenclature by `nmId`, preserve leading zeros, exclude zero rows, reject negative/fractional quantities, missing/invalid/ambiguous barcode evidence and multiple active nomenclature matches, merge duplicate barcodes in first-appearance order and reconcile the exact quantity total with the operator recommendation. Any recommendation error cancels the whole in-memory export before a response body is written and reports destination, recommendation id and problematic SKU;
  - each successful factory-order and WB regional calculation first builds its exact historical XLSX/ZIP and then atomically updates the existing latest-result compatibility slot plus `sheet_vitrina_v1_supply_calculation_registry`. Export-build or transaction failure leaves both latest and history unchanged; a successful calculation never creates a complete record without its historical bytes. A registry record keeps exact settings, `selected_wb_supply_ids`, incident-policy revision/status/digest, bounded source/dataset states and fingerprints, fingerprints of the actual sales samples/regional demand estimates used by the calculation, warnings, summary, complete result payload and the exact historical XLSX/ZIP bytes. This evidence is sufficient to reopen the shown result and download it without reading current mutable sources; credentials, sessions, source secrets and unbounded raw WB responses are not stored.
  - complete registry rows are immutable and globally bounded to the newest `200`; the record referenced by each latest-result slot is retained even when its calculation timestamp is backdated. Per-record fail-closed storage limits are `32 MiB` canonical payload, `8 MiB` metadata/evidence and `64 MiB` export. `calculation_id` is the immutable identity: an exact repeat with equal canonical payload, evidence, timestamp and export digest is idempotent, while any divergent collision fails closed before latest-result update.
  - existing `sheet_vitrina_v1_wb_regional_supply_calculation_audit` remains as the bounded metadata-only compatibility audit. Schema initialization exposes its old rows in the unified registry as `legacy_metadata`, without copying or inventing selected supply ids, barcodes, row payloads or exports. They are clearly marked incomplete and never used as calculation/planning truth.
  - `Поставки -> Расчёты -> Реестр расчётов` is a read-only third subsection for both types. It shows type, calculation time, report date, server summary, key settings, selected supply count/quantity, incident revision/status, warnings and completeness; filters by type/date; paginates in the same stable order; opens exact detail; and offers download only when the stored historical export exists. The same supply authorization protects list, detail and download; no delete/mutation UI is present.
  - direct district XLSX and the ZIP operator member contain district identification plus compact rows `nmId / SKU / Количество к поставке`; quantity comes from factual allocation after the `stock_ff` limit. Districts outside the latest methodology selection stay excluded.
...
- `Отчёт по остаткам`:
  ...
  - visible stock-report column order after `Акция` is `на произв.`, `в пути Китай`, `ост. ФФ`, `поставки ВБ`, `ост. ВБ`
  - `на произв.` reads the existing supplier shipment registry (`sheet_vitrina_v1_supplier_shipments` + product lines) by active SKU `internal_nm_id` and sums positive `matched_by_barcode` product-line `qty` only for order status `production` (`На производстве`); legacy `matched` / `matched_by_compatibility` rows remain readable, and rows without `order_status` are normalized by the current runtime model to `production`
  - `в пути Китай` uses the same supplier shipment registry source and sums the same matched product-line `qty` only for order status `in_transit` (`В пути`); `accepted_ff` and every other status do not enter these cycle columns
  - table renders a top `Итого` row before SKU rows; browser sorting reorders only detail SKU rows while `Итого` stays first. Quantitative stock/supply columns and supported district stock columns are summed, `avg_sales_per_day` is summed as total daily demand, and days-left columns are calculated from aggregate stock / aggregate demand or district burn instead of averaging row-level days.
...
- `Настройки → Автообновления` keeps the sole compact server-driven Vitrina schedule card; the Vitrina/operator page contains no duplicate block, schedule request, listener or hidden control:
  - The top readback uses explicit operator states `Работает штатно`, `Запускается`, `Приостановлено пользователем`, `Приостановлено общей паузой`, `Есть расхождение`, `Ошибка процесса`, `Нет свежего подтверждения` and `Состояние неизвестно`; a yellow state always includes its concrete reason.
  - Every process card separates desired from actual and shows last successful cycle, scheduler freshness, pause/stop reason, last error and readback source. Raw timer/service details, fingerprints and technical IDs stay in the closed technical disclosure.
  - Autoanswers remains monitoring-only here and continues to be owned by `Отзывы → Отзывы`. Its card additionally shows mode, readonly/worker desired and actual, lifecycle, policy revision/transition run and read-only budget evidence. A conservative uncertainty hold is explicitly described as a possible-cost upper bound, not confirmed spend; rendering this status never changes feature policy, caps, holds or starts provider/WB calls.
  ...
  - Policy mode `interval` accepts only `interval_hours` 3/4/6, defaults to 4, fixes `window_start_hhmm=10:00`, `window_end_hhmm=22:00`, `timezone=Asia/Yekaterinburg`, returns slot metadata and materializes read-only effective schedule rows from policy. Unsupported intervals, intervals below 3h, duplicate enabled manual times and more than 6 runs/day are rejected. Interval materialization preserves server-owned lifecycle fields by canonical schedule id and by matching interval slot (`timezone + HH:mm`) so cadence/id migration does not reset row status to permanent `pending`. The Settings UI maps this contract through one selector (`Ручные настройки`, `Каждые 3 часа`, `Каждые 4 часа`, `Каждые 6 часов`) and treats the save response as canonical state.
...
- smoke:
  ...
  - `apps/supply_calculation_registry_smoke.py`
  - `apps/sheet_vitrina_v1_factory_order_http_smoke.py`
  - `apps/sheet_vitrina_v1_wb_regional_supply_http_smoke.py`
  - `apps/sheet_vitrina_v1_operator_ui_persistence_smoke.py`
...
- Smoke проверяет:
  ...
  - что operator page рендерит compact `Ручная загрузка данных`, не содержит дублирующий `Автообновления`, а Settings-карточка Витрины показывает truthful runtime schedule/status;
...
- Existing live daily refresh scheduler materialize-ится как repo-owned systemd due-check timer `wb-core-sheet-vitrina-refresh.timer`: timer cadence is every 10 minutes, service runs `apps/sheet_vitrina_v1_auto_refresh_tick.py`, and the runner reads runtime JSON schedules before starting due full-refresh jobs.
  - default runtime business schedules are `11:00` and `20:00 Asia/Yekaterinburg`; add/edit/delete/disable stays on `GET/POST /v1/sheet-vitrina-v1/web-vitrina/auto-schedules` and is exposed only by `Настройки → Автообновления`;
...
# 21. SQLite contention HTTP boundary

Every protected GET/POST request establishes a sanitized SQLite operation context. A final `SQLITE_BUSY`/`SQLITE_LOCKED` exhaustion is normalized centrally, including exceptions wrapped by route-specific services: HTTP status is `503`, `Retry-After: 2`, and the JSON contract is `wb_core_sqlite_contention_v1` with a Russian user message and `retryable=true`. Raw SQL, database filenames and internal exception text never cross the HTTP boundary.
...
The same truthful boundary applies to CNY account upload, opening balance, explicit replay, archive and supplier-owned CNY restore/relink/status contours. After canonical document/ledger readback, a derived pending/error outcome returns `202 sheet_vitrina_v1_cny_write_pending_v1` with `operation_applied=true`, `readback_confirmed=true`, `pending_phase=derived_replay`, a truthful `retryable` boolean and a durable stable source/revision identity; an exact shipment/SKU data-quality blocker uses `retryable=false` instead of suggesting a blind retry. `200` means full success or a proven terminal no-op. An unconfirmed primary outcome remains an error and is not relabelled pending. The browser clears and rereads after `200/202`; for a lost upload response it compares the exact local SHA with the CNY read model and its ledger operation before choosing saved-warning versus genuine red failure.
