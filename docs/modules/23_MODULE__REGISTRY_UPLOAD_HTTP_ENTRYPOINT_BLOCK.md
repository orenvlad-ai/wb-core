...
- `Выполнение плана`:
  ...
  - when contract-start mode is enabled, every selected/MTD/QTD/YTD block is trimmed with `effective_start = max(original_period_start, contract_start_date)` before fact, plan, DRR, ads-plan and coverage are calculated; the default H1/H2 denominator remains the full calendar half-year denominator, while annual-even mode uses the remaining annual calculation window from contract start through year end;
