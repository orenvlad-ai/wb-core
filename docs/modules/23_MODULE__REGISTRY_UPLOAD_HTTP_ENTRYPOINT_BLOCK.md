...
- `GET /sheet-vitrina-v1/operator` = compatibility entry to the same unified shell as `/sheet-vitrina-v1/vitrina`; active top-level tabs include `Витрина` / `Расчёт поставок` / `Отчёты` / `Отзывы` / `Исследования`, while former standalone `Обновление данных` is not a separate top-level tab
  ...
  - current bounded live UX on `/sheet-vitrina-v1/vitrina` now includes:
    ...
    - the former bulky `Свежесть данных` and `Строки` cards are merged away from the top summary; automatic freshness is surfaced in `Автообновления` through last run/success/error/next run, while the top strip keeps browser reread timestamp, semantic status and selected period. Refresh/manual/auto job semantics are reduced over active metric sources before persistence, so an archived-only 1C failure remains in `technical_semantic_*` / `technical_source_*` audit fields but cannot mark the active job failed or suppress `last_successful_auto_update_at`
    - `export_layer`
