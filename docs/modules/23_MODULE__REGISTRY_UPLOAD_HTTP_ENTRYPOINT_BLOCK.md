...
- `Выполнение плана`:
  ...
  - operator UI defaults, when `plan_report_inputs` is missing or invalid in namespaced `localStorage`, are `period=first_half`, `H1=155379879`, `H2=294620121`, `plan_drr_pct=6`, `use_contract_start_date=true`, `contract_start_date=2026-02-01`, `annual_plan_evenly_distributed=false`; `H2=294620121` is the annual remainder to `450000000`, while Q3+Q4 source figures total `294620120`, so the 1 rub rounding discrepancy is deliberately visible;
  ...
  - with `annual_plan_evenly_distributed=true`, buyout plan remains read-only and uses `h1_buyout_plan_rub + h2_buyout_plan_rub` as the annual plan, distributed evenly by calendar day across the year or contract-start calculation window; the persisted H1/H2 input values are not rewritten;
  - when contract-start mode is enabled, every selected/MTD/QTD/YTD fact window and fixed target window is trimmed with `effective_start = max(original_period_start, contract_start_date)` before facts, plan, DRR, ads-plan and coverage are calculated; the default H1/H2 denominator remains the full calendar half-year denominator, while annual-even mode uses the remaining annual calculation window from contract start through year end;
  ...
  - H1/H2/DRR/period, annual-even checkbox and optional contract-start checkbox/date browser inputs may persist only in namespaced `localStorage` as UI convenience; this is not server-side profile state and not a fact/source-of-truth layer.
