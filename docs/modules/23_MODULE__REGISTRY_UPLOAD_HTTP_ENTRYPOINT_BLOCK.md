...
current_update_note: "`Настройки` теперь встроены в общий WebCore shell как top navigation tab; внутри есть группы `Справочники` (`Номенклатура`, `Договоры`, `Инвойсы`) и admin-only `Пользователи`. Runtime users live in SQLite `sheet_vitrina_v1_users`; env operator is bootstrap admin, env supplier stays backward-compatible supplier-only, and `supply_operator` is restricted to `Поставки`."
---
...
- Runtime users are server-owned in SQLite table `sheet_vitrina_v1_users`; passwords are stored only as PBKDF2 hashes and are never returned by API/UI. Supported roles are `admin`, `operator`, `supply_operator` and `supplier`. `admin` has full access plus user administration. `operator` keeps the current full operator surface except user administration. `supply_operator` can open the common shell only on `Поставки` and the needed `/v1/sheet-vitrina-v1/supply/...` routes, while `Витрина`, `Отчёты`, `Отзывы`, `Исследования`, `Настройки` and users/settings APIs are forbidden. Disabled/archived runtime users do not authenticate, and active sessions for them fail subsequent protected requests after the runtime row is deactivated.
...
- `Настройки` is a common-shell top navigation tab, not a separate-looking site. `GET /sheet-vitrina-v1/settings` remains backward-compatible and renders the same shell with active `settings` tab; the embedded settings body is also available through `?embedded=1`. The settings content has two top groups: `Справочники` and `Пользователи`. `Справочники` contains existing `Номенклатура`, `Договоры` and `Инвойсы`; `Договоры` renders only contract documents and supports operator/admin inline edit for `number`, `document_date` and `supplier_name`. `Инвойсы` renders only invoice documents, shows the `Сумма invoice` column from document `amount_total`/`currency`, and supports operator/admin link/change/unlink for invoice->contract. Neither document table needs a type column. Tab/group state may be stored in browser-local UI preference only; the server-owned runtime remains the source of truth for nomenclature, documents and users.
- Settings document routes are admin/operator-only: `GET/POST/PATCH/DELETE /v1/sheet-vitrina-v1/settings/documents...` and generic `.../{document_id}/file` never authorize supplier or supply-only roles by document id alone. Users routes `GET/POST/PATCH/DELETE /v1/sheet-vitrina-v1/settings/users...` are admin-only, return controlled JSON errors, reject duplicate usernames against runtime/env principals, validate password presence/length on create/change, and block deleting/disabling/demoting the last runtime admin when no bootstrap admin fallback exists. Supplier/factory document file access is scoped through `GET /v1/sheet-vitrina-v1/supply/supplier-shipments/{shipment_id}/invoice|contract`, where the document must be linked to an accessible shipment.
