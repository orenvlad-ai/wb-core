...
- Во второй tab `Расчёт поставок` current bounded scope materialize-ит два sibling block внутри одного narrow operator page:
  ...
  - рядом расположен общий selector `Не учитывать при расчёте`, backed by a fresh complete official WB stock snapshot. Stable identity is numeric `warehouseID`, including `0 / Остальные`; duplicate names stay separate. Selected IDs are sent as `excluded_wb_warehouse_ids` to both calculators and are never dropped merely because a warehouse became zero or temporarily disappeared. The server revalidates IDs/snapshot on every calculation and persists snapshot fingerprint plus actual/excluded/effective per-`nmID` reconciliation. Historical `exclude_elektrostal_stock=true` is read-only compatibility for ID `120762`; new payloads never emit the boolean;
