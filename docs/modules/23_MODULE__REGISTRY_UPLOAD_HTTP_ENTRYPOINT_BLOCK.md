...
- `GET /sheet-vitrina-v1/operator` = simple repo-owned page с top-level tabs `Обновление данных` / `Расчёт поставок` / `Отчёты`
  ...
  - current bounded live UX on `/sheet-vitrina-v1/vitrina` now includes:
    ...
    - `POST /v1/sheet-vitrina-v1/web-vitrina/group-refresh` is the web-vitrina group action seam; the client surfaces launch/loading/error state per group and writes a visible launch-failure line when the POST returns non-2xx before job creation; once the route reaches the app, it starts a job/log-backed partial refresh/load for one source group and must not clear, overwrite or timestamp unrelated groups
