...
- Во второй tab `Расчёт поставок` current bounded scope materialize-ит два sibling block внутри одного narrow operator page:
  ...
  - result and row diagnostics expose `regional_demand_method`, `included_district_keys`, `excluded_district_keys`, `district_selection_mode`, `requested_valid_day_count`, `selected_valid_day_count`, `inspected_day_count`, `initial_window_valid_day_count`, `excluded_day_reason_counts`, used depletion dates, lookup depth, average depletion shares, `total_daily_demand_source=orderCount`, fallback fields and compact UI warning text;
  - limited `stock_ff` box allocation now chooses the next box by `marginal_saved_units = min(order_batch_qty, remaining_raw_shortage_units_for_district)` before coverage-days, district-demand and stable-order tie-breaks. Full recommendation, truthful deficit and box multiple semantics stay unchanged;
...
- Для regional supply block result contract теперь также server-driven:
  ...
  - compact diagnostics surface under result shows stock-depletion/fallback method, requested/selected valid days, inspected lookup depth and first warning; long fallback `nmId` lists and reason counters are kept inside a bounded expandable diagnostics panel so the result card cannot expand horizontally; district XLSX stays compact and does not add service columns;
