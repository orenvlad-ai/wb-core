...
- Во второй tab `Расчёт поставок` current bounded scope materialize-ит два sibling block внутри одного narrow operator page:
  ...
  - block `Поставка на Wildberries` использует тот же shared `stock_ff` source selector, свои settings (`sales_avg_period_days`, `cycle_supply_days`, legacy scalar `lead_time_to_region_days`, canonical `lead_time_to_region_days_by_district`, `safety_days`, `order_batch_qty`, `report_date_override`, `included_district_keys`) и отдельный result/download surface. New UI payload sends delivery/acceptance lead time per canonical district key (`central`, `northwest`, `volga`, `ural`, `south_caucasus`, `far_siberia`); legacy payload with only scalar `lead_time_to_region_days` remains valid and expands to all districts, while a provided per-district map must contain only known keys and all six keys;
  - selected WB supplies affect regional allocation by reducing available ФФ pool and adding each mapped event only to its planned/target warehouse destination calculation district (`central`, `northwest`, `volga`, `ural`, `south_caucasus`, `far_siberia`); actual/transit warehouses remain route/display evidence and never decide the district. Unmapped planned/target warehouses warn and do not add district quantity;
  ...
  - operator-facing vocabulary around supply inputs is unified as `Период усреднения продаж` / lead times / safety / `Кратность штук в коробке` / `Цикл`; WB regional district selector uses compact labels `ЦФО`, `СЗФО`, `ПФО`, `УФО`, `ЮФО/СКФО`, `ДВФО/СФО` as primary UI text and keeps full district names only as tooltip/aria context;
