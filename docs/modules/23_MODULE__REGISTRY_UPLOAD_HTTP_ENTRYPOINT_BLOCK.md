...
- Optional supplier credentials use the same session-secret/hash style through `WB_CORE_SUPPLIER_AUTH_USERNAME`, `WB_CORE_SUPPLIER_AUTH_PASSWORD_HASH` and `WB_CORE_SUPPLIER_AUTH_DISPLAY_NAME`. Supplier role can access only `/sheet-vitrina-v1/supplier`, supplier shipment parse/list/create/detail/patch APIs, invoice downloads, login/logout and needed static/browser assets; supplier cannot access `/sheet-vitrina-v1/vitrina`, `/sheet-vitrina-v1/operator`, `/sheet-vitrina-v1/settings`, nomenclature APIs, supplier delete/rematch or unrelated operator APIs. Operator role remains full-access and can use the supplier module inside `Поставки`.
...
- Во второй tab `Расчёт поставок` current bounded scope materialize-ит два sibling block внутри одного narrow operator page:
  ...
  - valid stock-depletion day requires paired `D-1`/`D` stock snapshots, full SKU/district coverage across selected districts, numeric non-negative stocks, no selected-district restock/upward correction, no selected-district out-of-stock risk on positive SKU order signal, positive selected-district depletion signal and an `orderCount` sample above the same low-signal tolerance idea as factory demand (`median_positive_orderCount * 0.45`, floor `1`). Guarded persistent-zero neutralization is the narrow exception: exact selected-district `0 -> 0` pairs for a SKU/district with current zero/no-usable-stock signal (`current_stock < order_batch_qty`) are neutralized and get `0` depletion share instead of invalidating the whole day, but `positive -> 0`, restock, missing/non-numeric/negative stock, missing snapshots/SKU coverage and low-order days remain invalid;
  - operator can choose federal districts participating in stock-depletion validation/share calculation. Default is all repo-owned districts. Excluded districts are not removed from output tables/XLSX, do not invalidate days, receive `0` share/demand in primary stock-depletion mode, and `calculate` rejects empty/unknown district selections with controlled validation errors;
  - per-day district shares are `district_depletion / total_depletion` across selected districts; average shares are normalized across selected districts. A selected district with clean zero depletion remains present with `0` share/demand; the Far/Siberia merged district is never removed from the system;
  - if the bounded lookup cannot collect the requested valid days, regional calculation falls back explicitly to current-stock-share distribution across selected districts with `regional_demand_method=current_stock_share_fallback`, `fallback_used=true`, `fallback_reason` and operator-visible warning. Excluded districts remain at `0`; silent partial 7/8/9-day success is not treated as the primary methodology;
  - result and row diagnostics expose `regional_demand_method`, `included_district_keys`, `excluded_district_keys`, `district_selection_mode`, `requested_valid_day_count`, `selected_valid_day_count`, `inspected_day_count`, `initial_window_valid_day_count`, `excluded_day_reason_counts`, used depletion dates, lookup depth, average depletion shares, `total_daily_demand_source=orderCount`, persistent-zero neutralization counters, seed/test-box counters, fallback fields and compact UI warning text;
  ...
  - exploratory seed allocation is separate from demand allocation: a selected persistent-zero SKU/district with no usable current district stock (`current_stock < order_batch_qty`), positive total SKU demand and valid `order_batch_qty` may receive one test box if remaining `stock_ff` allows. The seed consumes `stock_ff`, is exposed as `seed_qty`/`demand_qty` diagnostics, can be unfulfilled when FF stock is insufficient, and is explicitly not treated as proven demand;
...
- Для regional supply block result contract теперь также server-driven:
  ...
  - compact diagnostics surface under result shows stock-depletion/fallback method, requested/selected valid days, inspected lookup depth, persistent-zero/seed summary and first warning; long fallback/persistent-zero `nmId` lists, seed details and reason counters are kept inside a bounded expandable diagnostics panel so the result card cannot expand horizontally; district XLSX stays compact and does not add service columns, while `Количество к поставке` includes any allocated seed qty;
