...
- Settings UI is split into two inner subsections, `Номенклатура` and `Договоры и инвойсы`, under the single `Настройки` page. Tab state may be stored in browser-local UI preference only; the server-owned runtime remains the source of truth for nomenclature and documents.
