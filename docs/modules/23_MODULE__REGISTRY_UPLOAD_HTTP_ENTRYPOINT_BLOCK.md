...
- Во второй tab `Расчёт поставок` current bounded scope materialize-ит два sibling block внутри одного narrow operator page:
  ...
  - per-day district shares are `district_depletion / total_depletion`; average shares are normalized across all 6 districts. A district with clean zero depletion remains present with `0` share/demand; the Far/Siberia merged district is never removed;
  - if the bounded lookup cannot collect the requested valid days, regional calculation falls back explicitly to current-stock-share distribution with `regional_demand_method=current_stock_share_fallback`, `fallback_used=true`, `fallback_reason` and operator-visible warning. Silent partial 7/8/9-day success is not treated as the primary methodology;
  - result and row diagnostics expose `regional_demand_method`, `requested_valid_day_count`, `selected_valid_day_count`, `inspected_day_count`, `initial_window_valid_day_count`, `excluded_day_reason_counts`, used depletion dates, lookup depth, average depletion shares, `total_daily_demand_source=orderCount`, fallback fields and compact UI warning text;
...
- Для regional supply block result contract теперь также server-driven:
  ...
  - compact diagnostics surface under result shows stock-depletion/fallback method, requested/selected valid days, inspected lookup depth and first warning; district XLSX stays compact and does not add service columns;
