...
- Operator page keeps one compact server-driven auto block `Автообновления`:
  ...
  - Policy mode `manual` preserves current behavior: rows expose stable `id`, `enabled`, editable local `HH:mm`, timezone, trigger/action, `next_run_at`, `last_run_at`, `last_success_at`, `last_status`, `last_error_summary` and run-now capability; save may edit business fields but cannot overwrite server-owned lifecycle fields.
  - Policy mode `interval` accepts only `interval_hours` 3/4/6, defaults to 4, fixes `window_start_hhmm=10:00`, `window_end_hhmm=22:00`, `timezone=Asia/Yekaterinburg`, returns preview slots and materializes read-only effective schedule rows from policy. Unsupported intervals, intervals below 3h, duplicate enabled manual times and more than 6 runs/day are rejected.
