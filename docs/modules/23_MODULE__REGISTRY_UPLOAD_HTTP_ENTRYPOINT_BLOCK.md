...
- `Отчёт по остаткам`:
  ...
  - `на произв.` reads the existing supplier shipment registry (`sheet_vitrina_v1_supplier_shipments` + product lines) by active SKU `internal_nm_id` and sums positive matched / matched-by-compatibility product-line `qty` only for order status `production` (`На производстве`); legacy rows without `order_status` are normalized by the current runtime model to `production`
