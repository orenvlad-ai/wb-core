...
- Во второй tab `Расчёт поставок` current bounded scope materialize-ит два sibling block внутри одного narrow operator page:
  ...
  - common ФФ overlay: when selected WB supplies exist, `effective_stock_ff = max(base_stock_ff - selected_wb_supply_qty, 0)` and `over_reserved_qty = max(selected_wb_supply_qty - base_stock_ff, 0)`; without selected WB overlay the source balance, including a negative `ff_stock_ledger` balance, is passed through so the calculation can surface the deficit instead of hiding it;
