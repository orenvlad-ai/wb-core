...
- Hosted EU default producer for this adapter is the localhost-only owner runtime API `http://127.0.0.1:8000`; `SHEET_VITRINA_WEB_SOURCE_SNAPSHOT_BASE_URL` or shared `SHEET_VITRINA_WEBSOURCE_CURRENT_SYNC_API_BASE_URL` may override it only when the owner runtime is intentionally moved.
