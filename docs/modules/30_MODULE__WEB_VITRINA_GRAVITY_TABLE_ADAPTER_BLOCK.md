...
- browser-local metric presentation state on `/sheet-vitrina-v1/vitrina` (scope-local `Итого`/`SKU` metric order, per-metric display status `Показано`/`Свернуто`/`Скрыто`, icon-only disclosure expansion state, plus transient non-persisted bulk selection controls for batch status/order changes) belongs to the page-composition client layer above the adapter payload; it does not change adapter contract, metric registry, formulas or source truth.
- export layer
