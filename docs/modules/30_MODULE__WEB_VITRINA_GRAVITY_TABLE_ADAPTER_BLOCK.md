...
- browser-local metric presentation state on `/sheet-vitrina-v1/vitrina` (scope-local block/group order, selected-metric order, anchor and hidden-under-anchor collapse) belongs to the page-composition client layer above the adapter payload; it does not change adapter contract, metric registry, formulas or source truth.
- export layer
