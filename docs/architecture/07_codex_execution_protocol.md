...
Watcher не конструирует visible report из raw snapshot. Scheduled run публикует только stdout repo-owned `heartbeat-response`: при любом active user-level acceptance envelope он всегда возвращает `decision=NOTIFY`, а message является canonical `report` stdout. Unchanged fingerprint остаётся periodic update с краткой строкой renderer, а не причиной `DONT_NOTIFY`; последний допустим только при отсутствии active blocks и owner-relevant attention/incident. Второго report вне wrapper нет, compaction/rotation правило не меняют. Единица visible report — active user-level acceptance envelope: parent и required corrective children образуют один русский block, progress равен минимальному доказанному member progress без двойного учёта, а ETA/delta/current берутся из текущего critical member. Независимые envelopes остаются отдельными blocks. Task IDs, UUID, revisions, digests, enums и registry/queue/lease/follow-up/batch jargon остаются только в evidence/audit.

Каждый heartbeat читает `progress-state`; fresh executor checkpoint или bounded file/test readback может доказать только ранний floor, а linked PR/check/Release Train state имеет приоритет для поздних этапов. `apply-progress` допускается не больше одного раза на task/run owner, сохраняет same-stage fresh delta/current и не уменьшает процент для обычного test finding. Explicit objective invalidation ограничен одним предыдущим уровнем. `100%` создаёт только contour-aware terminal attention после diagnostic/artifact proof, linked `release:done` либо linked `release:production`; curator acknowledgement и owner acceptance остаются отдельными gates.
...
Ротация Watcher выполняется по generation/run-count из trusted `origin/main`: title+pin нового Luna поколения и exact title/pin/automation readbacks → prepare → до activation smoke registry/thread/outbox contract и канонического русского envelope-report без raw machine state → atomic activate → старое поколение теряет lease и становится no-op → pause старой automation → searchable archive старой Watcher task → registry confirmation обоих readbacks. Missing pin evidence запрещает smoke/activation. В registry остаются поколения и audit. Одновременно активна ровно одна generation и одна Watcher heartbeat automation.
