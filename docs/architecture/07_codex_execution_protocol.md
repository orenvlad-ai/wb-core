...
Ротация Watcher выполняется по generation/run-count из trusted `origin/main`: title+pin нового Luna поколения и exact title/pin/automation readbacks → prepare → smoke mechanical/full escalation, owner reminders, curator lifecycle, registry/thread/outbox и русского report без raw machine state → atomic activate → старое поколение теряет lease и становится no-op → first-successor liveness proof → pause/archive старого Watcher с readbacks. Missing evidence запрещает smoke/activation/retirement. В registry остаются поколения и audit; heartbeat не ставится на паузу между задачами или во время подготовки rotation. Данный change не выполняет G5→G6 cutover: это отдельная задача на объединённом `main`.
