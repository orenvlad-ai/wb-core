...
Одна Luna-задача Watcher имеет единственную 10-минутную heartbeat automation. Per-curator, per-executor, external supervisor reporter и self-heartbeat automations не создаются. Каждый run сначала получает generation-bound lease и уникальный `run_id`, затем repo-owned `heartbeat-plan` фиксирует полный ordered phase list и exact target set. Каждый target получает отдельный immediate `wait_threads` readback и `heartbeat-record-target`; один early wake не покрывает остальные pending targets. Только после полного coverage `heartbeat-actuate` обрабатывает progress/checkpoint, objective evidence, failure/incident и terminal evidence, максимум один meaningful transition на task/run. Active turn только наблюдается; idle non-terminal target получает не более одного bounded follow-up с receipt.
