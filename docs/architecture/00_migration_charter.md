...
- registry upload path уже дошёл до artifact-backed bundle, local validator, file-backed service, DB-backed runtime, live HTTP entrypoint и sheet-side operator trigger: `migration/86`, `migration/87`, `migration/88`, `migration/89`, `migration/90`, `migration/91` вместе с соответствующими модулями уже materialize-ятся в текущей линии.
