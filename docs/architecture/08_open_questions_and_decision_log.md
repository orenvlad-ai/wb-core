...
| Q-03 | Какова полная authoritative schema `METRICS` и живой словарь metric keys? | Открыт | Для текущего `main` current authoritative sheet/upload dictionary уже materialized как `19` metric rows и этот `19`-key set теперь полностью отображается в `DATA_VITRINA`. Открытым остаётся не текущий main-confirmed set, а numeric live fill beyond current `7` public readback metrics и future expansion beyond `19` upload rows. |
