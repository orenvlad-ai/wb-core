...
Post-plan `DISPATCH_REQUEST` создаёт отдельную user-owned Codex task и подтверждает её через `TARGET_CREATE_READBACK`. Та же launch operation формирует versioned Task Passport, pin/title curator и executor, регистрирует exact task/thread/PR resources и user-level acceptance envelope в локальном registry и проверяет одно active generation глобального Watcher. Обычная task создаёт собственный envelope root; corrective child явно присоединяется к незавершённой родительской цели. Per-task heartbeat automation больше не является частью контракта.
...
Ротация Watcher выполняется из trusted `origin/main`: `prepare → smoke exact automation/outbox и русского acceptance-envelope report без raw machine state → atomic activate → old generation no-op → pause old automation → searchable archive old task/readback`. До успешного smoke новое поколение не считается active. Registry/JSONL/dashboard живут локально на Mac; Mac и Codex Desktop должны быть включены. Внешний управляющий сервис, Entire и Telegram не являются v1 dependency. Полный контракт: [`12_codex_global_orchestration.md`](12_codex_global_orchestration.md).
