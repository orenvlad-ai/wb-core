...
Ротация Watcher выполняется из trusted `origin/main`: `title+pin/readbacks → prepare → smoke mechanical/full escalation, owner reminder, event-driven curator, automation/outbox и русского report без raw machine state → atomic activate → old generation no-op → successor liveness → pause/archive old with readbacks → registry retirement confirmation`. Missing title/pin/automation evidence не допускает smoke/activation, а old automation не ставится на паузу до liveness successor. Данный change не выполняет G5→G6 cutover. Full front-door canary после repository release требует normal owner-created fresh chat в ChatGPT project `wb_core_3`; Codex/cloud/fork substitute запрещён, поэтому capability boundary обозначается `FRONT_DOOR_CANARY_READY`, а существующие пары остаются backend/load evidence. Registry/JSONL/dashboard живут локально на Mac; Mac и Codex Desktop должны быть включены. Внешний управляющий сервис, Entire и Telegram не являются v1 dependency. Полный контракт: [`12_codex_global_orchestration.md`](12_codex_global_orchestration.md).
