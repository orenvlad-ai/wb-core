...
Каждый run получает generation-bound lease и unique run identity. `heartbeat-plan` фиксирует ordered phases и exact target set; каждый target покрывается отдельным immediate readback, поэтому ранний wake не завершает coverage остальных. `heartbeat-actuate` запускается только после полного coverage и обрабатывает progress/objective/failure/terminal evidence максимум одним meaningful transition на task/run. Active turns только наблюдаются. Registration является основным acquisition path; fallback-discovery разрешён только для pinned tasks с доказанным project/repository `orenvlad-ai/wb-core`. Сторонние, projectless, личные и медицинские chats исключены.
