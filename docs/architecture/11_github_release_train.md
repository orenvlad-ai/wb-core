...
```text
/wb-core orchestration release-lane <ANCHOR_PR> task <TASK_ID> revision <POSITIVE_TASK_REVISION> outcome <completed|parked> evidence sha256:<EVIDENCE_HASH>
```

Task revision является обязательной частью exact proof и command identity; no-revision форма fail closed, как показал run `30886716812`, а corrected exact revision была принята в run `30886851119`. Повтор той же exact command после release возвращает `already-released` без новых proof comments или label transitions. Parking запрещён при merged ambiguity, `release:running`, `release:awaiting-ui` или `release:halted`. Закрытый/merged legacy backlog из versioned manifest переводится в `release:retired` только `/wb-core orchestration retire-legacy ...` после exact head/merge/manifest/digest proof из trusted `main`; ручное снятие legacy labels запрещено.
...
Post-plan `DISPATCH_REQUEST` создаёт отдельную user-owned Codex task и подтверждает её через `TARGET_CREATE_READBACK`. Та же launch operation формирует versioned Task Passport, title+pin curator и executor, доказывает exact assignment-time pin readbacks, затем регистрирует task/thread/PR resources, evidence и user-level acceptance envelope в локальном registry и проверяет одно active generation глобального Watcher. Missing pin/readback fail closed. Обычная task создаёт собственный envelope root; corrective child явно присоединяется к незавершённой родительской цели, атомарно переоткрывает её и инвалидирует старую owner notification. После evidence curator выдаёт один короткий dispatch summary, завершает turn и остаётся event-driven idle: без executor/GitHub polling и собственного heartbeat; wake sources — user message или exact Watcher attention. Per-task heartbeat automation больше не является частью контракта.

Один Luna Watcher с medium reasoning каждые 10 минут начинает с mechanical preflight: trusted `origin/main`/protocol digest, local registry signals и fresh read-only Release Train snapshot. Fast path разрешён только при exact empty target set и отсутствии attention/incident/failure/lane/watcher-operation/rotation signals; он выполняет `heartbeat-fast-finish` без executor/Watcher thread readbacks. Всё иное, включая classifier error и periodic full scan, fail-safe эскалируется в прежний full target/attention protocol. Измеримый proxy — `9 → 3` explicit model-facing command/readback steps и `1 → 0` thread readbacks, без token claims. Он не хранит release truth в chat history и не подменяет Release Train. Sol используется только incident arbiter. Watcher может выполнять только deterministic registration, bounded retry/replacement, incident arbitration, attention delivery, evidence-proven executor archive, exact orchestration admission/lane release и bounded idle follow-up; merge/deploy/LOOP ack/UI acceptance выполняют существующие repo-owned paths и task owner по exact evidence.
...
Каждый run получает generation-bound lease и unique run identity. Mechanical classification и cost evidence сохраняются durable. Protocol/docs reload происходит при trusted-main/protocol digest change, новом generation либо фактической потребности full action, а не на каждом quiet heartbeat. На `FULL` `heartbeat-plan` принимает exact queue JSON вместе с совпадающим canonical digest, фиксирует ordered phases, exact target set и release-lane closure state; `queue-status` получает `--release-proof-pr <OWNER_PR>` для каждого незавершённого closure row. Каждый target покрывается отдельным immediate readback, поэтому ранний wake не завершает coverage остальных. `heartbeat-actuate` запускается только после полного coverage и обрабатывает progress/objective/failure/terminal evidence максимум одним meaningful transition на task/run. Если GitHub readback показывает terminal anchor с `release:lane-owner`, а registry доказывает task state `DONE_PENDING_HANDOFF`, `DONE_AWAITING_ACCEPTANCE` либо `ACCEPTED`, durable closure outbox создаёт exact positive-revision command. GitHub transport/failure receipt фиксируется `heartbeat-record-release-lane`; следующий queue readback с Actions-owned release proof подтверждает release либо планирует повтор той же command. Одного отсутствия label недостаточно. Три безуспешных попытки открывают один incident вместо дальнейшего blind retry. Active turns только наблюдаются. Registration является основным acquisition path; fallback-discovery разрешён только для pinned tasks с доказанным project/repository `orenvlad-ai/wb-core`. Сторонние, projectless, личные и медицинские chats исключены; curator не является progress target.

Для progress objective truth materialize-ится в local registry через linked PR state: open/draft подтверждает созданный PR, successful CI или `staged/ready/admitted` — release admission, `running/merged` — выполняющийся выпуск, `deployed/awaiting-ui/production` — финальную live-проверку. Эти machine facts сильнее executor checkpoint. Repo-only terminal требует linked `release:done`, live/runtime и LOOP — linked `release:production`; 100% не выводится из merge, elapsed time или одного лишь статуса очереди.

User-visible отчёт не строится из raw snapshot: scheduled Watcher завершает run единственным repo-owned `heartbeat-finish` либо mechanically proven `heartbeat-fast-finish`; full finish fail closed проверяет coverage/follow-up/release-lane/attention receipts, освобождает lease и возвращает canonical wrapper. Wrapper остаётся machine-owned transport receipt, а owner surface показывает только его parsed и repo-validated русский `message`; manual/service/delegated/recovery turn использует plain `watcher-service-response` после сохранения receipts в machine channels. При любом actionable workstream wrapper имеет `decision=NOTIFY`; unchanged fingerprint остаётся обязательным periodic update. После current-revision owner notification этот envelope подавляется до configurable reminder clock (default `240` минут): между reminders `DONT_NOTIFY`, на exact boundary `NOTIFY`; acceptance остаётся доступна. Активность другого workstream не повторяет не-due owner-awaiting block. Renderer даёт отдельный компактный русский block для root и каждого параллельного `required-child` внутри envelope; corrective/replacement generations складываются в соответствующий anchor, а independent envelopes одного curator остаются отдельными. Minimum proven member одной workstream детерминированно задаёт status, progress, ETA/остаток и current action; objective release stage использует repo-owned copy и не сообщает terminal meaning до terminal evidence. Owner acceptance остаётся одна на envelope после завершения всех required workstreams. Raw JSON/XML, IDs/revisions/digests/internal jargon не видны. Поля: `Статус`, `Задача`, `Прогресс · Осталось`, `С прошлого отчёта`, `Сейчас`; `Блокер` появляется только при доказанной strict human-only причине.

Technical completion, terminal failure, strict HumanGate и доказанная серьёзная остановка проходят durable at-least-once attention outbox со стабильным event ID, attempts/lease/timestamps и exact curator identity. Только curator acknowledgement переводит pending-handoff state в terminal/awaiting state. После последнего required ack curator получает digest-bound `prepare-owner-handoff`, показывает exact короткий текст владельцу в commentary, выполняет `confirm-owner-notification` и повторяет тот же текст на final surface; это одна logical notification, а report после неё пишет `Ожидается приёмка владельца.`. User phrase `Задача принята` обрабатывает exact curator для единственного awaiting acceptance envelope current revision; несколько независимых envelopes fail closed. Watcher chat не сканируется для acceptance, pin не снимается.

Curator/current executor pin выполняется один раз при назначении роли; Watcher heartbeat не делает re-pin и сохраняет последующий ручной owner unpin. Они никогда автоматически не archive/unpin. Corrective successor может сделать predecessor inactive legacy только после same-envelope `TARGET_CREATE_READBACK`, prompt delivery, registry link, checkpoint transfer, assignment-time pin и active readback. Затем supported Desktop archive подтверждается searchable exact readback digest в registry. Terminal executor без successor остаётся current; final acceptance не архивирует current curator/current executor.
...
Ротация Watcher выполняется из trusted `origin/main` как durable machine operation: `due REQUIRED → title+pin/readbacks → prepare → smoke mechanical/full escalation, owner reminder, event-driven curator, exact automation/outbox и русского acceptance-envelope report без raw machine state → atomic activate → old generation no-op → первый полный successor heartbeat/liveness → pause old automation → searchable archive old task/readback → registry retirement confirmation`. Повторный overdue heartbeat не завершается без progress/retry/attention receipt; три подтверждённых сбоя Desktop capability/readback materialize-ят `ATTENTION_REQUIRED`. Missing title/pin/automation evidence не допускает smoke/activation. До успешного smoke новое поколение не считается active, а pause/archive старого запрещены до liveness successor. Full front-door canary после repository release требует normal owner-created fresh chat в ChatGPT project `wb_core_3`; Codex/cloud/fork substitute запрещён, поэтому capability boundary обозначается `FRONT_DOOR_CANARY_READY`, а существующие пары остаются backend/load evidence. Registry/JSONL/dashboard живут локально на Mac; Mac и Codex Desktop должны быть включены. Внешний управляющий сервис, Entire и Telegram не являются v1 dependency. Полный контракт: [`12_codex_global_orchestration.md`](12_codex_global_orchestration.md).
