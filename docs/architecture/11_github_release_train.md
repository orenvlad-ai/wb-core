...
User-visible отчёт не строится из raw snapshot: scheduled Watcher завершает run единственным repo-owned `heartbeat-response`. При любом active acceptance envelope wrapper имеет `decision=NOTIFY`, а parsed message равен canonical `report` stdout; unchanged fingerprint остаётся обязательным periodic update. `DONT_NOTIFY` допустим только без active blocks и owner-relevant attention/incident, второго visible report вне wrapper нет. Renderer даёт один компактный русский block на user-level acceptance envelope, агрегирует parent/required corrective members по минимальному доказанному progress без двойного учёта и не раскрывает task IDs, UUID, revisions, digests, enums или registry/queue/lease/follow-up/batch diagnostics. Независимые envelopes дают отдельные blocks. Поля: `Статус`, `Задача`, `Прогресс · Осталось`, `С прошлого отчёта`, `Сейчас`; `Блокер` появляется только при доказанной strict human-only причине. Без meaningful change видимый текст кратко говорит, что работа продолжается.
