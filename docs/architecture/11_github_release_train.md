...
User-visible отчёт не строится из raw snapshot: scheduled Watcher завершает run единственным repo-owned `heartbeat-finish`, который fail closed проверяет coverage/follow-up/release-lane/attention receipts, освобождает lease и возвращает canonical wrapper. При любом active workstream wrapper имеет `decision=NOTIFY`; unchanged fingerprint остаётся обязательным periodic update. `DONT_NOTIFY` допустим только без active blocks и owner-relevant attention/incident. Renderer даёт отдельный компактный русский block для root и каждого параллельного `required-child` внутри envelope; corrective/replacement generations складываются в соответствующий anchor, а independent envelopes одного curator остаются отдельными. Owner acceptance остаётся одна на envelope после завершения всех required workstreams. Raw IDs/revisions/digests/internal jargon не видны. Поля: `Статус`, `Задача`, `Прогресс · Осталось`, `С прошлого отчёта`, `Сейчас`; `Блокер` появляется только при доказанной strict human-only причине.
...
Ротация Watcher выполняется из trusted `origin/main`: `title+pin/readbacks → prepare → smoke exact automation/outbox и русского acceptance-envelope report без raw machine state → atomic activate → old generation no-op → pause old automation → searchable archive old task/readback → registry retirement confirmation`. Missing title/pin/automation evidence не допускает smoke/activation. До успешного smoke новое поколение не считается active. Registry/JSONL/dashboard живут локально на Mac; Mac и Codex Desktop должны быть включены. Внешний управляющий сервис, Entire и Telegram не являются v1 dependency. Полный контракт: [`12_codex_global_orchestration.md`](12_codex_global_orchestration.md).
