...
Baseline is a guarded migration fact, not a continuously refreshed fallback. It must cover 100% of opening owned quantity and normally may use only the discovered certified June supplier shipment or nearest retrospective FF-stage 1C cost `<= 2026-05-16`. The only estimate exception is the business-approved allowlist `{497415593, 497416931}`, whose cost is derived from that exact current primary layer's weighted FF cost and remains zero-confirmation. Future evidence, zero, near-future proxy, hidden carry-forward and any general estimated fallback remain forbidden. Post-apply correction requires a separately fingerprinted audited recalibration.
