...
`Управление SKU → Баланс запасов` не получает собственный token path или WB write URL. Его current apply runtime принимает только durable `dry_run` и гарантирует `wb_patch_called=false`; `live_wb` недоступен и fail closed. Future live adapter boundary может быть открыт только отдельным versioned capability change и обязана делегировать каждый exact `nm_id + advert_id + placement` прежнему SKU Management preview/commit/matching-readback contract. Она не может загружать token в browser, вводить bulk PATCH либо считать ambiguous transport успешным.
