...
- protected `GET|POST /v1/sheet-vitrina-v1/partner-report/...` options/settings/preview/finalize/finalized/package routes
...
- `warehouse-functional-sync` is the privileged CLI manual path and intentionally takes a fresh timestamped restore point immediately before its mutation. The hourly service and UI operator button use the same bounded business pipeline but share one canonical restore point for the open business day; this distinction preserves the stronger fresh-backup contract for an explicit administrative run without making every hourly cycle accumulate another full copy. Capacity uses the coherent SQLite page/WAL bound and actual mount topology: the backup filesystem independently reserves the raw copy plus worst-case archive, while the live-runtime filesystem reserves a bounded SQLite publication margin; when both paths share one filesystem the requirements are summed. This matters on the canonical host where `state/backups` is a separate mount from the live DB, and prevents a misleading free-space reading from either path. The pipeline then runs official supply refresh, supply-specific downstream component materialization, complete official stock capture, one canonical warehouse/cost publication and targeted Proxy publication. A successful run losslessly archives its restore point with verified compressed and decompressed SHA-256; later hourly/operator runs of the same business day revalidate the actual daily archive bytes and reuse that archive. CLI and operator-button runs share one cross-process lock, so the bounded pipeline never overlaps itself; versioned calculation-parameter publication uses the same lock. It does not invoke legacy daily cost/product-capital rebuild or the global vitrina refresh;
...
- `/v1/sheet-vitrina-v1/partner-report/...` is an auth-protected `reports` surface. Settings/finalized truth lives only in the existing runtime SQLite with immutable versions/audit; generated XLSX/ZIP are response-only with bounded request lifetime and no public persistent link. Preview may use arbitrary weeks; finalization requires a continuous period and full Finance/cost/ads coverage. Package response is fail-closed on reconciliation or confidentiality findings and its weekly Finance workbooks use an explicit safe-column allowlist for the selected SKU only.
- Canonical production retro-cost operations use only `finance-retro-dry-run`, `finance-retro-apply` and `finance-retro-readback` commands in `apps/registry_upload_http_entrypoint_hosted_runtime.py`. Apply validates active target/runtime paths, exact reviewed plan/scope/fingerprint and a human approval reference before invoking the deployed Finance runner and bounded backup directory; ad-hoc production SQL or server-only scripts are prohibited.
