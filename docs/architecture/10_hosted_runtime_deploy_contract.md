...
Live/public verify that creates temporary runtime users must use an unmistakable test prefix, run cleanup in a finally-style path, and verify that the admin users list no longer contains those test rows. Archived/inactive test users such as `codex_debug_*` are not acceptable live leftovers; if API delete only deactivates a row, the verify/cleanup step must use an explicit bounded runtime cleanup for that test prefix and must not touch real users.
