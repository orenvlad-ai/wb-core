...
- `warehouse-functional-supplier-certification-dry-run --output <absolute plan.json>` and paired `...-apply --plan-file <same plan.json> --fingerprint <exact sha256:...>` are the bounded recovery path when a legacy active functional version lacks its supplier certification projection. They consume only canonical supplier/CNY/financial sources and require either exact frozen fingerprints or, for an older legacy version, equality of the persisted supplier-shipment, CNY-ledger and financial-document watermarks plus exact per-SKU quantity/capital and contributing payment, CNY-fee and China→FF document identities already frozen in immutable balance provenance. The version-wide local digest is retained as audit evidence but is not this gate because it also covers independently advancing WB/FF/history projections. The runner pins the exact active version and non-target digest, preflights disk space, retains a coherent `0600` backup under `/opt/wb-core-runtime/backups/warehouse-supplier-certification-replay`, and appends ordered version-scoped corrections with `supersedes` provenance without rewriting the immutable version. Changed or unprovable supplier source state fails closed and must be handled by a newly calculated functional version; exact repeat is a no-op. `warehouse-functional-supplier-certification-rollback --fingerprint <replay sha256> --reason <audit reason>` appends a rollback tombstone and preserves the replay audit;
