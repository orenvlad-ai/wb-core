...
- when a large JSON response exceeds the bounded probe excerpt, top-level key presence may use a synthetic sentinel, but nested identity checks must use explicit bounded excerpt evidence and type guards. The FF detail probe therefore verifies the top-level `warehouse` object's `warehouse_key=ff` without calling mapping operations on a boolean sentinel; absent proof fails closed instead of crashing deploy verification;
