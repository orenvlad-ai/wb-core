...
- `GET/POST/PATCH/DELETE /v1/sheet-vitrina-v1/settings/users...` is an admin-only runtime user admin surface. It must never return plaintext passwords or password hashes, must reject duplicate usernames against env/runtime principals and must not allow the final admin path to be removed.
