...
- Migration 138 deploys only an unused query-only cutover planner/status CLI and
  ...
  protocol-v2 Apply Runner operation with an exact manifest on trusted/deployed main.
...
- Migration 140 adds a separately owner-gated production-apply runner for only
  the two facility rows and the
...
- The separate `wb-core-fbs-warehouse-registry.timer` runs every 15 minutes and
  reads official seller warehouses/offices plus read-only FBS stock snapshots.
  Its append-only registry/readback is not part of the order collector's
  success path and never writes WB, inventory, movement, quantity, capital or
...
- Migration 144 adds
  ...
  canonical runtime and exact `.wb-core-runtime-sha`; plan/readback are
  query-only. Apply accepts only the external reviewed plan, exact proof
  fingerprint, owner authorization reference and actor, then re-derives the
  complete later-cutover proof under the warehouse writer lock. Its only DB
  effects are one immutable Recovery Policy relation, one CAS lifecycle update
  to `superseded` and one transition row. Old checkpoint/manifest bytes and
  failure evidence remain preserved; opening, historical debit, shipment
...
- `ff-pool-overhead-backfill-dry-run|apply|readback` is the only bounded production path for the five `2026-08-21` facility/FBS overhead documents agreed in migration 152. Dry-run is query-only and resolves exact server-owned document, operation, facility, pool, SKU and event-revision identities; the prompt totals are eligibility constraints, never apply identities. The external plan binds exact deployed SHA, pre-change digests, coherent backup, current-only aggregate projection, quantity/capital/document/lifecycle/non-target invariants and deterministic queue revisions. Exact Decimal comparison uses the canonical `ROUND_HALF_UP` kopeck boundary while retaining raw tails in the manifest. It explicitly distinguishes selected capital still pending from already-current aggregate capital. Apply requires the separately reviewed fingerprint and exact post-merge authorization, backs up coherently and writes only rows still behind; when facility detail is already numerically reflected, aggregate update count and capital rewrite are zero and the runner reconciles only missing canonical queue/status/economics/Finance identities. Completed stages are not rerun. It never re-posts a document, changes historical fulfilled handoff cost or uses ad-hoc SQL/server-only code; a fresh repeat dry-run/readback is a proven zero-write no-op;
...
- `finance-daily-parity --target-date <24|25.08>` is query-only exact 171-cell
  reconciliation against a fresh terminal-204 POST source.
  `finance-daily-recovery-plan|apply|readback` is the only hosted historical
  Finance publication path for exact 26–27.08. The wrapper pins the active EU
  target, canonical runtime/environment and `.wb-core-runtime-sha`, rejects
  plans/evidence inside Git, writes local plan mode `0600` and streams the exact
  reviewed normalized plan over stdin. One-date plan pins 33/33 source,
  pages/final cursor/digest, 171 cell identities and before states, ready-plan
  CAS and non-target digest. Apply refetches nothing and atomically changes only
  the exact Finance data slice, existing temporal accepted/closure seams and its
  STATUS row; raw Finance rows are never copied. Query-only readback proves
  171/171, totals, no duplicates, terminal completion, closure/overall status,
  Proxy-gap exclusion and non-target preservation. Deploy invokes none of these
  actions;
...
- `warehouse-functional-sync` is the privileged CLI manual path; the hourly service and UI operator button use the same business pipeline and retain the official hourly source capture. Manual/hourly publication selects T2 and takes a fresh warehouse/cost domain checkpoint instead of the monolithic store. Operator-authored calculation parameters and dependent economics select T1 and persist exact parameter/ready-snapshot before images. Central reservations use measured domain or serialized undo bytes plus operational reserve, with post-write watermark and lifecycle-aware retention. The pipeline then runs official supply refresh, the bounded process-owned global due transit-cost collector, supply-specific downstream component materialization, complete official stock capture, one canonical warehouse/cost publication and targeted Proxy publication. Transit collection is all-eligible rather than UI-page-scoped, joins an active single-flight run, applies durable classified attempt/backoff state, preserves last successful amounts on a later error and enqueues only confirmed changes for canonical recalculation. It is part of hourly/manual sync, not the reviewed `sync-apply` optimistic-plan contour. A separate `.warehouse-functional-job.lock` serializes top-level hourly/manual work without owning the common writer lock. Heavy capture, plan, digest, economics and Finance phases therefore run outside `.warehouse-functional-sync.lock`; each actual canonical write retains its own short optimistic writer/CAS boundary and exact readback. Finance plans and exact canonical WB/FBS/nomenclature/raw/report dependency fingerprints on a query-only connection, normalizes target after-images by SQLite affinity and persisted primary key, then uses only a per-database observer handoff, indexed target-before CAS, scoped replacement and exact readback under `BEGIN IMMEDIATE`. Unrelated status/UI commits during long planning remain admissible, while actual dependency, handoff or target drift aborts before replacement. Interactive FF document/status paths do not wait for the multi-minute job scope. The warehouse sync process alone raises the DB-backed runtime's bounded SQLite busy wait to 120 seconds. Every hourly/manual phase, job-lock wait and short writer section is timed; exhausted contention or CAS drift leaves the last-good version active. The hourly oneshot has `TimeoutStartSec=3h`. It does not invoke legacy daily cost/product-capital rebuild or global vitrina refresh;
- functional economics full plan/source/snapshot revalidation runs without a SQLite transaction; its exact-date product-capital lookup is built before the shared WB+FF inventory-cost blend, and that one versioned/freshness/coverage fingerprint supplies visible cost plus Proxy 3/4. TOTAL cost is the aggregate evidence capital/quantity ratio and Proxy totals use eligible SKU sums/denominators. Immediately after one bounded `BEGIN IMMEDIATE`, the same writer connection re-reads the exact warehouse input manifest with component digests for active functional identity, relevant versions/balances/cost states, supplier/CNY/financial source rows, daily WB cost and both settings families, plus the complete ready-snapshot manifest and every target `plan_json` before-hash. Only then may per-row optimistic `plan_json` CAS, exact readback and the undo manifest run. `PRAGMA data_version` remains structured observer telemetry and is never a semantic gate: unrelated same-operational-store FBS shadow observation/transition/poll/lifecycle commits are admissible, while any named material-component or target-row drift aborts before publication with component-specific evidence. Atomic publication, recovery before-images, repeat no-op and last-good safety remain mandatory;
- New T2 checkpoints resolve from canonical `REGISTRY_UPLOAD_RUNTIME_DIR` to `state/backups/warehouse-recovery/domain-checkpoints`; production mount identity, not the pathname alone, proves routing to the dedicated backup device. The writer lock runs automatic retention before and after backup-only/hourly/manual/reviewed sync. It protects the two newest verified rollback points, retains at most three successful T2 generations, bounds the optional third by a 2 GiB steady-state cap, releases eligible successful points after 24 hours and exposes 24-hour/14-day no-GC projections plus 8 GiB degraded / 4 GiB hard-stop watermarks. Hosted `warehouse-recovery-retention-dry-run|apply` pins the exact deployed SHA and stable plan fingerprint; failed, quarantined, superseded, corrupt, incomplete, current and foreign artifacts are never ordinary candidates;
- `warehouse-functional-maintenance status|hold|restore` remains the bounded legacy maintenance boundary for explicitly allowlisted flows that actually require the hourly warehouse timer to stop. It records the exact timer/service/last-next-trigger/shared-lock/Finance-process baseline in mode `0600`, waits for an already-running service without killing it and restores the exact prior intent. The fixed-cutoff schema-v3 canonical Finance apply does not use this boundary: it does not stop timers, hold `.warehouse-functional-sync.lock`, take a coherent whole-store backup or checkpoint the warehouse domain. Its heavy plan/projection and reconciliation are query-only around one short target-only CAS with T1 Finance-row before-images. Arbitrary `systemctl`/SSH holds and a second lock are not supported;
- `business-data-maintenance status|hold|restore|set-process|barrier-*` is the canonical cross-writer quiet-window boundary and the backend for Settings `Автообновления`. `status`, `barrier-status` and Settings GET are strictly read-only. Owner-policy v2 contains individual desired state only for Settings-owned processes: Web-vitrina refresh, its separate closure retry, warehouse/cost and Finance. Autoanswers and auto-complaints are feature-owned monitoring-only processes; SPP is no longer scheduled or represented as an auto-update process. `hold` disables Settings-owned tickers, leaves canonical schedule JSON unchanged, suspends actual auto-complaints and invokes the dedicated Autoanswers lifecycle. It also inventories the manual SPP execution lock/current job as a cross-writer safety boundary without owning an SPP timer. The runner inventories every installed `wb-core-*.timer`, paired service, matching cron, known writer and shared lock; unknown or active unbounded writers fail closed, while already-running oneshots are waited rather than killed. Mode-`0600` maintenance/audit retains exact pre-hold and final readback evidence plus a stable control signature. Repeating `prepare` from an already `prepared` or `held` quiet boundary is an audited no-op only when revision/fingerprint and every current control intent still match exactly. `restore --expected-revision <N>` requires exact revision and quiet/no-writer/no-lock/no-drift proof. The durable HTTP barrier is acquired before writer drain; while active, authenticated business mutations return exact audited `423`. The browser disables mutations only after a valid successful `active=true` readback, renders ordinary maintenance as a warning and invalid fail-closed state as danger, preserves the last confirmed state across status timeout/error, and polls single-flight with bounded timeout/backoff plus hidden-tab throttling. The browser guard never rewrites application-owned native `disabled` state; server-side `423` remains authoritative even before the first confirmed client status. Registry HTTP, archived Data MCP compatibility, PR Gate, one-shot Release Runner and deploy/verification infrastructure are not stopped. The deploy manifest never enables or restarts business timers;
- A process restart after core `prepare` but before the nested warehouse hold
  may continue only that same durable `prepared` revision. The hosted `hold`
...
- The same inventory explicitly pauses and restores the exact pre-hold
  ...
  and the maintenance runner never toggles it. This is an exact unit list, not
  a prefix or blanket allowlist; every other newly installed `wb-core-*.timer`
  remains unknown and fail-closed. An acquiring barrier with no started hold
  uses the same complete classification to prove unchanged maintenance
  state/audit and release through the audited `acquire_aborted` path;
...
- `warehouse-functional-enable-hourly` enables the timer only after successful cutover/readback. Each hourly/manual functional sync finishes supply-layer, functional-version and functional-economics publication, then recalculates stale Finance weeks from an immutable/query snapshot outside the common warehouse writer lock. Finance apply uses a short `PRAGMA data_version` CAS and publishes its fingerprint into the originating targeted queue only after exact post-verify and non-target readback; source drift aborts without mixed publication;
...
- `artifacts/registry_upload_http_entrypoint/systemd/wb-core-root-storage-policy.service`
...
Canonical deploy adapter работает от current checked-out worktree, но one-shot
Release Runner вызывает его только после squash merge и checkout exact merge
SHA. Deploy из незамёрженной branch/PR запрещён.
...
Repo-owned [one-shot Release Runner](11_github_release_train.md) принимает один
successful exact-head pull-request `pr-gate` plan. Он один раз проверяет open
non-draft same-repository PR в `main`, provenance, plan hash, mergeability и
expected head SHA; затем squash-merges exact head и читает exact merge SHA.
Очереди, schedule/poll loop, release labels и blind resubmit отсутствуют.

Для `live_runtime` и deploy-части `production_mutation` Runner checkout-ит
только trusted exact merge SHA и вызывает единственный canonical adapter:

`python3 apps/registry_upload_http_entrypoint_hosted_runtime.py deploy-and-verify`

Immutable Actions receipt связывает exact PR/head/merge=deployed SHA, canonical
target `wb_core_eu_hosted_runtime_active`, service
`wb-core-registry-http.service` и digests deploy/probe/readback evidence.
Transport ambiguity допускает только bounded query/readback reconciliation;
file sync, dependency install, merge и production apply не повторяются вслепую.
`repo_only` deploy не вызывает. `production_mutation` после exact deploy только
читает manifest query-only и завершает release receipt в `awaiting_apply`;
default-off Apply Runner требует отдельное exact OWNER/MEMBER authorization,
один apply, readback и reconciliation. GitHub Environment secrets остаются вне
Git/logs.
...
- The embedded supply probe also requires the `Счёт CNY` / `Конвертации RUB → CNY` write/delete wiring, including the existing CNY documents route token, truthful saved/pending/error presentation and the explicit balance/rate/ledger replay warning. Runtime UI verification must confirm that `200` and durable `202` both trigger an immediate read-model refresh, `202` remains yellow with a cleared file input, ambiguous transport is reconciled by SHA/readback, eligible conversion rows expose `Удалить`, source-owned rows do not expose an active direct delete, and no real production document is uploaded, replayed or deleted during ordinary deploy verification.
...
- The date-bounded `web-vitrina-closed-day-2026-08-22-v1` experiment is executed by the existing `wb-core-sheet-vitrina-refresh.timer`, without changing runtime-managed user schedules. Its only due slots are `2026-08-23T01:30:00+05:00`, `03:30`, `06:30`, and `08:30`, with `target_date=2026-08-22`; it invokes the canonical publishing `auto_refresh`, retains retryable busy slots for at most 50 minutes, and cannot replay after the final window. Each accepted slot has an append-only pre-trigger receipt/exclusive claim and an immutable `state/experiments/web-vitrina-closed-day-2026-08-22-v1/<slot>.json` terminal archive containing job semantics, exact-date Web Vitrina contract/source status and stable fingerprints. Four terminal slot artifacts produce one immutable `comparison.json`. `apps/sheet_vitrina_v1_auto_refresh_tick.py --experiment-status` is the read-only manifest/artifact-state probe; it must report four exact pending slots before the first due without starting a source call.
- The four immutable `web-vitrina-closed-day-2026-08-22-v1` slot artifacts remain append-only historical evidence even when an observation failed. A typed retryable `503 wb_core_sqlite_contention_v1 / sqlite_write_busy` from job polling is retried through `retry_after_ms`/`Retry-After` plus bounded jitter/backoff; bounded `429/502/503/504` and transport timeouts are also retryable, while auth/permission/contract errors fail closed. A retryable polling window cannot create a terminal slot artifact before the exact slot deadline.
- A corrective one-slot control run is armed only after exact-SHA deploy through the deployed `sheet_vitrina_v1_auto_refresh_tick.py --arm-control-canary` command. Its immutable manifest uses a new `web-vitrina-closed-day-2026-08-22-canary-*` id, fixed `target_date=2026-08-22`, an absolute due time at least ten minutes after arming, a deadline no more than fifty minutes later and at most three accepted attempts. The same `wb-core-sheet-vitrina-refresh.timer` owns the trigger and the same canonical async `auto_refresh`/job path owns each attempt; the four former night slots are neither re-armed nor synthesized by this contour.
- Before a control attempt, only manifest-listed repo-owned timers whose paired services are currently idle may be stopped. The supported set is closed to the operational-SQLite Web Vitrina closure retry, WB Finance tick, warehouse functional sync and FBS shadow collector. Feedback auto-complaints uses its runtime JSON/journal contour and Autoanswers uses the separate `wb_autoanswers_runtime.sqlite3`, so those unrelated timers are not pause-eligible. The wrapper never stops a service or active job, never disables a timer and never changes runtime-managed user schedule content. It writes an immutable exact desired/actual before-state, unit-definition digest, paired-service idle readback, hard restore deadline and owner process/boot identity before the first `systemctl stop`.
- The wrapper restores every selected timer to its exact prior active/enabled state in `finally` and records actual/next-run readback. The independently enabled `wb-core-sheet-vitrina-canary-restore.timer` checks durable intents every minute and restores immediately after owner-process/boot loss or at the 25-minute hard pause expiry, including after host restart. Desired enablement or unit-definition drift fails closed; unrelated services and the owning refresh timer are never paused.
- A control artifact is written once only after an observed job terminal state or the exact canary deadline. A terminal `sqlite_contention_exhausted` may start a later bounded attempt only after backoff and the backend single-flight barrier has atomically observed no active refresh; every trigger, accepted job and terminal result remains append-only. Technical success may carry an explicit semantic warning, but acceptance additionally requires an exact-date ready-snapshot refreshed inside the canary window, non-empty contract/source/count/diagnostic evidence, matching immediate fresh-read fingerprint, bounded non-duplicate attempt lineage and complete timer restoration. The one terminal artifact produces one immutable one-slot `comparison.json` and can never replay.
- The owner-authorized `web-vitrina-closed-day-2026-08-23-night-*` plan is a date-bounded reuse of that released pause/restore contour, not a new acquisition path. Its immutable parent `plan.json` binds one exact deployed SHA, `target_date=2026-08-23`, timezone `Asia/Yekaterinburg`, exactly four child slots `2026-08-24T01:30:00+05:00`, `03:30`, `06:30`, `08:30`, each with a 50-minute deadline, the full four-unit SQLite conflict allowlist, automatic terminal expiry and no-next-day replay. Every child manifest is digest-bound to the parent; each slot independently requires idle paired services, writes its own before-state/attempt/job/raw canonical/source/fingerprint/restore receipts and runs only through `wb-core-sheet-vitrina-refresh.timer` plus canonical `auto_refresh`.
- Immediately before each slot pause, the runner reads the canonical business-data write barrier. Active or ambiguous FBS/other production-mutation state causes zero-action bounded waiting until a later normal timer tick inside the same deadline; it never launches, pauses, kills or overlaps. An untouched pre-due plan may be rebound after deployed-SHA drift only by an immutable supersede receipt plus a replacement parent with the same four exact slots and zero prior attempt/pause/artifact evidence. Superseded child manifests are permanently excluded from selection. At or after first due, rebinding is forbidden and SHA drift fails closed before pause/source action.
- The known fingerprint defect remains intentionally unchanged: raw immediate/fresh fingerprints still include `meta.generated_at` and `status_summary.business_now`, and immutable prior artifacts are not rewritten. A child whose recursive diff is exactly those two paths and whose only failed acceptance check is that raw fingerprint comparison remains formally failed but exits the owning tick successfully after technical success, allowed semantic status, exact-date ready snapshot, non-empty raw evidence and complete timer restoration; later authorized slots continue. Any additional data-bearing diff or other source, job, evidence or restore failure remains an error. The four-child terminal parent comparison preserves raw hashes and instructs morning read-only analysis to exclude only those two volatile fields, verify per-source freshness/fallback first, treat closed/financial metrics as stability candidates and state metrics as timestamped observations rather than immutable truth.
...
- Canonical production Finance recalculation uses only `finance-canonical-dry-run`, `finance-canonical-apply` and `finance-canonical-readback` in `apps/registry_upload_http_entrypoint_hosted_runtime.py`. Dry-run uses the fixed historical cutoff `C = 2026-08-23` after proving that canonical raw Finance has reached that fully closed week, excludes forward ingress from `2026-08-24`, streams ordered raw/non-target identities into deterministic digests and publishes exact pooled-FBS/common-inventory/remaining order-unit-RUB coverage plus a target-only before-image. Ordinary `recalculate-stale-cost` defaults to the forward boundary; only a repo-owned contour that passes an explicit earlier bounded `date_from` may consider historical weeks. Deploy, operator refresh, nomenclature edits and ordinary canonical-cost refresh therefore cannot apply or reverse the historical FBS plan. Global capitalization allocations, canonical daily costs, active-facility FBS physical state, same-day common inventory cells, the active archival overlay/first-factual boundary, nomenclature and resolved SKU/date evidence are cached only for that query-only connection and rebuilt on a later connection/source state. Exact target before-images and target-reachable source dependencies are re-read after long planning, so an internally mixed plan fails closed without using a global timestamp or max-sequence fingerprint. The scale regression covers 295,919 sale rows across certified/archival/missing states plus 50,000 functional events and 50,000 supply cost layers while enforcing its fixture-specific 512 MiB RSS ceiling; production evidence separately records actual duration and peak working set. Apply validates the active target/runtime paths, schema-v3 plan, exact newly reviewed fingerprint, fixed date bounds and separate owner approval reference. It captures T1 before-images only for target Finance rows; there is no full SQLite copy, T2 warehouse checkpoint, shared warehouse lock or timer hold. Heavy planning and after-image assembly precede one short target CAS, and that same transaction stores the immutable exact-fingerprint apply identity before commit; post-commit zero-delta reconciliation remains query-only. Durable server-owned operation identity/status/result is authoritative after transport ambiguity and the operation is never resubmitted. The superseded `finance-retro-*`/`business-approved-backfill` path is unavailable; ad-hoc production SQL or server-only scripts are prohibited.
...
- Proxy V4 initialization is a two-runner exact-SHA production mutation, never a deploy side effect. `apps/sheet_vitrina_v1_buyout_mature_backfill.py` may official-refetch and replace only `2026-07-06..2026-07-12` after strict `33 × 7` `DETAIL_HISTORY_REPORT` coverage. Only after that reconciliation may `apps/sheet_vitrina_v1_proxy_v4_initialize.py` create the two reviewed exact-3-of-3 as-of revisions (`2026-08-01` from `2026-07-06..2026-07-26`, `2026-08-08` from `2026-07-13..2026-08-02`) and update V4 rows in exact ready snapshots from `2026-08-01`; its manifest records each contributing week plus Buyout `orderCount` and Finance `net_revenue` denominators. Missing `2026-08-03..2026-08-09` does not block the already as-of-supported `2026-08-01..2026-08-09` rows. Both runners default to dry-run, require private external manifests, exact deployed SHA and human approval reference, hold the shared warehouse writer lock for apply, create a verified coherent backup, preserve V3/non-target digests, and require idempotent readback/reconciliation evidence. Neither runner changes V3 formula/history, Finance raw, canonical WB WAC or any pre-boundary public value. Ordinary post-initialization rollover inspects the same current three closed slots but persists rates from exactly the freshest one common READY COMPLETE Buyout∩Finance week; the separate one-to-three-week Settings combined remains analytics only. It keeps the last version when none are common or the selected fingerprint has not advanced. Ordinary full refresh must freeze every previously published historical V4 SKU/TOTAL cell (including a proven blank) before the current business date; only the current date may take a newer effective version, including one same-day higher revision for the legacy-combined-to-latest-week transition. If a pre-fix refresh has already drifted this history, `apps/sheet_vitrina_v1_proxy_v4_reconcile.py` is the sole repair path: its dry-run requires the exact reviewed initialization source manifest and bounded closed-date window; apply additionally requires its own reviewed manifest SHA, exact deployed SHA and human approval reference, shared-writer lock, verified coherent backup, compare-and-swap, V3/V4-version/non-target invariants, post-apply readback and idempotent repeat. Deploy and ordinary refresh never invoke this reconciliation implicitly.
...
deploy or UI acceptance. It requires a separate exact protocol-v2 production
