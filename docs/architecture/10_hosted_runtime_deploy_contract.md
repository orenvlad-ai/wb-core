...
Production failure после merge ставит global `release:halted` и блокирует выбор следующего release. SSH exit `255` не считается доказанным deploy failure: это `transport-indeterminate`, после которого bounded reconciler читает exact metadata/runtime SHA, metadata bytes digest/schema/deployed timestamp, canonical EnvironmentFile key-presence без вывода values, systemd state/MainPID и mandatory status/operator probes. Healthy exact SHA продолжает transition; wrong/mixed SHA, invalid auth env, inactive systemd и failed probes сохраняют halted. Разрешены только safe retries `daemon-reload/restart/probes/readback`; rsync и dependencies не повторяются. Единственное исключение для metadata — explicit `--safe-finalize-incomplete`, вызванный exact deploy-stage recovery либо отдельно через trusted exact-SHA runner: когда metadata SHA = runtime SHA = exact merge, `deployment_complete=false`, schema/metadata/runtime byte digests, active MainPID, auth и probes совпадают с immutable plan, runner выполняет один CAS только completion bit, fsync/atomic replace и query-only post-readback. Он не делает rsync, install или restart; disconnect после CAS reconciles только readback, а drift/repeat never submits a second CAS. После такого finalize обычный repo-owned `resume-halted` всё равно отдельно читает completed evidence и снимает label только по exact PR/head/merge/canonical-target binding. `scope:repo-only` не вызывает deploy. `scope:production-mutation` автоматически не выполняется. GitHub Environment secrets остаются вне Git/logs.
