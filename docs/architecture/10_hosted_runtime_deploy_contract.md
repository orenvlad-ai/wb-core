...
- `apps/warehouse_cost_unified_recovery.py` is the only reviewed bounded production-data recovery for the coupled supplier-date, approved bank-fee, explicit WB-supply physical-debit and optional unique whole-box scope defined by migration 114. It is query-only by default, excludes Finance raw rows and any full-database copy/integrity/SHA pass, requires the exact current fingerprint for apply, holds the shared warehouse lock, journals idempotent target-scoped steps before mutation, validates exact source revisions on resume and requires a second no-op readback. Its economics checkpoint carries the same affected nmID closure plus earliest business date, can update only those SKU/date cells and their direct totals, and stores an exact non-target digest plus snapshot before-images. The former 26GN527 and transit/reservation apply entrypoints are disabled;
