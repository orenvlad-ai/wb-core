...
- `warehouse-functional-enable-hourly` enables the timer only after successful cutover/readback. Each hourly/manual functional sync materializes supply-cost layers and, before releasing the shared warehouse write lock, atomically recalculates every Finance week made stale by those layers; the sync result carries exact Finance post-verify and non-target-preservation evidence;
