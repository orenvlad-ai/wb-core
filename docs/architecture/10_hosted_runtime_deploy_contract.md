...
- `warehouse-functional-backup` creates a coherent `0600` SQLite backup under the canonical active target without changing business or derived rows; the controlled `warehouse-functional-sync` additionally creates that backup before its first mutation while holding the same process lock used by hourly sync, so no hourly/manual/backup run can overlap; the shared backup gate verifies free space, `integrity_check=ok` and SHA-256;
