...
- Finance storage pre-cutover uses only `finance-storage-split-dry-run`, `finance-storage-split-health` and the later-gated `finance-storage-split-apply`. Dry-run pins `.wb-core-runtime-sha`, opens the canonical monolith `mode=ro` with `PRAGMA query_only=ON`, creates no destination bytes and writes private evidence only on the controlling host. It emits the complete table-owner/read/write matrix and direct-open inventory, coherent logical digests/chunks/watermarks, actual query plans, `sqlite_dbstat` allocation, capacity reserve, exact writers/timers, target generations, non-target invariants, rollback scope and one stable plan fingerprint. Health is read-only. Apply is not invoked by deploy: it requires the reviewed external plan/fingerprint/approval, private active `business-data-maintenance` hold, warehouse writer lock and a fresh capacity recheck before any generation directory. It may build only an unselected shadow candidate and never writes the global generation manifest, changes canonical readers/writers, runs live-tail apply, retires the monolith or performs in-place delete/vacuum;
