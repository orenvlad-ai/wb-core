...
- `GET /sheet-vitrina-v1/vitrina` returns `200` + `text/html` as a real operator-grade web-vitrina page shell: page must contain `Web-витрина`, `Операторский сайт`, primary `Загрузить и обновить`, top Seller session indicator rendered as compact dot + lowercase `сессия`, top-level tab `Отзывы`, block `Автообновления`, canonical JSON route token `/v1/sheet-vitrina-v1/web-vitrina`, auto-schedules route token `/v1/sheet-vitrina-v1/web-vitrina/auto-schedules`, feedbacks route token `/v1/sheet-vitrina-v1/feedbacks`, explicit `surface=page_composition` wiring, bottom `Действия и состояния`, grouped date-scoped `Обновить группу` controls and Seller Portal recovery control. The web-vitrina recovery action is one-click: `Восстановить сессию` starts recovery and launcher download is attempted automatically after `awaiting_login`, with a fallback link only after readiness/auto-download attempt. `JSON Connect`, the old cheap top-panel `Обновить` button and the permanent top status badge are not rendered. Page open may call only lightweight session-check/status reads; it must not trigger hidden full refresh or hidden heavy group/source fetch.
...
- `GET /v1/sheet-vitrina-v1/plan-report` returns `200` + JSON for valid primary query params `period`, `h1_buyout_plan_rub`, `h2_buyout_plan_rub`, `plan_drr_pct`, optional `as_of_date` and optional boolean `annual_plan_evenly_distributed`; legacy complete `q1_buyout_plan_rub`..`q4_buyout_plan_rub` may be accepted only as transitional fallback:
  ...
  - with `annual_plan_evenly_distributed=true`, plan values use `h1_buyout_plan_rub + h2_buyout_plan_rub` evenly across calendar days of the year or contract-start calculation window, without mutating persisted H1/H2 inputs;
  - optional `use_contract_start_date=true&contract_start_date=YYYY-MM-DD` trims selected/MTD/QTD/YTD windows before facts, plan and coverage are calculated; annual-even mode then uses the remaining annual calculation window from contract start through year end;
