...
- `warehouse-functional-backup` creates a coherent `0600` SQLite backup under the canonical active target without changing business or derived rows; the controlled manual sync creates that backup before its first mutation, while hourly/operator sync prepares the business-day restore point described below. All paths hold the same process lock, so no hourly/manual/backup run can overlap; the shared backup gate verifies free space, `integrity_check=ok` and SHA-256;
...
- Hosted `warehouse-functional-sync` backup-only, manual and reviewed sync-apply actions derive their backup directory from canonical `REGISTRY_UPLOAD_RUNTIME_DIR` as `state/backups/warehouse-functional-sync`; they must not fall back to the sibling live-runtime filesystem path `/opt/wb-core-runtime/backups`.
- `warehouse-functional-emergency-dry-run --output <absolute plan.json>` and `warehouse-functional-emergency-apply --plan-file <same plan.json> --fingerprint <exact sha256:...>` are the only post-cutover full-rebuild/recovery path. The public UI is preview-only because a full backup/apply can exceed the managed proxy timeout; its synchronous apply endpoint fails closed. Ordinary rebuild uses persisted functional sources and the immutable historical boundary. Only an actual missing whole pre-cutover date admits a pinned persisted ready-snapshot manifest with an exact dated `stock_total` column; complete frozen history excludes mutable snapshots from the source digest. The correction-only loader admits a later persisted bundle when its outer `as_of_date` is post-cutover but its `date_columns` contains the missing exact date; ordinary hourly replay remains cutover-bounded. For each corrected date the newest single coherent snapshot column is authoritative: its SKU universe must equal the union declared by every persisted candidate for that exact date, every `stock_total` cell must be valid, and the projected SKU identity set must match it exactly, without per-SKU stitching from older snapshot versions. A zero-stock SKU absent from the frozen opening remains an explicit zero-capital row with `zero_quantity_without_cost_basis`; canonical consumers expose no invented unit cost, while a later positive quantity without a cost seed still fails closed. Frozen quantities, not a mutable snapshot window, drive the overlap replay; exact snapshot columns enter only the missing dates. Drift and manifest pin the selected date/SKU/quantity evidence and its source identity, not unrelated ready-snapshot rows or metadata, so legacy-metric cleanup cannot invalidate unchanged recovery arithmetic. Before backup and again under `BEGIN IMMEDIATE`, apply re-derives the complete correction contract and rows from current frozen opening, current persisted source evidence and exact columns and requires byte-semantic equality with the reviewed plan. It removes its own backup if the transaction never commits, and otherwise retains a fresh coherent `0600` backup under `/opt/wb-core-runtime/backups/warehouse-functional-recovery`; only the uniquely derived missing identities enter plain `INSERT` plus a versioned `supersedes` audit, and an exact repeat is a no-op;
- `warehouse-functional-economics-dry-run --output <absolute plan.json>` and `warehouse-functional-economics-apply --plan-file <same plan.json> --fingerprint <exact sha256:...>` are the only bounded `2026-07-01+` ready-snapshot WB cost/Proxy 3 publication path; apply preserves a separate `0600` backup, byte-preserves snapshots without target dates or archived metrics, and proves non-target digest invariance and idempotent readback;
- `warehouse-functional-supplier-certification-dry-run --output <absolute plan.json>` and paired `...-apply --plan-file <same plan.json> --fingerprint <exact sha256:...>` are the bounded recovery path when a legacy active functional version lacks its supplier certification projection. They consume only canonical supplier/CNY/financial sources and require either exact frozen fingerprints or, for an older legacy version, exact target-scoped conservation of every immutable per-SKU quantity/capital plus contributing payment, CNY-fee and China→FF document identity. Every mutable shipment/payment/CNY-document/financial-document row contributing to that target must also have a server-owned revision timestamp strictly earlier than the immutable version; equal whole-second timestamps are ambiguous and fail closed. Version-wide source-group watermarks and the broader local digest remain audit diagnostics, not eligibility gates, because unrelated informational-document and WB/FF/history projections legitimately advance them. The exact complete current supplier source manifest is nevertheless pinned in the reviewed plan and rechecked under the apply transaction, so concurrent drift fails closed. The runner pins the exact active version and non-target digest, preflights disk space, retains a coherent `0600` backup under `/opt/wb-core-runtime/backups/warehouse-supplier-certification-replay`, and appends ordered version-scoped corrections with `supersedes` provenance without rewriting the immutable version. Changed target allocation/revision or unprovable supplier source state fails closed and must be handled by a newly calculated functional version; exact repeat is a no-op. `warehouse-functional-supplier-certification-rollback --fingerprint <replay sha256> --reason <audit reason>` appends a rollback tombstone and preserves the replay audit;
- `warehouse-functional-sync` is the privileged CLI manual path and intentionally takes a fresh timestamped restore point immediately before its mutation. The hourly service and UI operator button use the same bounded business pipeline but share one canonical restore point for the open business day; operator-authored calculation-parameter versions always take their own fresh pre-save restore point because that history is not reconstructable from upstream sources, and persist the exact preview fingerprint in a private fsynced raw sidecar before the settings transaction. This distinction preserves the stronger fresh-backup contract for administrative writes without making every hourly cycle accumulate another full copy. Capacity uses the coherent SQLite page/WAL bound and actual mount topology: the backup filesystem independently reserves the raw copy plus worst-case archive, while the live-runtime filesystem derives its bounded publication margin from the current coherent database size on every run, never from a morning archive; when both paths share one filesystem the requirements are summed. This matters on the canonical host where `state/backups` is a separate mount from the live DB, and prevents a misleading or stale free-space reading. Every scope first revalidates and prunes eligible older archives, including privileged manual checkpoints, before its capacity gate; a fresh incoming checkpoint reserves one retention slot first, so steady state at the retention limit cannot fail capacity while an eligible oldest archive still occupies that slot. The pipeline then runs official supply refresh, supply-specific downstream component materialization, complete official stock capture, one canonical warehouse/cost publication and targeted Proxy publication. A successful run losslessly archives its restore point with verified compressed and decompressed SHA-256; later hourly/operator runs of the same business day revalidate the actual daily archive bytes and reuse that archive. Archive retention is bounded independently for daily, operator-settings and privileged manual-sync scopes: only older exact archive/manifest pairs are removed after byte-level revalidation, with an atomically replaced durable `0600` intent/completion journal compatible with prior completed audit rows; every retained candidate is verified first and newest restore points remain retained. Successful manual-sync readback exposes the verified archive as primary backup evidence and marks the raw source removed. CLI and operator-button runs share one re-entrant cross-process lock, so the bounded pipeline never overlaps itself while nested canonical publications do not deadlock; versioned calculation-parameter publication uses the same lock. The warehouse sync process alone raises the DB-backed runtime's bounded SQLite busy wait to 120 seconds (operator override `WB_CORE_WAREHOUSE_SYNC_SQLITE_BUSY_TIMEOUT_MS`, hard-clamped to 5–300 seconds), so an unrelated short transaction can finish without changing web/worker latency or creating a second global lock. Every hourly/manual phase and the shared-lock wait are timed in the machine-readable result; exhausted SQLite contention is reduced to the phase-specific `warehouse_sync_sqlite_write_wait_expired` reason while the last-good version remains served. The hourly oneshot has `TimeoutStartSec=3h`, so a capacity-gated backup, full publication, reconciliation and lossless archive can finish on the supported production data volume instead of being terminated by systemd's shorter default boundary. It does not invoke legacy daily cost/product-capital rebuild or the global vitrina refresh;
- `warehouse-functional-maintenance status|hold|restore` is the only bounded Finance-maintenance boundary for the hourly warehouse timer. It records the exact timer/service/last-next-trigger/shared-lock/Finance-process baseline in mode `0600`, stops only the timer and waits for an already-running service without killing it, then restores exact enabled/active state only after Finance has released the shared writer lock. Timer-unit drift remains fail-closed. A service-unit refresh made by a deploy while the hold is active is accepted only when the service is quiescent, the shared lock is free, Finance is absent, the timer digest is unchanged, `NeedDaemonReload=no`, the loaded service has no drop-ins and its fragment is byte-for-byte equal to the repo-deployed systemd artifact; the evidence is revalidated after timer mutations, and both digests and paths are persisted in the maintenance audit. The canonical Finance apply itself holds `.warehouse-functional-sync.lock` across plan, coherent backup, atomic apply and transactional readback; arbitrary `systemctl`/SSH holds and a second lock are not supported;
- `business-data-maintenance status|hold|restore|set-process` is the canonical cross-writer quiet-window boundary and the backend for Settings `Автообновления`. `status` and Settings GET are strictly read-only. Owner-policy v2 contains individual desired state only for Settings-owned processes: Web-vitrina refresh, its separate closure retry, warehouse/cost and Finance. Autoanswers, auto-complaints and SPP test are feature-owned monitoring-only processes; their desired state is derived from the canonical feature settings/schedule, and direct `set-process` is rejected before policy write. `hold` disables Settings-owned tickers, leaves all canonical schedule JSON unchanged, suspends actual auto-complaints/SPP execution without changing their feature schedule intent and invokes the dedicated Autoanswers lifecycle with master suspension so both readonly and worker stop without losing mode/run/cap. It also invokes `warehouse-functional-maintenance hold --disable-timer`. The runner inventories every installed `wb-core-*.timer`, paired service, matching cron, known writer and shared lock; unknown or active unbounded writers fail closed, while already-running oneshots are waited rather than killed. Mode-`0600` maintenance/audit retains exact pre-hold and final readback evidence. Legacy generic Autoanswers desired entries are ignored as intent. `restore --expected-revision <N>` requires the exact revision and quiet/no-writer/no-lock/no-drift preflight, restores Settings-owned desired state and asks each feature-owned canonical lifecycle to reconcile its latest intent. The generic plane never directly enables Autoanswers timers and never creates a transition run. Autoanswers resume requires exact existing run/cap, timer readback and then a fresh scheduler tick; a pending tick is not confirmed success. Partial restore disables the allowlist again and remains fail closed. Settings POST returns confirmed only with revision/persistence/runtime readback; an explicit lifecycle-pending result stays pending, and the browser keeps any mutation/readback error visible after factual reload. Registry HTTP, archived Data MCP compatibility, Release Train and deploy/verification infrastructure are not stopped. The deploy manifest never enables or restarts business timers, so an acquired hold remains intact across rsync, unit installation, daemon-reload and service restart until the explicit lifecycle-aware restore;
...
- `warehouse-ui-flow --evidence-dir <absolute path outside repo>` uses a fresh isolated Playwright context and reconciles navigation, six warehouses, WB contour, settings/reference, Proxy 3, supplier cost/bank fee fields, consumers, legacy redirects and sync freshness with protected readback. For SKU-management it captures the page's original protected response and reconciles the exact source nmID/populated-Proxy-3 sets with the rendered status, rows and cells; it does not issue a redundant second full `/sku-management` download after the page has loaded that same read model. Evidence stays outside Git.
...
- `managed_systemd_units = registry-http + wb-ai-api + refresh/closure/auto-complaints/SPP/Finance/warehouse/Autoanswers service+timer units + archived-compatibility wb-core-data-mcp.service`; deploy installs every listed unit and runs `daemon-reload`, but only infrastructure services are deploy-enabled/restarted. Business timers have `enable=false,restart=false`: their actual enabled state is owned exclusively by Settings/feature lifecycles and is preserved across deploy.
...
- If a verified correction candidate cannot be built because immutable historical SQLite backups exhaust the target filesystem, only `apps/sqlite_backup_archive.py` may losslessly transcode an existing file below a `backups/` directory. Hosted `sqlite-backup-archive-dry-run|apply` further restricts this path to one raw checkpoint in the canonical `warehouse-functional-sync` directory. Planning opens SQLite with `mode=ro&immutable=1`, requires `query_only=ON`, records unchanged sidecars, source stat/SHA/integrity, a non-target directory digest and source-size-plus-expansion-plus-explicit-reserve capacity before creating bytes. Apply tests the zstd frame and streamed decompressed size/SHA, writes private fsynced archive/manifest bytes, independently revalidates the retained archive before raw removal, removes only the exact source and its unchanged empty WAL/SHM sidecars, rechecks non-target entries and finalizes `verified_pending_source_removal → retained`. A crash preserves a machine-resumable archive/manifest family; an exact repeat completes or returns no-op. It never deletes or rewrites business/runtime rows.
