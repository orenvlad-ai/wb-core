...
- functional economics full plan/source/snapshot revalidation runs without a SQLite transaction while an idle connection pins `PRAGMA data_version`; its exact-date product-capital lookup is built before the shared WB+FF inventory-cost blend, and that one versioned/freshness/coverage fingerprint supplies visible cost plus Proxy 3/4. TOTAL cost is the aggregate evidence capital/quantity ratio and Proxy totals use eligible SKU sums/denominators. A bounded `BEGIN IMMEDIATE` is acquired only after unchanged-version proof and contains only optimistic target updates, exact readback and the undo manifest. A concurrent commit invalidates the plan and retries through the normal worker instead of allowing a multi-minute rollback-journal transaction to starve interactive FF preview/status/confirm. Atomic publication, recovery before-images, repeat no-op and last-good safety remain mandatory;
