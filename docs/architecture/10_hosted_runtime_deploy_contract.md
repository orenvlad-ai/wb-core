...
- the buyer-session recovery for `Цены -> Проверка СПП` reuses only the installed headed-browser OS/runtime dependencies, never Seller Portal storage or lock. Its canonical state is `/opt/wb-core-runtime/state/wb_buyer_session` with separate `buyer_session.lock`, localhost-only VNC/web ports and protected routes `.../prices/spp-test/buyer-session/check`, `.../recovery/status`, `.../recovery/start`, `.../recovery/stop`, `.../recovery/launcher.zip`. The first valid WB buyer account is bound by an irreversible keyed fingerprint; subsequent wrong-account login is rejected. Session state survives repo deploy/restart, remains outside Git, and is never returned in HTTP payloads or logs.
- the buyer recovery supervisor is an isolated process-group leader; Xvfb/openbox/x11vnc/websockify and its browser stay in that same group. `stop` terminates the complete group and removes candidate/pid state, while normal completion terminates each child and removes the same temporary files. After visible `/lk` login, candidate validation performs a bounded settled retry of the same atomic storage snapshot before another browser-state rewrite and publishes only sanitized interim reasons.
