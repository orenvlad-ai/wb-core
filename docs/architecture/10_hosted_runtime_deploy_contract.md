...
- `business-data-maintenance status|hold|restore|set-process` is the canonical cross-writer quiet-window boundary for an explicitly approved production maintenance run and the backend for Settings `Автообновления`. `status` and Settings GET are strictly read-only and never initialize owner policy. `hold` first disables the five managed business tickers (Web-vitrina refresh, closure retry, auto-complaints, SPP schedule and Finance), changes all three server-owned business schedules to `enabled=false` through their authenticated loopback APIs, invokes the existing autoanswers OFF lifecycle and disables both autoanswers timers, and invokes `warehouse-functional-maintenance hold --disable-timer`. The durable warehouse option preserves the exact restorable baseline while making the timer both disabled and inactive. The runner inventories every installed `wb-core-*.timer`, its paired service, matching cron entries, known writer processes and warehouse/Web-schedule/SPP/Seller-Portal locks; unknown timers or cron writers fail closed. Already-running oneshots are never killed: the hold waits for terminal service/process/lock evidence. Mode-`0600` maintenance state/audit retains pre-hold systemd/runtime-schedule evidence; a separate versioned owner policy/audit retains master desired state and each allowlisted process desired state with provenance, revision, actor/reason and pre/post readback. Initial migration happens only inside an explicit mutation, never assumes all-on and leaves unknown provenance fail-closed. `set-process` requires exact revision and audited actor/reason; lifecycle-owned Autoanswers `ON`, no-op desired state and stale revision fail before policy write or hold, while an erroneous Autoanswers desired state may still be corrected back `OFF`. `restore --expected-revision <N>` requires exact optimistic revision, quiet/no-writer/no-lock/no-drift preflight, restores only desired `ON`, preserves intentional `OFF`, and disables the allowlist again on partial failure. A completed restore closes the hold generation so the next hold captures a fresh baseline instead of reusing the prior window. Settings POST returns success only with a receipt proving revision advance, persisted state and runtime readback; the browser then performs a second GET and keeps any mutation/readback error visible instead of overwriting it with a generic GET success. Registry HTTP, Data MCP, Release Train and deploy/verification infrastructure are not stopped. A later deploy may re-enable target-managed timers, so the same hold/readback is mandatory again after every deploy;
