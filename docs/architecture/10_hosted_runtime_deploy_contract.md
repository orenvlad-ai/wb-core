...
- `warehouse-functional-sync` is the bounded manual/hourly WB pipeline: before its first mutation it creates or verifies one coherent restore point for the open canonical business day. Capacity is checked against actual mount topology: the backup filesystem independently reserves the raw copy plus worst-case archive, while the live-runtime filesystem reserves a bounded SQLite publication margin; when both paths share one filesystem the requirements are summed. This matters on the canonical host where `state/backups` is a separate mount from the live DB, and prevents a misleading free-space reading from either path. The pipeline then runs official supply refresh, supply-specific downstream component materialization, complete official stock capture, one canonical warehouse/cost publication and targeted Proxy publication. A successful run losslessly archives the restore point with verified compressed and decompressed SHA-256; later hourly runs of the same business day revalidate the actual archive bytes and reuse that archive instead of accumulating a full-database copy on every open-day refresh. CLI and operator-button runs share one cross-process lock, so the bounded pipeline never overlaps itself. It does not invoke legacy daily cost/product-capital rebuild or the global vitrina refresh;
