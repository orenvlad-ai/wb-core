...
- functional economics full plan/source/snapshot revalidation runs without a SQLite transaction while an idle connection pins `PRAGMA data_version`; a bounded `BEGIN IMMEDIATE` is acquired only after unchanged-version proof and contains only optimistic target updates, exact readback and the undo manifest. A concurrent commit invalidates the plan and retries through the normal worker instead of allowing a multi-minute rollback-journal transaction to starve interactive FF preview/status/confirm. Atomic publication, recovery before-images and last-good safety remain mandatory;
