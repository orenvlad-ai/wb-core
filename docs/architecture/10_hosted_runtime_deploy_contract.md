...
- the buyer-session recovery for `Цены -> Проверка СПП` reuses only the installed headed-browser OS/runtime dependencies, never Seller Portal storage or lock. Its canonical state is `/opt/wb-core-runtime/state/wb_buyer_session` with a single-flight start lock and one supervisor-owned session automation lock, localhost-only VNC/web ports and protected routes `.../prices/spp-test/buyer-session/check`, `.../recovery/status`, `.../recovery/start`, `.../recovery/stop`, `.../recovery/launcher.zip`. Opening the SPP subsection first rejoins an existing exact `run_id`, otherwise checks the session and starts bounded recovery automatically. `start_recovery()` publishes `starting` before spawning exactly one supervisor and does not run another session probe; active recovery status gates later UI/status probes. The supervisor bounded-waits for any already-running preflight, acquires the buyer lock itself, and owns it continuously through session check, headed-browser recovery, candidate capture, independent validation and atomic save, with terminal `completed` published only after lock release. Exactly one safe saved-account `Войти` action is clicked automatically; VNC/websockify and launcher are started only after WB requires SMS/phone/captcha/security/multiple-account or another human action. Candidate state is captured after settle, independently probed, atomically saved and probed again; failed final validation restores the prior state. Account binding uses HMAC-only normalized stable identity, migrates v1 only with proof and never rebinds a genuinely different or unprovable account. Session state survives repo deploy/restart, remains outside Git, and is never returned in HTTP payloads or logs.
...
- the buyer recovery supervisor is an isolated process-group leader; Xvfb/openbox/x11vnc/websockify and its browser stay in that same group. `stop` terminates the complete group and removes candidate/pid state, while normal completion terminates each child and removes the same temporary files. After visible `/lk` login, recovery waits for bounded auth settle, captures a new atomic candidate snapshot, validates it through an independent browser probe and publishes only sanitized interim reasons.
