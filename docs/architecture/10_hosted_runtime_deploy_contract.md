...
- `python3 apps/wb_incident_policy_legacy_disable.py --runtime-dir <canonical state> --actor <actor> --expected-deployed-sha <exact SHA>` is the dry-run-default, query-only path for the Migration 147 policy retirement. It is bounded to one seller revision effective `2026-08-16`, resolves the canonical operational generation through `StoreRegistry`, requires both completed deployed-SHA markers and emits a fingerprinted machine-readable manifest without writing. `--apply` additionally requires the exact outside-Git reviewed-manifest SHA, immutable post-merge owner apply-gate reference/digest and an outside-Git evidence directory. Apply revalidates generation/source/history/protected-table digests, creates a coherent mode-0600 SQLite backup, repeats the source check under `BEGIN IMMEDIATE`, and performs one append-only inactive policy INSERT while preserving the prior JSON identity/entry payloads. Reconciliation proves every previous revision and ready/raw/evidence/cache/rematerialization-audit table unchanged; it never invokes incident rematerialization and exact repeat is T0. Standalone `--readback` is query-only. Any recovery is a separately reviewed forward append; backup restore requires all writers quiesced and proof that no later valid writes would be lost;
