...
- Canonical production Finance recalculation uses only `finance-canonical-dry-run`, `finance-canonical-apply` and `finance-canonical-readback` in `apps/registry_upload_http_entrypoint_hosted_runtime.py`. Dry-run/apply scope is the full loaded Finance history. The dry-run keeps that complete scope while streaming ordered raw/non-target identities into deterministic digests and retaining only persisted target readback evidence plus the required aggregated operation-date matrix. Repeated missing-cost operations collapse by week/SKU/operation-date/reason with operation and sale/return quantities retained; their per-row identities remain in the source hash. Global capitalization allocations and the complete supply-layer manifest are computed once per coherent SQLite connection and rebuilt on a later connection/source state instead of re-hashed for every SKU. The production-scale regression covers 295,919 sale rows including roughly 148k missing-cost rows and 50,000 supply cost layers. Apply validates active target/runtime paths, schema-v2 plan, exact newly reviewed fingerprint and human approval reference before invoking `canonical-cost-backfill` with the bounded backup directory. The superseded `finance-retro-*`/`business-approved-backfill` path is unavailable; ad-hoc production SQL or server-only scripts are prohibited.
