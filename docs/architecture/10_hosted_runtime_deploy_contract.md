...
- `warehouse-recovery-canary-dry-run|apply` pins `.wb-core-runtime-sha`, proves T0 zero bytes/rows, an exact rolled-back T1 marker replay, a retained T2 domain checkpoint, unchanged warehouse-domain digest and a clean orphan scan without business mutation;
- `warehouse-ui-flow --evidence-dir <absolute path outside repo> --acceptance-profile warehouse_recovery_policy_20260726` uses a fresh isolated Playwright context and additionally verifies the protected recovery API/card, terminal canary lifecycle, capacity/writer/timer/rollback visibility and zero orphan/quarantine leak. Evidence stays outside Git.
