...
- `ff-pool-overhead-backfill-dry-run|apply|readback` is the only bounded production path for the five `2026-08-21` facility/FBS overhead documents agreed in migration 152. Dry-run is query-only and resolves exact server-owned document, operation, facility, pool, SKU and event-revision identities; the prompt totals are eligibility constraints, never apply identities. The external plan binds exact deployed SHA, pre-change digests, coherent backup, current-only aggregate projection, quantity/capital/document/lifecycle/non-target invariants and deterministic queue revisions. Apply requires the separately reviewed fingerprint and exact post-merge authorization, backs up coherently, performs only the short current aggregate/queue mutation, then runs Warehouse/economics/Finance publication and query-only reconciliation outside the broad writer boundary. It never re-posts a document, changes historical fulfilled handoff cost or uses ad-hoc SQL/server-only code; exact repeat is a proven no-op;
