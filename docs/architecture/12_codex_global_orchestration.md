...
2. читает registry snapshot/integrity и read-only `python3 apps/github_release_train.py queue-status`, добавляя `--release-proof-pr <OWNER_PR>` для каждого non-confirmed closure row, затем передаёт exact queue JSON вместе с совпадающим canonical digest в durable `heartbeat-plan` с полным ordered phase list, exact target set и release-lane recovery state;
...
6. для terminal/accepted task с residual `release:lane-owner` обслуживает durable release-lane closure outbox: отправляет exact positive-revision command, фиксирует transport/failure receipt, подтверждает освобождение следующим queue readback и после трёх неуспешных попыток открывает incident;
7. обслуживает revision-bound attention outbox и подтверждает delivery receipts;
8. завершает run единственным stdout `heartbeat-finish`; команда fail closed проверяет coverage/follow-up/release-lane/attention, освобождает lease и возвращает canonical `NOTIFY/DONT_NOTIFY` wrapper.

`watcher_run_plans`, `watcher_run_targets` и `release_lane_closures` являются локальным durable audit обязательных действий, а не новым control service. Desktop readback/transport остаются агентными, но их результат принимается только как exact machine observation `wb-core-watcher-target-observation/v1`, связанный с task/revision, executor thread/generation/host, turn/final item, timestamp и digests. Completed target без такого binding не превращается в terminal task по одному слову из чата.
...
Platform heartbeat response тоже repo-owned. `heartbeat-finish` проверяет complete target/follow-up/release-lane/attention receipts, вызывает canonical renderer и сериализует ровно один `<heartbeat>` wrapper; standalone `heartbeat-response` остаётся только низкоуровневым renderer/smoke interface. При одном или нескольких active workstreams `decision=NOTIFY` обязателен на каждом scheduled run, а parsed XML `message` равен canonical report stdout. Поэтому unchanged fingerprint остаётся видимым periodic update: renderer заменяет delta на краткое `Изменений нет; работа продолжается: ...`, но wrapper не превращает его в `DONT_NOTIFY`. Несколько workstreams/envelopes сохраняются отдельными русскими blocks в одном message. Никакого plain report, summary или diagnostics после wrapper Watcher не добавляет. XML escaping относится только к transport serialization; parsed message text остаётся исходным report.
