...
def main() -> None:
    ...
    recovery_raw = recovery_manifest_path.read_bytes()
    recovery_manifest = json.loads(recovery_raw)
    recovery_binding = {
        "path": "release/production-mutations/wbc0035_recovery_scratch_bootstrap.json",
        "sha256": runner.sha256(recovery_raw),
    }
    bridge = runner.build_recovery_scratch_release_bridge(
        recovery_binding,
        recovery_manifest,
        MERGE,
    )
