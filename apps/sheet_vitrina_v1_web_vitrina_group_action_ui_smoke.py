...
def _assert_group_action_launch_error() -> None:
    with LocalWebVitrinaFixtureServer(with_ready_snapshot=True) as base_url:
        with sync_playwright() as playwright:
            ...
            if page.locator("[data-refresh-source-group='onec_product_capital']").count() != 0:
                raise AssertionError("archived 1C group refresh must not be exposed in active UI")
            ...
            print("web_vitrina_onec_group_archived: ok -> hidden")
            browser.close()
