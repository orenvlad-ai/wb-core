...
def run(output:Path):
    ...
    with sync_playwright() as p:
        ...
        with running_fixture() as f:
            ...
            expect(page.locator('[data-kc-batch-result]')).to_contain_text('Нужна сверка WB',timeout=8000)
            ...
            page.reload(wait_until='domcontentloaded');expect(page.locator('[data-kc-batch-result]')).to_contain_text('Нужна сверка WB',timeout=8000)
            ...
            expect(page.locator('[data-kc-batch-result]')).to_contain_text('Уточнить результат WB')
            page.get_by_role('button',name='Уточнить результат WB').click()
            ...
            expect(page.get_by_role('button',name='Уточнить результат WB')).to_have_count(0)
