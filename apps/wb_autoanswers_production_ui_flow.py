...
def run_autoanswers_ui_flow(
    *,
    ...
    expected_transition_run_id: str = "",
    deployed_sha: str = "",
) -> dict[str, Any]:
    ...
    if expected_state not in {"off-force", "off-unforced", "manual", "auto-all"}:
        raise ValueError(
            "expected_state must be off-force, off-unforced, manual or auto-all"
        )
    expected_transition_run_id = str(expected_transition_run_id or "").strip()
    deployed_sha = str(deployed_sha or "").strip()
    if expected_state == "auto-all":
        if expected_policy_epoch is None or int(expected_policy_epoch) < 1:
            raise ValueError("auto-all acceptance requires expected_policy_epoch")
        if not expected_transition_run_id:
            raise ValueError(
                "auto-all acceptance requires expected_transition_run_id"
            )
        if len(deployed_sha) != 40:
            raise ValueError("auto-all acceptance requires an exact deployed SHA")
    ...
    with sync_playwright() as playwright:
        ...
        lifecycle_before = dict(settings.get("lifecycle") or {})
        if expected_state == "off-force":
            ...
            _assert(setting_values.get("master_enabled") is True, "Full acceptance requires persisted master ON")
            _assert(setting_values.get("force_off") is False, "Full acceptance requires force-off removed")
            _assert(setting_values.get("effective_enabled") is True, "Full acceptance requires effective ON")
            _assert(str(setting_values.get("mode") or "") == "auto_all", "effective production mode must be auto_all")
            _assert(page.locator("[data-autoanswers-mode]").input_value() == "auto_all", "selector must show Full")
            _assert(not page.locator("[data-autoanswers-mode]").is_disabled(), "admin mode selector must be enabled")
            _assert(
                int(setting_values.get("policy_epoch") or 0)
                == int(expected_policy_epoch),
                "Full acceptance policy epoch does not match the requested run",
            )
            _assert(
                lifecycle_before.get("lifecycle_state") == "running",
                "Full acceptance requires running lifecycle",
            )
            _assert(
                lifecycle_before.get("drift_status") == "matched",
                "Full acceptance requires matched lifecycle",
            )
            _assert(
                lifecycle_before.get("actual") is True
                and lifecycle_before.get("desired") is True,
                "Full acceptance requires desired and actual lifecycle ON",
            )
            _assert(
                lifecycle_before.get("fresh_scheduler_tick") is True,
                "Full acceptance requires a fresh scheduler tick",
            )
            _assert(
                not str(lifecycle_before.get("last_error") or "").strip(),
                "Full acceptance must not retain a lifecycle error",
            )
            _assert(
                str(lifecycle_before.get("transition_run_id") or "")
                == expected_transition_run_id,
                "Full acceptance transition run does not match",
            )
            _assert(
                int(lifecycle_before.get("policy_epoch") or 0)
                == int(expected_policy_epoch),
                "Full acceptance lifecycle policy epoch does not match",
            )
            _assert(
                str(lifecycle_before.get("stop_reason") or "")
                not in {"worker_error", "reservation_missing"},
                "Full acceptance contains a blocking worker error",
            )
            components = dict(
                lifecycle_before.get("components")
                or lifecycle_before.get("component_states")
                or {}
            )
            for component_name in ("readonly_sync", "worker"):
                ...
                _assert(
                    ...
                    f"Full acceptance requires matched {component_name}",
                )
            lifecycle_summary = page.locator(
                "[data-autoanswers-lifecycle-summary]"
            ).inner_text()
            for marker in ("Полный режим", "Readonly sync", "Worker"):
                _assert(
                    marker in lifecycle_summary,
                    f"Full lifecycle summary misses {marker}",
                )
            _assert(
                lifecycle_summary.count("Работает") >= 2,
                "Full lifecycle summary must show both components working",
            )
            _assert(
                not page.locator("[data-autoanswers-error]").inner_text().strip(),
                "Full UI retained a stale error banner",
            )
        else:
        ...
        _assert(page.locator("[data-autoanswers-backlog]").is_disabled(), "backlog control must remain disabled")
        ...
        for stage_name, locator in stage_specs:
            ...
            if expected_state == "auto-all":
                _assert(
                    ...
                    f"{stage_name} retained a stale manual pause",
                )
                _assert("Статус:" in text, f"{stage_name} active status is missing")
            else:
        ...
        if detail_sample:
            ...
            if expected_state != "manual":
                _assert(
                    page.locator("[data-autoanswers-generate]").count() == 0,
                    "automatic/OFF mode must hide manual generation",
                )
            detail_evidence = {
        ...
        if expected_state != "auto-all":
            _assert(jobs_unchanged, "UI acceptance created jobs")
        else:
            lifecycle_after = dict(settings_after.get("lifecycle") or {})
            _assert(
                lifecycle_after.get("lifecycle_state") == "running"
                and lifecycle_after.get("drift_status") == "matched"
                and lifecycle_after.get("fresh_scheduler_tick") is True,
                "Full lifecycle lost matched/fresh state during UI acceptance",
            )
            _assert(
                str(lifecycle_after.get("transition_run_id") or "")
                == expected_transition_run_id,
                "Full transition run changed during UI acceptance",
            )
            _assert(
                not page.locator("[data-autoanswers-error]").inner_text().strip(),
                "Full UI acquired an error banner during acceptance",
            )
    ...
    evidence = {
        ...
        "expected_transition_run_id": expected_transition_run_id or None,
        "jobs_unchanged": jobs_unchanged,
        "read_only_ui_flow": True,
