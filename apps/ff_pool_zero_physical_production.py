#!/usr/bin/env python3
"""Dry-run/apply/readback CLI for confirmed Moscow FBS physical zeros."""
...
def _read_plan(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        raise FfPoolZeroPhysicalProductionError(
            "reviewed zero-physical plan is invalid JSON"
        ) from exc
    if not isinstance(payload, dict):
        raise FfPoolZeroPhysicalProductionError(
            "reviewed zero-physical plan must be a JSON object"
        )
    return payload
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Owner-gated exact Moscow FBS physical-zero production runner."
    )
