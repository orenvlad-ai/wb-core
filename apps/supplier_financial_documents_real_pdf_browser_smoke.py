        ...
        try:
            ...
            with sync_playwright() as playwright:
                ...
                try:
                    ...
                    expect(summary_cards).to_contain_text("57 136", timeout=5000)
                    expect(summary_cards).to_contain_text("Логистика по КП, USD", timeout=5000)
                    expect(summary_cards).to_contain_text("16 151", timeout=5000)
                    expect(summary_cards).to_contain_text("КП таможня, USD", timeout=5000)
                    expect(summary_cards).to_contain_text("40 985", timeout=5000)
                    expect(summary_cards).to_contain_text("Счета логиста, ₽", timeout=5000)
                    expect(summary_cards).to_contain_text("1 215 975", timeout=5000)
                    expect(summary_cards).to_contain_text("Счета НДС, ₽", timeout=5000)
                    ...
                    expect(summary_cards).to_contain_text("295,62", timeout=5000)
                    expect(summary_cards).to_contain_text("КП: доставка+таможня, % от стоимости груза", timeout=5000)
                    expect(summary_cards).to_contain_text("50,94%", timeout=5000)
                    expect(summary_cards).to_contain_text("Факт: доставка+таможня, % от таможенной стоимости", timeout=5000)
                    expect(summary_cards).to_contain_text("49,42%", timeout=5000)
                    expect(summary_cards).not_to_contain_text("3 799", timeout=5000)
                    expect(summary_cards).not_to_contain_text("5 123", timeout=5000)
                    panel_text = page.locator("#financialDocumentsPanel").inner_text()
                    if "NaN" in panel_text or "Infinity" in panel_text:
                        raise AssertionError(f"financial panel contains invalid numeric output: {panel_text}")
                    ...
                    customs_row = page.locator("#financialDocumentsRows tr[data-financial-document-row]", has_text="ДТ").first
                    customs_row.click()
                    ...
                    expect(page.locator("#financialSummaryGroups")).to_contain_text("1 215 975", timeout=5000)
