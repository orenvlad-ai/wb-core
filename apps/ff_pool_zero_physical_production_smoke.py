"""Smoke checks for the bounded Moscow FBS physical-zero mutation."""
...
FACILITY_ID = "fac_moscow_production"
...
def main() -> None:
    with TemporaryDirectory(prefix="ff-pool-zero-physical-") as directory:
        ...
        assert plan["expected_effects"]["balance_insert_count"] == 3
        ...
        assert [item["state"] for item in readback["target_rows"]] == [
            "explicit_zero",
            "explicit_zero",
            "explicit_zero",
        ]
        assert [item["reserved"] for item in readback["target_rows"]] == [5, 0, 0]
        assert [item["available"] for item in readback["target_rows"]] == [-5, 0, 0]
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        conn.executemany(
            ...
            [
                ...
                for index, nm_id in enumerate(TARGET_NM_IDS, start=1)
            ],
        )
        conn.execute(
