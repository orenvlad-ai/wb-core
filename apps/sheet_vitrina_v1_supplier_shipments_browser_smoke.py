...
def _assert_supplier_role_browser_ui(browser, tmp_path: Path, invoice_path: Path) -> None:
    ...
    with _patched_env(
        ...
        try:
            ...
            expect(page.get_by_role("button", name="Проверить цены")).to_have_count(0)
            planned_shipment_date = page.get_by_label(
                "计划出货日期 / Planned shipment date / Плановая дата отгрузки"
            )
            save_button = page.get_by_role("button", name="保存 / Save / Сохранить")
            expect(save_button).to_be_disabled()
            planned_shipment_date.fill("2026-05-14")
            expect(save_button).to_be_enabled()
            planned_shipment_date.fill("")
            expect(save_button).to_be_disabled()
            planned_shipment_date.fill("2026-05-14")
            expect(save_button).to_be_enabled()
