...
26. `Поставки -> Расчёты -> Поставка на Wildberries` переведён с primary current-stock-share split на regional share ladder: `sales_avg_period_days` запрашивает качество истории, total demand остаётся `orderCount`, full clean days остаются preferred path, partial `SKU + district + day` observations восстанавливают доли без требования идеального дня по всем округам сразу, missing districts заполняются deterministic SKU group prior и затем global prior, а seed floor применяется только как последняя one-box страховка при отсутствии own/group/global share и no-usable current stock. Result/API/UI diagnostics показывают share source counts, low-confidence directions, per-row `share_source/share_confidence`, seed quantities and wording `SKU / направления SKU-округ`; current-stock-share fallback не является нормальным путём.
