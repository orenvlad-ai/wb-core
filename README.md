...
- вкладка `Отзывы` в active website/operator contour: read-only WB feedbacks load/filter table со строгими server-side `date_from/date_to/stars/is_answered` фильтрами, official WB review tags/chips в таблице/Excel/AI input, bounded 62-day feedback date picker independent from ready-snapshot dates and defaulting to the current server/business week (`D-6..D`) when no same-day manual range exists, chunked `take/skip` загрузкой без тихого row cap, diagnostic meta, Excel export текущей таблицы, resizable columns, bounded AI-assisted разбор через server-side prompt+model config/OpenAI route и подраздел `Жалобы` над runtime-журналом жалоб; operator UI показывает статус жалобы по `feedback_id`, даёт выбрать только non-journaled rows после AI-разбора и запускает protected async `submit-selected` job через тот же guarded Seller Portal submit/resolver path с hard cap `5`, exact/actionable gates and event log; shared Seller Portal resolver treats `Есть ответ` / `Ждут ответа` as first-class status tabs, uses date/star diagnostics, safe WB article/nmId search and bounded virtualized scroll before declaring a row not actionable; AI-разметка не является accepted truth/ЕБД persistence, а неопределённые submit-попытки проверяются отдельными read-only confirmation/detail-network probe runners с direct-id или strict strong-composite proof;
