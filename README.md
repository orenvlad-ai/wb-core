...
26. `Поставки -> Расчёты -> Поставка на Wildberries` переведён с primary current-stock-share split на stock-depletion valid-day methodology: `sales_avg_period_days` запрашивает число валидных дней выбывания, total demand остаётся `orderCount`, выбранные округа участвуют в validation/share calculation while excluded districts stay visible with zero primary demand, guarded persistent-zero `0 -> 0` cases with `current_stock < order_batch_qty` are neutralized instead of causing normal fallback, selected persistent-zero districts can receive a separately diagnosed one-box test seed, regional fallback стал явным diagnostic state, long `nmId` fallback lists moved out of the main result card into expandable diagnostics, а хвост распределения коробов при ограниченном `stock_ff` сначала оптимизирует rescued units.
