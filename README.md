...
- `Отчёты -> Отчёт по остаткам` в operator UI теперь является full active-SKU sortable table over persisted ready snapshot truth: stock and `promo_participation` come from `DATA_VITRINA/yesterday_closed`, `sales_avg_period_days` defaults to `14` and uses the shared availability-adjusted demand helper over persisted `sales_funnel_history`, total/district days-left stay numeric/null, and district расход is calculated only from positive depletion between consecutive persisted ready snapshots without reviving Google Sheets/GAS or browser-side truth;
