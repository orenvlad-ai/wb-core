...
26. `Поставки -> Расчёты -> Поставка на Wildberries` переведён с primary current-stock-share split на stock-depletion valid-day methodology: `sales_avg_period_days` запрашивает число валидных дней выбывания, total demand остаётся `orderCount`, regional fallback стал явным diagnostic state, а хвост распределения коробов при ограниченном `stock_ff` сначала оптимизирует rescued units.
