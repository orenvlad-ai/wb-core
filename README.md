...
25. `Поставки -> Wildberries` добавлен как read-only WB/FBW supplies registry: official WB Supplies API adapter, server cache/incremental sync/full backfill/status routes, runtime SQLite persistence with run progress, compact operator table, filters, route/warehouse/quantity/cost evidence normalization, clickable supply-date sorting and pagination without WB mutations.
