(() => {
  ...
  function negateAmount(amount) { return typeof amount === "string" && amount && !amount.startsWith("-") ? `-${amount}` : amount?.startsWith("-") ? amount.slice(1) : amount; }
  function documentAmount(doc) { const amount = doc.amount || doc.amount_decimal || doc.total_amount; if (doc.document_type === "transfer" || doc.document_type === "opening") return amount; const expenseEffect = doc.document_type === "expense"; return isCorrection(doc) !== expenseEffect ? negateAmount(amount) : `+${amount}`; }
