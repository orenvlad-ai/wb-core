...
def _normalize_supply_row(
    *,
    ...
    merged = {**dict(raw_list), **dict(detail)}
    supply_id_value = _first_value(merged, "supplyID", "supplyId", "supply_id", "ID", "id")
    preorder_id_value = _first_value(merged, "preorderID", "preorderId", "preorder_id", "orderID", "orderId")
    ...
    status_id = _optional_int(_first_value(merged, "statusID", "statusId", "status_id"))
    warehouse_id = _id_to_string(_first_value(merged, "warehouseID", "warehouseId", "warehouse_id"))
    actual_warehouse_id = _id_to_string(
        _first_value(merged, "actualWarehouseID", "actualWarehouseId", "actual_warehouse_id")
    )
    transit_warehouse_id = _id_to_string(
        _first_value(merged, "transitWarehouseID", "transitWarehouseId", "transit_warehouse_id")
    )
    warehouse_name = _first_string(merged, "warehouseName", "warehouse_name") or warehouse_by_id.get(warehouse_id, "")
    actual_warehouse_name = (
        _first_string(merged, "actualWarehouseName", "actual_warehouse_name")
        or warehouse_by_id.get(actual_warehouse_id, "")
    )
    transit_warehouse_name = (
        _first_string(merged, "transitWarehouseName", "transit_warehouse_name")
        or warehouse_by_id.get(transit_warehouse_id, "")
    )
    box_type_id = _optional_int(_first_value(merged, "boxTypeID", "boxTypeId", "box_type_id"))
    planned_quantity = _optional_number(_first_value(merged, "quantity", "plannedQuantity", "addedQuantity"))
    goods_quantity = _sum_goods_field(raw_goods, "quantity") if raw_goods is not None else None
    accepted_quantity = _optional_number(_first_value(merged, "acceptedQuantity", "accepted_quantity"))
    if accepted_quantity is None and raw_goods is not None:
        accepted_quantity = _sum_goods_field(raw_goods, "acceptedQuantity")
    unloading_quantity = _optional_number(_first_value(merged, "unloadingQuantity", "unloading_quantity"))
    ...
    acceptance_cost = _optional_number(_first_value(merged, "acceptanceCost", "acceptance_cost"))
    acceptance_coefficient = _optional_number(
        _first_value(merged, "paidAcceptanceCoefficient", "acceptanceCoefficient", "acceptance_coefficient")
    )
    create_date = _first_string(merged, "createDate", "createdAt", "created_at")
    supply_date = _first_string(merged, "supplyDate", "supply_date")
    fact_date = _first_string(merged, "factDate", "fact_date")
    updated_date = _first_string(merged, "updatedDate", "updated_at", "updatedDate")
    ...
    return {
        ...
        "box_type_label": f"boxTypeID {box_type_id}" if box_type_id is not None else "",
        "type_label": _type_label(box_type_id=box_type_id, is_transit=is_transit),
        "is_box_on_pallet": _optional_bool(_first_value(merged, "isBoxOnPallet", "is_box_on_pallet")),
        ...
        "quantity_added": planned_quantity,
...
def _quantity_for_size_filter(
    *,
    ...
    if planned_quantity is not None:
        return planned_quantity, "detail.quantity"
...
def _row_matches_warehouse(row: Mapping[str, Any], warehouse_id: str, warehouse: str) -> bool:
    ...
    if warehouse:
        ...
        return needle in " ".join(
            str(row.get(key) or "")
            for key in ("warehouse_name", "actual_warehouse_name", "transit_warehouse_name", "warehouse_display")
        ).casefold()
    return True
...
def _status_options(rows: list[Mapping[str, Any]]) -> list[dict[str, Any]]:
    status_ids = sorted({status_id for row in rows if (status_id := _optional_int(row.get("status_id"))) is not None})
...
def _type_label(*, box_type_id: int | None, is_transit: bool) -> str:
    ...
    if box_type_id is not None:
        parts.append(f"boxTypeID {box_type_id}")
    if is_transit:
        parts.append("Транзит")
    return " · ".join(parts)
...
def _warehouse_display(
    *,
    ...
    if transit_warehouse_name and warehouse_name and transit_warehouse_name != warehouse_name:
        return f"{transit_warehouse_name} → {warehouse_name}"
    return warehouse_name or actual_warehouse_name or transit_warehouse_name


def _warehouse_fact_line(*, warehouse_name: str, actual_warehouse_name: str) -> str:
