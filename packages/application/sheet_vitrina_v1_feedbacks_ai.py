...
Review tags:
- Во входных данных может быть review_tags: WB-теги/чипы отзыва, например "Плохое качество", "Внешний вид", "Размер", "Качество", "Удобство", "Не соответствует описанию".
- Используй review_tags вместе с text, pros и cons.
- Если text/pros/cons пустые, но review_tags непустые, отзыв НЕ пустой.
- Нельзя писать "отзыв без текста", "текст отсутствует", "нет описания" или похожее, если review_tags непустые.
- Товарные теги вроде "Плохое качество", "Внешний вид", "Размер", "Качество", "Удобство", "Не соответствует описанию" обычно означают обычную товарную претензию и чаще complaint_fit=no.
- Evidence может ссылаться на тег: "тег: Плохое качество".
- Для no при товарном теге можно писать: "Жалобу не подавать: отзыв содержит товарный тег \"Плохое качество\", то есть обычную претензию к товару."
- Если tags указывают на товарное качество, не жалуйся только из-за отсутствия свободного текста.
        ...
        rows.append(
            {
                ...
                "review_tags": normalize_review_tags(raw_row.get("review_tags") or []),
                "tag_source": _short_string(raw_row.get("tag_source"), 80),
                "pros_tags": normalize_review_tags(raw_row.get("pros_tags") or []),
                "cons_tags": normalize_review_tags(raw_row.get("cons_tags") or []),
...
def _validated_results(raw_results: list[dict[str, Any]], *, expected_rows: list[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    expected_by_id = {str(row["feedback_id"]): row for row in expected_rows}
    ...
    for raw in raw_results:
        ...
        expected_row = expected_by_id.get(feedback_id) or {}
        review_tags = normalize_review_tags(expected_row.get("review_tags") or [])
        if reason_contradicts_review_tags(reason, review_tags):
            raise OpenAiFeedbacksAnalysisError("OpenAI reason contradicts review_tags")
        by_id[feedback_id] = {
            "feedback_id": feedback_id,
