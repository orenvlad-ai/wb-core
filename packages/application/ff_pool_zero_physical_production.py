"""Owner-gated publication of three confirmed Moscow FBS physical zero rows."""
...
TARGET_POOL = "FBS"
TARGET_NM_IDS = (497413772, 497415593, 497416931)
ABSOLUTE_TARGET = 0
DECISION_DATE = "2026-08-19"
REQUEST_ID = "ff-moscow-fbs-zero-physical-20260819-v1"
SOURCE_ID = "ff-moscow-fbs-zero-physical-20260819"
...
class FfPoolZeroPhysicalProductionMutation:
    ...
    def build_plan(self) -> dict[str, Any]:
        ...
        plan: dict[str, Any] = {
            ...
            "scope": {
                "facility_id": snapshot["facility"].get("facility_id", ""),
            ...
            "expected_effects": {
                ...
                "pool_inventory_document_count": 1 if insert_count else 0,
                "warehouse_operation_count": 1 if insert_count else 0,
                "absolute_target_line_count": len(TARGET_NM_IDS) if insert_count else 0,
    ...
    def apply(
        ...
        *,
        ...
        if int(fresh["expected_effects"]["balance_insert_count"]) == 0:
            raise FfPoolZeroPhysicalProductionError(
                "reviewed plan has no missing rows and no matching immutable document"
            )

        scope = dict(reviewed_plan["scope"])
        facility_id = str(scope["facility_id"])
        epoch = int(fresh["pre_change"]["feature_epoch"]["epoch"])
        expected_missing_nm_ids = [
            int(item["nm_id"])
            for item in fresh["pre_change"]["target_rows"]
            if item["state"] == "missing"
        ]
        expected_existing_zero_nm_ids = [
            int(item["nm_id"])
            for item in fresh["pre_change"]["target_rows"]
            if item["state"] == "explicit_zero"
        ]
        identity = DocumentIdentity(
            request_id=_request_id_for_fingerprint(fingerprint),
            source_system="owner_business_decision",
            source_type=CONFIRMED_ZERO_PHYSICAL_SOURCE_TYPE,
            source_id=SOURCE_ID,
        ...
        manifest = {
            ...
            "confirmed_zero_physical": {
                "contract_name": CONFIRMED_ZERO_PHYSICAL_CONTRACT,
                "pool": TARGET_POOL,
                "target_nm_ids": list(TARGET_NM_IDS),
                "expected_missing_nm_ids": expected_missing_nm_ids,
                "expected_existing_zero_nm_ids": expected_existing_zero_nm_ids,
...
def _read_snapshot(db_path: Path) -> dict[str, Any]:
    with closing(_open_query_only(db_path)) as conn:
        ...
        elif not bool(facility.get("active")) or str(facility.get("city") or "") != TARGET_FACILITY_CITY:
        ...
        if not set(TARGET_NM_IDS).issubset(active_nm_ids):
            blockers.append("one or more target SKU are not active in the current registry")
        nomenclature_rows = [
            dict(row)
            for row in conn.execute(
    ...
    with closing(_open_query_only(db_path)) as conn:
        request = conn.execute(
            f"""SELECT request_id,state,document_kind,source_system,source_type,
                ...
                LIMIT 1""",
            (CONFIRMED_ZERO_PHYSICAL_SOURCE_TYPE, SOURCE_ID),
        ).fetchone()
        if request is None:
        ...
        confirmed_zero = dict(preview_manifest.get("confirmed_zero_physical") or {})
        return {
            ...
            "approval_reference": str(
                confirmed_zero.get("approval_reference") or ""
            ),
...
def _verify_completed_readback(
    ...
    *,
    ...
    if require_document and (
        ...
        or document.get("source_type") != CONFIRMED_ZERO_PHYSICAL_SOURCE_TYPE
