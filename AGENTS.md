...
[Канонический монитор исполняемых/ожидающих PR](https://github.com/orenvlad-ai/wb-core/pulls?q=is%3Apr+-label%3Arelease%3Asuperseded+label%3A%22release%3Aready%2Crelease%3Arunning%2Crelease%3Aawaiting-agent%2Crelease%3Aawaiting-ui%2Crelease%3Aneeds-resume%2Crelease%3Ablocked%2Crelease%3Ahalted%2Cfinance%3Amigration-deploy-lease%22+sort%3Acreated-asc) не ограничивается `is:open`: merged LOOP PR с `release:awaiting-ui` и terminal production PR, который держит repo-owned global Finance migration deploy lease, остаются активными глобальными gates. Монитор исключает `release:superseded`; обычные terminal `release:production`/`release:done` без отдельного lease не включаются; сортировка остаётся `created-asc`. Finance lease атомарно приобретается только trusted-main workflow при отсутствии running/awaiting/halted deploy: audit guard ставится вместе с active hold до binding proof, поэтому partial acquire или потеря одного label остаются fail-closed. Lease никогда не auto-expires open, блокирует unrelated selection/merge/deploy и разрешает только exact owner-bound recovery PR. Stale, lost или ambiguous lease остаётся fail-closed; recovery deploy требует exact rebind и инвалидирует прежние baseline/snapshot/plan/fingerprint. Release/abort разрешён только после repo-owned exact reconciliation полного writer/timer/policy/manual-barrier/SHA/non-target restore.
