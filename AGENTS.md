...
На поверхности с фактически доступной thread-heartbeat capability для каждой новой нетерминальной задачи, создаваемой либо получаемой в интерфейсе, обязателен ровно один 10-минутный monitor exact target task/thread identity. Preferred role при Chat → Codex — external supervisor heartbeat в инициирующем Chat/потоке: его durable prompt содержит exact target identity, он независимо читает target status и может отчитываться, пока target turn active. Принимающий Codex при первой безопасной возможности проверяет existing external/self monitor; создаёт preferred supervisor только после появления пригодных target и monitor thread identities, а self-heartbeat внутри target использует лишь как mutually-exclusive fallback, когда external supervisor технически недоступен. Capability определяется callable contract, а не названием macOS/Desktop/CLI/IDE/пути. Если capability недоступна, обычная задача продолжается без blocker и без ложного monitor claim.

Create/update выполняется только поддерживаемым automation tool без hardcoded schedule syntax. После операции обязателен readback: `ACTIVE`, cadence 10 минут, правильный monitor/destination thread и exact target identity в durable prompt; mismatch исправляется update существующей automation, не duplicate create, а external supervisor и self-heartbeat никогда не работают одновременно для одной target identity. Supervisor при active target только читает доказанный progress, даёт краткий отчёт и не отправляет follow-up; при idle/non-terminal отправляет ровно один bounded follow-up на ближайшее безопасное действие; при human-only boundary сообщает exact blocker. Self-heartbeat умеет только возобновить idle target и не обещается как независимый active-turn reporter.

Каждый осмысленный supervisor run сообщает: `Прогресс ≈<процент>% · ETA ≈<диапазон> · сделано: <одна короткая фраза>.`; при внешнем ожидании используется `ETA ≈зависит от <точная внешняя зависимость>`. Для PR-backed `wb-core` durable state остаётся в GitHub, а возобновлённый target использует canonical waiter/shepherd и его dispositions: monitor не создаёт второй state machine, не заменяет 5-минутное GitHub observation и не обходит `release:needs-resume`/ownership/continuity. После доказанных terminal success, terminal failure или explicit user stop supervisor останавливает/удаляет monitor по supported contract. Для локальных файлов компьютер и Desktop должны быть запущены, а проект — оставаться доступным.
