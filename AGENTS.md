...
3. Переименовать и закрепить curator/executor, затем атомарно зарегистрировать exact identities через `apps/codex_task_orchestrator.py register-task`.
...
Watcher публикует по каждой active task только жёсткий формат: `Статус`, `Задача`, `Прогресс · Осталось`, `С прошлого отчёта`, `Сейчас`, и `Блокер` только при доказанном human-only состоянии. `ACCEPTED` после фразы владельца «Задача принята» исключается из следующего отчёта; UI unpin остаётся ручным. Chat history не является state: rotation выполняется по generation/run-count через prepare → smoke → atomic activate → old-generation no-op → archive.
