...
The report pins unchanged baseline fingerprint; target header/line, non-target and pre-cutover digests; exact canonical changed-row counts; target stage/physical/paid-equivalent/recognized-capital/paid-capital/coverage/confirmation snapshots and deltas at cutover/current date; second-run zero change; and all preserved source families. Apply requires the exact current fingerprint plus explicit backup directory, creates a `0600` online backup, uses one `BEGIN IMMEDIATE` transaction for source header, derived status cache, canonical materialization and correction audit, preserves the live SQLite inode, and restores the backup in place if post-commit integrity/idempotency verification fails. Repeating the same authorized correction after success is zero-change.
