...
| Default-off facility × pool document posting | one immutable request/document graph, Stage 1 movements and exact affected pool balances | `0` full-store bytes | durable request lifecycle resumes `processing` and finalizes `posted/replay`; exact semantic repeat is T0 | T1 | request and affected balance before-images plus append-only undo manifest | request/document/root ids, recovery operation and immutable readback |
