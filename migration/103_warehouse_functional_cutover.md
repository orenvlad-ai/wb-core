...
6. Run `warehouse-functional-economics-dry-run --output <absolute plan.json>`, review its target cells, centrally enumerated archived rows and non-target digest, then apply the exact plan with `warehouse-functional-economics-apply --plan-file <same plan.json> --fingerprint <exact sha256:...>`. The backfill requires exact `DATA_VITRINA` header/date alignment and indexes only stable `scope|metric` projection rows; it removes only archived public metrics, preserves legacy presentation-only rows and all other non-target data, while duplicate stable keys fail closed with snapshot identity. Require an idempotent second dry-run with zero changes. Then run the bounded WB sync and require readback with no missing dates from 2026-07-01. Enable the hourly timer only after both reconcile.
...
If an already-active legacy functional version has no version-scoped supplier certification rows, do not force an unrelated WB rebuild and do not insert into the immutable version base table. Use the repo-owned `warehouse-functional-supplier-certification-dry-run/apply` contour. It may append only exact active-version supplier correction/audit rows proven by either frozen fingerprints or target-scoped exact per-SKU quantity/capital and contributing document identities in immutable balance provenance; every mutable target source row must also have a server-owned revision timestamp strictly earlier than the immutable version, while equal whole-second timestamps are ambiguous and fail closed. Global source-group watermarks and the broader local digest are audit-only because unrelated informational documents and WB/FF/history projections can advance them; the complete current supplier-source manifest remains pinned in the plan and is rechecked atomically after source/non-target recheck and coherent backup. Missing proof or changed target allocation/revision requires a newly calculated version and must not certify stale WAC/capital; primary invoices, CNY/payment rows, financial documents, supplier shipments, FF ledger, WB supplies and raw WB snapshots remain digest-invariant. Prepare rollback with the paired exact-fingerprint tombstone command and require exact-repeat no-op plus authenticated UI readback.
