...
4. Apply the exact plan/fingerprint. The runner must make another uncached official snapshot, optimistically recheck sources, create a coherent `0600` SQLite backup with `integrity_check=ok`, and publish all derived state plus initial settings inside one `BEGIN IMMEDIATE` transaction.
